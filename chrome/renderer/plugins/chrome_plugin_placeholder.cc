...
void ChromePluginPlaceholder::PluginListChanged() {
  ...
  blink::WebPlugin* new_plugin = ChromeContentRendererClient::CreatePlugin(
      render_frame()->GetWebFrame(), render_frame(), GetPluginParams(), *plugin_info);
...
blink::WebPlugin* ChromePluginPlaceholder::CreatePlugin() {
  ...
  return render_frame()->CreatePlugin(render_frame()->GetWebFrame(), GetPluginInfo(), GetPluginParams(),
                                      std::move(throttler));
}
