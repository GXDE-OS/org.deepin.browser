...
void ExternalProcessImporterHost::ShowWarningDialog() {
  ...
  // add by hanll, 给弹框添指定父窗口指针, 2020/10/12, start
  int browserCount = BrowserList::GetInstance()->size();
  //LOG(ERROR) << "BrowserList::GetInstance().size():" << browserCount;
  for (int bi = 0; bi < browserCount; bi++) {
    Browser* bw = BrowserList::GetInstance()->get(bi);
    Profile* p = bw->profile();
    if(p&&profile_&&(p == profile_)){
      content::WebContents* web_contents = bw->tab_strip_model()->GetActiveWebContents();
      if(web_contents){
        gfx::NativeWindow web_window = web_contents->GetTopLevelNativeWindow();
        set_parent_window(web_window);
      }
    }
  }
  // add by hanll, 给弹框添指定父窗口指针, 2020/10/12, end
