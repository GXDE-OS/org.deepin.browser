...
#include "chrome/browser/ssl/ssl_client_certificate_selector_gm.h"
#endif
...
// GMTLS
#ifndef OPENSSL_NO_GMTLS
  if(cert_request_info->is_gm_){
    skfmodule * out_skf_module;
    X509_NAME * out_issuer;
    if(chrome::CheckModulesAndDevices(web_contents, cert_request_info,&out_skf_module,&out_issuer)){
      return chrome::ShowClientSSLPinDialog(web_contents, cert_request_info, std::move(delegate),out_skf_module,out_issuer);
    }
    return base::OnceClosure();
  }
#endif
...
#if 0
  additional_allowed_schemes->push_back(content::kUosUIScheme);
#endif
  for (size_t i = 0; i < extra_parts_.size(); ++i) {
