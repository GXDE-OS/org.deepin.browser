...
Polymer({
  ...
  ready() {
    ...
    this.selectName = settings.SortMethod.MOST_VISITED;
    if (this.storagePressureFlagEnabled_) {
      ...
      if (Object.values(this.sortMethods_).includes(sortParam)) {
        this.selectName = sortParam;
      }
    }
    this.selectValue = this.returnSelectValue(this.selectName);
    this.sortMethod_ = this.selectName;
  },
  /** @private */
  ...
  selectClick: function(e){
    ...
    this.$$('#visited').classList.remove('select');
    this.$$('#storage').classList.remove('select');
    this.$$('#name').classList.remove('select');
    this.selectName = dataValue;
    this.selectValue = this.returnSelectValue(this.selectName);
    this.sortMethod_ = this.selectName;
    this.filteredList_ =
  ...
  returnSelectValue(value){
    if (value == settings.SortMethod.MOST_VISITED) {
      this.$$('#visited').classList.add('select');
      return this.i18n('siteSettingsAllSitesSortMethodMostVisited');
    } else if (value == settings.SortMethod.STORAGE) {
      this.$$('#storage').classList.add('select');
      return this.i18n('siteSettingsAllSitesSortMethodStorage');
    } else if (value == settings.SortMethod.NAME) {
      this.$$('#name').classList.add('select');
      return this.i18n('siteSettingsAllSitesSortMethodName');
    }
  },
  /** @override */
  attached() {
  ...
  sortSiteGroupList_(siteGroupList) {
    const sortMethod = this.selectName;
