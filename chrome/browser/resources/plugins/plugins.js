...
function isPluginPolicyClickToPlay(plugin) {
    return plugin.policyClickToPlay == true;
}
...
function initializeProxies() {
    browserProxy = new plugins.mojom.PluginsPageHandlerPtr;
    Mojo.bindInterface(
      plugins.mojom.PluginsPageHandler.name, mojo.makeRequest(browserProxy).handle);

    /** @constructor */
    var PluginsPageImpl = function(request) {
      this.binding_ = new mojo.Binding(plugins.mojom.PluginsPage, this, request);
    };
    ...
    var client = new plugins.mojom.PluginsPagePtr;
    pageImpl = new PluginsPageImpl(mojo.makeRequest(client));
...
function main() {
  setupFn().then(function() {
    ...
    loadShowDetailsFromPrefs(response.showDetails);
