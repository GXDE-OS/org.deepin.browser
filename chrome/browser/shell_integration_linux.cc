...
bool SetDefaultWebClient(const std::string& protocol) {
  //modify by xiepinze, set default browser, 2020/08/26, start
  // Mime dbusmgr("com.deepin.daemon.Mime", "/com/deepin/daemon/Mime",
                              //  QDBusConnection::sessionBus(), nullptr);
  // dbusmgr.SetDefaultApp(mimeType, browserid);
  return dbus::uos::DBusThreadManager::Get()->GetDdeMimeClient()->SetUosDefaultBrowser();
...
#endif
*/
  //modify by xiepinze, set default browser, 2020/08/26, end
}
...
// |protocol|.
shell_integration::DefaultWebClientState GetIsDefaultWebClient(const std::string& protocol) {
  // modify by wzx, set default browser, 2021/01/13, start
  bool bIsDefault = dbus::uos::DBusThreadManager::Get()->GetDdeMimeClient()->IsUosDefaultBrowser();
  return bIsDefault ? shell_integration::IS_DEFAULT : shell_integration::NOT_DEFAULT;
  // modify by wzx, set default browser, 2021/01/13   end
}
  // modify by xiepinze, set default browser, 2020/08/26, start
  // Mime dbusmgr("com.deepin.daemon.Mime", "/com/deepin/daemon/Mime",
                              //  QDBusConnection::sessionBus(), nullptr);

  // auto reply = dbusmgr.GetDefaultApp(mimeType[0]);
  // auto defaultId = QJsonDocument::fromJson(reply.value().toStdString().c_str()).object()["Id"].toString();
  // if (defaultId == browserid)
    // return shell_integration::IS_DEFAULT;
  // else
...
std::string GetIconName() {
  // add by hwb fix [67176] start  
  return "org.deepin.browser";
  // add by hwb fix [67176] end
...
base::FilePath GetWebShortcutFilename(const GURL& url) {
  ...
  std::string filename =
     std::string(chrome::kUOSBrowserProcessExecutableName) + "-" + url.spec();
  // add by hwb fix [67176] end       
...
std::vector<base::FilePath> GetExistingProfileShortcutFilenames(
  ...
  std::string prefix(chrome::kUOSBrowserProcessExecutableName);
  // add by hwb fix [67176] end  
  prefix.append("-");
  std::string suffix("-");
