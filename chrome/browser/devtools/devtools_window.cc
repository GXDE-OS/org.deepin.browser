...
void DevToolsWindow::ToggleDevToolsWindow(
    ...
    const DevToolsToggleAction& action) {
#if 0
  // wangjuna:内置页面禁止开发者工具，2020/09/15，--start
  auto webcontent = browser->tab_strip_model()->GetActiveWebContents();
  if (webcontent) {
    if (webcontent->GetURL().SchemeIs(content::kChromeUIScheme)) {
      std::string temp;
      std::unique_ptr<base::Environment> environment(base::Environment::Create());
      if (!(environment.get())->GetVar("UOS_BROWSER_DEBUG", &temp)) {
        return;
      }
    }
  }
  // wangjuna:内置页面禁止开发者工具，2020/09/15，--end
#endif
