...
void ExtensionService::Init() {
  ...
  for (const crx_file::UOSDefaultExtension& path : crx_file::GetUOSDefaultExtensionPath()) {
    std::string input;
    ...
    if (root) {
      std::unique_ptr<ExtensionSet> all_extensions = registry_->GenerateInstalledExtensionsSet();
      scoped_refptr<CrxInstaller> installer(CrxInstaller::CreateSilent(this));
      installer->set_allow_silent_install(true);
      installer->set_is_gallery_install(true);
      installer->set_install_immediately(true);
      installer->set_off_store_install_allow_reason(CrxInstaller::OffStoreInstallAllowedBecausePref);

      for (auto it = root->begin(); it != root->end(); ++it) {
        if (std::find(path.allow_list.begin(), path.allow_list.end(), it->first) == path.allow_list.end()) {
          continue;
        }
        std::string crx_file = it->first + ".crx";
        if (all_extensions->Contains(it->first)) {
          base::Version old_version = base::Version(all_extensions->GetByID(it->first)->VersionString());
          base::Value *value = it->second->FindKey("version");
          if (value && base::Version(value->GetString()).CompareTo(old_version) > 0) {
            LOG(INFO) << "upgrade " << it->first << " from " << old_version.GetString() << " to " << value->GetString();
            installer->InstallCrx(base::FilePath(path.pathname).Append(crx_file));
          }
        } else {
          installer->InstallCrx(base::FilePath(path.pathname).Append(crx_file));
        }
