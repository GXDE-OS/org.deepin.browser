...
base::Optional<ThemeHelper::OmniboxColor> ThemeHelper::GetOmniboxColorImpl(
  ...
  switch (id) {
    ...
    // modify by fangzhouyang, Use same color in both dark and light theme, 2021/3/29 --start
    #if 0
      return dark ? blend_toward_max_contrast(fg, 0x18)
                  : security_chip_color({gfx::kGoogleRed600, false});
    #endif
      if (dark){
        return {{SkColorSetRGB(0x9A, 0x2F, 0x2F), false}};
      } else {
        return security_chip_color({gfx::kGoogleRed600, false});
      }
    // modify by fangzhouyang, Use same color in both dark and light theme, 2021/3/29 --end
    default:
      return base::nullopt;
  }
}
