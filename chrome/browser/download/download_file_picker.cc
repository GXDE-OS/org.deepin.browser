...
DownloadFilePicker::DownloadFilePicker(DownloadItem* item,
  ...
  // @wangjun: fix bug#73518, modal dialog implemented in select_file_dialog_impl_gtk.cc, start, 2021/04/23
  if (!owning_window->GetHost()) {
    LOG(INFO) << " Find ActiveWindow as owning_window, for extension chrome.downloads.download";
    owning_window = chrome::FindBrowserWithActiveWindow()->window()->GetNativeWindow();
  }
  // @wangjun: fix bug#73518, modal dialog implemented in select_file_dialog_impl_gtk.cc, end, 2021/04/23
