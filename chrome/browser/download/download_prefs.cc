...
void DownloadPrefs::RegisterProfilePrefs(
  ...
  registry->RegisterFilePathPref(prefs::kUsbKeyDirectory,
                                 default_download_path);
#endif
...
const base::FilePath& DownloadPrefs::GetDefaultDownloadDirectory() {
  base::FilePath path = GetDefaultDownloadDirectorySingleton().path();
  LOG(INFO)<<"DownloadPrefs::GetDefaultDownloadDirectory:"<<path.MaybeAsASCII();
...
base::FilePath DownloadPrefs::DownloadPath() const {
  base::FilePath path = SanitizeDownloadTargetPath(*download_path_);
  LOG(INFO)<<"DownloadPrefs::DownloadPath:"<<path.MaybeAsASCII();
  return SanitizeDownloadTargetPath(*download_path_);
}

void DownloadPrefs::SetDownloadPath(const base::FilePath& path) {
  LOG(INFO)<<"DownloadPrefs::SetDownloadPath:"<<path.MaybeAsASCII();
  download_path_.SetValue(path);
  SetSaveFilePath(path);
}

base::FilePath DownloadPrefs::SaveFilePath() const {
  base::FilePath path = SanitizeDownloadTargetPath(*save_file_path_);
  LOG(INFO)<<"DownloadPrefs::SaveFilePath:"<<path.MaybeAsASCII();
...
bool DownloadPrefs::PromptForDownload() const {
  ...
  LOG(INFO)<<"询问保存地址:"<<prompt_for_download_.GetValue();
...
void DownloadPrefs::SetShouldOpenPdfInSystemReader(bool should_open) {
  LOG(INFO)<<"DownloadPrefs::SetShouldOpenPdfInSystemReader:"<<should_open;
