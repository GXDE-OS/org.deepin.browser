...
void RenderViewContextMenu::InitMenu() {
  ...
  if (editable && params_.misspelled_word.empty() &&
      !content_type_->SupportsGroup(
          ContextMenuContentType::ITEM_GROUP_MEDIA_PLUGIN)) {
    menu_model_.AddSeparator(ui::NORMAL_SEPARATOR);
    // delete by hanll, 去掉拼写检查菜单项, 2020/08/24, start
    #if 0
    AppendLanguageSettings();
    #endif
    // delete by hanll, 去掉拼写检查菜单项, 2020/08/24, end
...
void RenderViewContextMenu::AppendDeveloperItems() {
  ...
  // delete by hanll, 去掉查看框架的源代码和重新加载框架菜单项, 2020/07/21
  // --start
  ...
  // delete by hanll, 去掉查看框架的源代码和重新加载框架菜单项, 2020/07/21 --end
...
void RenderViewContextMenu::AppendPageItems() {
  ...
  // delete by xiepinze, disable google translate, 2020/07/08 --start
  /*
  ...
  */
  // delete by xiepinze, disable google translate, 2020/07/08 --end
}
...
void RenderViewContextMenu::AppendMediaRouterItem() {
  // delete by hanll, 去掉网页右键投影菜单项, 2020/07/27 --start
  #if UNUSE
  ...
  #else
  #endif
  // delete by hanll, 去掉网页右键投影菜单项, 2020/07/27 --end
}
...
void RenderViewContextMenu::AppendEditableItems() {
  ...
  // delete by hanll, 去掉联想纠错菜单项, 2020/09/28, start
#if 0
  if (use_spelling)
    AppendSpellingSuggestionItems();
#endif
  // delete by hanll, 去掉联想纠错菜单项, 2020/09/28, end
