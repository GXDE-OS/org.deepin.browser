...
bool CheckUrlAgainstWhitelist(
  ...
  //@wangjun: 不检查rar后缀的文件，2021/04/12,start
  DownloadFileType::InspectionType inspection_type =
      FileTypePolicies::GetInstance()
          ->PolicyForFile(target_path)
          .inspection_type();

  LOG(INFO) << "CheckUrlAgainstWhitelist target_path: " << target_path.LossyDisplayName();
  LOG(INFO) << "CheckUrlAgainstWhitelist url: " << url.spec();
  if (inspection_type == DownloadFileType::RAR) {
    LOG(INFO) << "CheckUrlAgainstWhitelist NO CHECK FOR RAR FILES By target_path";
    return true;
  }

  std::string filename_ = url.ExtractFileName();
  if (filename_.length() > 0) {
    std::transform(filename_.begin(), filename_.end(), filename_.begin(), ::tolower);
    if (filename_.substr(filename_.find_last_of(".") + 1) == "rar") {
      LOG(INFO) << "CheckUrlAgainstWhitelist NO CHECK FOR RAR FILES By url";
      return true;
    }
  }
  //@wangjun: 不检查rar后缀的文件，2021/04/12,end
...
void CheckClientDownloadRequestBase::Start() {
  ...
  base::PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&CheckUrlAgainstWhitelist, source_url_, target_file_path_, database_manager_),
