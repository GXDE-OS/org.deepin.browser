...
bool IsPDFViewerPlugin(const base::string16& plugin_name) {
  return (plugin_name ==
         ...
         (plugin_name ==
          base::ASCIIToUTF16(ChromeContentClient::kPDFSuwellPluginName)) ||
         (plugin_name ==
          base::ASCIIToUTF16(ChromeContentClient::kPDFFoxitPluginName)) ||
         (plugin_name ==
...
base::FilePath PluginPrefs::PluginState::ConvertMapKey(
  ...
  if (IsComponentUpdatedPepperFlash(plugin)) {
    ...
    if (base::PathService::Get(chrome::FILE_PEPPER_FLASH_PLUGIN,
...
void PluginPrefs::EnablePluginGroupInternal(
  ...
  base::PostTask(FROM_HERE, {BrowserThread::UI},
      base::BindOnce(&PluginPrefs::OnUpdatePreferences, this, plugins));
  base::PostTask(FROM_HERE, {BrowserThread::UI},
      base::BindOnce(&PluginPrefs::NotifyPluginStatusChanged, this));
}
