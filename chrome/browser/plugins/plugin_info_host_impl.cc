...
void PluginInfoHostImpl::RegisterUserPrefs(
  ...
  // Set kPluginsAllowOutdated, kRunAllFlashInAllowMode to true for np test by wangweiwei
  registry->RegisterBooleanPref(prefs::kPluginsAllowOutdated, true);
  registry->RegisterBooleanPref(prefs::kRunAllFlashInAllowMode, true);
}
...
void PluginInfoHostImpl::Context::DecidePluginStatus(
  ...
  // allow flash plugin, remove html placeholder
  if (plugin_identifier.find("adobe-flash-player") != std::string::npos) {
    *status = chrome::mojom::PluginStatus::kAllowed;
    return;
  }
#endif
