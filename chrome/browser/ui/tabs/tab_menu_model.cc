...
void TabMenuModel::Build(TabStripModel* tab_strip, int index) {
  ...
  AddItemWithStringId(TabStripModel::CommandNewTabToRight,IDS_OPEN_NEWTAB);
  ...
  //modify by xiaohuyang, reload all tabs, add reload all tabs action(item). 2020/07/09 --start
  //command id 'CommandReloadAll' and action name 'IDS_TAB_CXMENU_RELOADALL' are added manually
  AddItemWithStringId(TabStripModel::CommandReloadAll, IDS_TAB_CXMENU_RELOADALL);
  //modify by xiaohuyang, reload all tabs, add reload all tabs action(item). 2020/07/09 --end
  ...
  AddItemWithStringId(
      ...
      will_pin ? IDS_PIN_THIS_TAB : IDS_UNPIN_THIS_TAB);
  ...
  AddSeparator(ui::NORMAL_SEPARATOR);
  AddItemWithStringId(TabStripModel::CommandCloseTab, IDS_CLOSE_THIS_TAB);
