...
// modify by xiaohuyang, fix bug#84477,  2021/06/30
// Use application Locale and define some favicon url constants.
#if 1
...
gfx::Image ChromeOmniboxClient::GetFaviconForDefaultSearchProvider(
  ...
  // modify by xiaohuyang, fix bug#84477,  2021/06/30  --start
  // Because the favicon url corresponding to the "bing" search engine is redirected, special processing is required when reading the data.
#if UNUSED
  return favicon_cache_.GetFaviconForIconUrl(default_provider->favicon_url(),
                                             std::move(on_favicon_fetched));
#else
  GURL favicon_url = default_provider->favicon_url();

  if (g_browser_process->GetApplicationLocale() == "zh-CN" && 
    default_provider->favicon_url().spec() == bing_favicon_url) {
  ...
  return favicon_cache_.GetFaviconForIconUrl(favicon_url,
                                             std::move(on_favicon_fetched));
#endif
  // modify by xiaohuyang, fix bug#84477,  2021/06/30  --end
}
