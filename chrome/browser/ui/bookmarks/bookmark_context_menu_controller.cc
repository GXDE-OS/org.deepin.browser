...
void BookmarkContextMenuController::BuildMenu() {
  ...
  // delete by hanll, 去掉剪切菜单项, 2020/07/20 --start
  // AddItem(IDC_CUT, IDS_CUT);
  AddItem(IDC_COPY, IDS_COPY);
  AddItem(IDC_PASTE, IDS_PASTE);
  // delete by hanll, 去掉剪切菜单项, 2020/07/20 --end
  ...
  AddSeparator();
  //add by hanll, 增加替换为当前网址, 2020/07/14 --start
  if (selection_.size() == 1 && selection_[0]->is_url()) {
    AddItem(IDC_BOOKMARK_BAR_REPLACEINTO_CURRENTURL, IDS_BOOKMARK_BAR_REPLACEINTO_CURRENTURL);
  }
  //add by hanll, 增加替换为当前网址, 2020/07/14 --end
  ...
  AddSeparator();
  #if 0
  //add by hanll, 增加书签右键菜单按名称排序功能, 2020/07/23 --start
  //AddItem(IDC_BOOKMARK_BAR_ORDER_BY_NAME, IDS_BOOKMARK_ORDERBYNAME);
  //add by hanll, 增加书签右键菜单按名称排序功能, 2020/07/23 --end
  #endif
  AddItem(IDC_BOOKMARK_MANAGER, IDS_BOOKMARK_MANAGER);

  //delete by hanll, 去掉应用快捷方式菜单项, 2020/07/20 --start
  ...
  //delete by hanll, 去掉应用快捷方式菜单项, 2020/07/20 --end
...
void BookmarkContextMenuController::ExecuteCommand(int id, int event_flags) {
  ...
  switch (id) {
    ...
    //add by hanll, 增加替换为当前网址, 2020/07/14 --start
    case IDC_BOOKMARK_BAR_REPLACEINTO_CURRENTURL: {
      base::RecordAction(UserMetricsAction("BookmarkBar_ContextMenu_RepaceIntoCurrentUrl"));
      //LOG(ERROR)<<"IDC_BOOKMARK_BAR_REPLACEINTO_CURRENTURL";
      //size_t index;
      //const BookmarkNode* parent = bookmarks::GetParentForNewNodes(parent_, selection_, &index);
      GURL url;
      base::string16 title;
      chrome::GetURLAndTitleToBookmark(browser_->tab_strip_model()->GetActiveWebContents(),&url, &title);

      //LOG(ERROR)<<"selection_.size:"<<selection_.size();      
      //LOG(ERROR)<<"url:"<<url;
      //LOG(ERROR)<<"title:"<<title;

      if(selection_.size()>0)
      {
        bookmarks::BookmarkNode* updatenode = const_cast<bookmarks::BookmarkNode*>(selection_[0]);
        model_->SetURL(updatenode, GURL(url));
        model_->SetTitle(updatenode, title);
      }
    ...
    #if 0
    //add by hanll, 增加书签右键菜单按名称排序功能, 2020/07/23 --start
    case IDC_BOOKMARK_BAR_ORDER_BY_NAME: {
        LOG(ERROR) << "exec-IDC_BOOKMARK_BAR_ORDER_BY_NAME";

        LOG(ERROR) << "selection_.size:" << selection_.size();
        if (selection_.size() > 0) {
            size_t index;
            const BookmarkNode *parent = bookmarks::GetParentForNewNodes(parent_, selection_, &index);
            LOG(ERROR) << "parent->id:" << parent->id();
            LOG(ERROR) << "parent->guid:" << parent->guid();
            LOG(ERROR) << "parent->GetTitledUrlNodeTitle:" << parent->GetTitledUrlNodeTitle();
