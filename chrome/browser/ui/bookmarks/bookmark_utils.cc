...
GURL GetURLToBookmark(content::WebContents* web_contents) {
  ...
  return search::IsInstantNTP(web_contents) ? GURL(kChromeUINewTabURL)
                                            : web_contents->GetURL();
#else
  return search::IsInstantNTP(web_contents) ? GURL("uos://newtab/")
                                            : web_contents->GetVisibleURLForUOS();
#endif
}
...
bool ShouldShowAppsShortcutInBookmarkBar(Profile* profile) {
  //modify by hanll, 书签栏上不展示应用图标, 2020/07/20 --start
#if UNUSE
  ...
  return false;
#endif
  //modify by hanll, 书签栏上不展示应用图标, 2020/07/20 --end
}
...
gfx::ImageSkia GetBookmarkFolderIcon(SkColor text_color) {
  ...
  folder = *ui::ResourceBundle::GetSharedInstance()
                ...
                .ToImageSkia();
#else
  // modify by xiaohuyang, Set bookmark folder icon, 2020/08/27 --start
  #if 0
  ...
  if(ui::TouchUiController::Get()->touch_ui()){
    folder = GetFolderIcon( vector_icons::kFolderTouchIcon,
                         text_color);
  }else{
    folder = *ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_BOOKMARK_FOLDER_ICON);
  }
  #endif
  // modify by xiaohuyang, Set bookmark folder icon, 2020/08/27 --end
#endif
