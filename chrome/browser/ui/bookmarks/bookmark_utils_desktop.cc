...
//add by wangfeng, pop menu, bug(54453) 2020/11/16 --start
bool ShouldOpenAll(gfx::NativeWindow parent,
                   const std::vector<const BookmarkNode*>& nodes,
  ...
  if (initial_disposition == WindowOpenDisposition::OFF_THE_RECORD)
    child_count = GetURLsToOpen(nodes, browser_context, browser_context != nullptr).size();
  else
    child_count = GetURLsToOpen(nodes).size();
...
//add by wangfeng, pop menu, bug(54453) 2020/11/16 --end
#endif
...
#else
//add by wangfeng, pop menu, bug(54453) 2020/11/16 --start
  if (!ShouldOpenAll(parent, nodes, initial_disposition, browser_context))
    return;
//add by wangfeng, pop menu, bug(54453) 2020/11/16 --end
#endif
