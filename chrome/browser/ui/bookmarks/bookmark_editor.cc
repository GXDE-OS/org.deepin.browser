...
const BookmarkNode* CreateNewNodeForUos(BookmarkModel* model,
                                  const BookmarkNode* parent,
                                  const BookmarkEditor::EditDetails& details,
                                  const base::string16& new_title,
                                  const GURL& new_url) {
  ...
  size_t insert_index =
      (parent == details.parent_node && details.index.has_value() &&
       details.index.value() <= child_count)
          ? details.index.value()
          : child_count;
  // modify by hanll, 新添加的书签都添加到文件夹最后一个, 2020/09/14, start
  insert_index = child_count;
  // modify by hanll, 新添加的书签都添加到文件夹最后一个, 2020/09/14, end
  if (details.type == BookmarkEditor::EditDetails::NEW_URL) {
    node = model->AddURL(parent, insert_index, new_title, new_url);
  } else if (details.type == BookmarkEditor::EditDetails::NEW_FOLDER) {
    node = model->AddFolder(parent, insert_index, new_title);
...
int BookmarkEditor::EditDetails::GetWindowTitleId() const {
  ...
  switch (type) {
    case EditDetails::EXISTING_NODE:{
          ...
          if(existing_node->type() == BookmarkNode::FOLDER){
            LOG(INFO)<<"EditDetails::EXISTING_NODE:"<<"modify Folder";
          }
          else if(existing_node->type() == BookmarkNode::URL){
            LOG(INFO)<<"EditDetails::EXISTING_NODE:"<<"modify URL";
          }
          else
          {
            LOG(INFO)<<"EditDetails::EXISTING_NODE:"<<"other";
          }
    ...
    case EditDetails::NEW_URL:{
          ...
          dialog_title = (type == EditDetails::EXISTING_NODE &&
                      existing_node->type() == BookmarkNode::FOLDER) ?
          IDS_BOOKMARK_FOLDER_EDITOR_WINDOW_TITLE :IDS_BOOKMARK_MANAGER_ADD_BOOKMARK_TITLE;
          //IDS_BOOKMARK_EDITOR_TITLE;
          LOG(INFO)<<"11111111111111111";
          LOG(INFO)<<"EditDetails::NEW_URL"<<"add url";
          break;
      }
    case EditDetails::NEW_FOLDER:
    {
        dialog_title = urls.empty() ?
          ...
          LOG(INFO)<<"EditDetails::NEW_FOLDER"<<"add Folder";
      break;}
    default:
  ...
  LOG(INFO)<<"dialog_title:"<<dialog_title;
  return dialog_title;
}
...
const BookmarkNode* BookmarkEditor::ApplyEditsWithNoFolderChange(
  ...
  if (details.type == EditDetails::NEW_URL ||
    ...
    return CreateNewNodeForUos(model, parent, details, new_title, new_url);
  }
...
const BookmarkNode* BookmarkEditor::ApplyEditsWithPossibleFolderChange(
  ...
  if (details.type == EditDetails::NEW_URL ||
      details.type == EditDetails::NEW_FOLDER) {
    return CreateNewNodeForUos(model, new_parent, details, new_title, new_url);
  }
