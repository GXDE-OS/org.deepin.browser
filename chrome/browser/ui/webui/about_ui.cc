...
std::string ChromeURLs() {
  ...
  // add by hanll, 新增禁用内部网址集合, 2020/08/19 --start
  std::list<std::string> disableurls;
  disableurls.push_back(content::kChromeUIBlobInternalsHost);
  disableurls.push_back(chrome::kChromeUICrashesHost);
  disableurls.push_back(chrome::kChromeUICreditsHost);
  disableurls.push_back(chrome::kChromeUIDevicesHost);
  disableurls.push_back(chrome::kChromeUINewTabPageHost);
  disableurls.push_back(chrome::kChromeUIPolicyHost);
  disableurls.push_back(chrome::kChromeUISuggestionsHost);
  disableurls.push_back(chrome::kChromeUISupervisedUserInternalsHost);
  disableurls.push_back(chrome::kChromeUISystemInfoHost);
  disableurls.push_back(chrome::kChromeUITermsHost);
  disableurls.push_back(chrome::kChromeUIThumbnailListHost);
  disableurls.push_back(content::kChromeUITracingHost);
  // add by hanll, 新增禁用内部网址集合, 2020/08/19 --end

  //modify by xiaohuyang, Added uos:xxx access to the built-in browser page, 2020/07/23 --start
  //Replace 'chrome:xxx' on the about page with 'uos:xxx' to display
  AppendHeader(&html, 0, "Chrome URLs");
  AppendBody(&html);
  html += "<h2>List of Chrome URLs</h2>\n<ul>\n";
  std::vector<std::string> hosts(chrome::kChromeHostURLs, chrome::kChromeHostURLs + chrome::kNumberOfChromeHostURLs);
  std::sort(hosts.begin(), hosts.end());
  for (std::vector<std::string>::const_iterator i = hosts.begin(); i != hosts.end(); ++i){
    //在禁用集合中的不显示
    if((std::find(disableurls.begin(),disableurls.end(),*i) == disableurls.end()))
...
void AboutUIHTMLSource::StartDataRequest(
  ...
  if (source_name_ == chrome::kChromeUIAboutHost) {
    response = ChromeURLs();
