...
#if 1
  // modify by hanll, 去掉书签管理器快捷键文本,  2020/09/01, start
  LOG(INFO)<<"str0:"<<str;
  //base::Erase(str, '&');
  std::size_t i = str.find('(',0);
  if(size_t(-1) != i)
    str = str.erase(i);
  LOG(INFO)<<"str1:"<<str;
  base::Erase(str, '&');
  // modify by hanll, 去掉书签管理器快捷键文本,  2020/09/01, end
#endif
...
content::WebUIDataSource* CreateBookmarksUIHTMLSource(Profile* profile) {
  ...
  static constexpr webui::LocalizedString kStrings[] = {
      ...
      {"name", IDS_BOOKMARK_NAME},
      {"url", IDS_BOOKMARK_URL},
      {"more", IDS_BOOKMARK_MORE},
  };
