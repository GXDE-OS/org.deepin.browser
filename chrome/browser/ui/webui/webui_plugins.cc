...
#include "chrome/browser/ui/webui/webui_plugins.h"
...
bool FundamentalValue::GetAsBoolean(bool* out_value) {
  if (out_value && is_bool())
    *out_value = boolean_value_;
  return is_bool();
}

bool FundamentalValue::GetAsInteger(int* out_value) {
  if (out_value && is_int())
    *out_value = integer_value_;
  return is_int();
}

bool FundamentalValue::GetAsDouble(double* out_value) {
...
FundamentalValue* FundamentalValue::DeepCopy() {
  switch (type()) {
    case Type::BOOLEAN:
...
bool FundamentalValue::Equals(const Value* other) {
  if (other->type() != type())
    return false;
...
StringValue::StringValue(const std::string& in_value)
    : Value(Type::STRING), value_(in_value) {
...
bool StringValue::GetAsString(std::string* out_value) {
  if (out_value)
    *out_value = value_;
  return true;
}

bool StringValue::GetAsString(string16* out_value) {
  if (out_value)
    *out_value = UTF8ToUTF16(value_);
  return true;
}

bool StringValue::GetAsString(const StringValue** out_value) {
  if (out_value)
    *out_value = this;
  return true;
}

StringValue* StringValue::DeepCopy() {
...
content::WebUIDataSource* CreatePluginsUIHTMLSource(Profile* profile) {
  ...
  source->UseStringsJs();
  source->AddResourcePath("plugins.js", IDR_UOS_PLUGINS_JS);
  source->SetDefaultResource(IDR_UOS_PLUGINS_HTML);
#if defined(OS_CHROMEOS)
...
class PluginsDOMHandler : public WebUIMessageHandler,
 ...
 private:
  ...
  typedef std::unordered_map<std::string, std::vector<const WebPluginInfo*>>
      PluginGroups;
...
PluginsDOMHandler::PluginsDOMHandler() : weak_ptr_factory_(this) {}
...
void PluginsDOMHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      "enablePlugin", base::Bind(&PluginsDOMHandler::HandleEnablePluginMessage,
                                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "setPluginAlwaysAllowed",
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::Bind(&PluginsDOMHandler::HandleSaveShowDetailsToPrefs,
                 base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getShowDetails", base::Bind(&PluginsDOMHandler::HandleGetShowDetails,
                                   base::Unretained(this)));
                                   LoadPlugins();
}
...
void PluginsDOMHandler::HandleGetShowDetails(const base::ListValue* args) {
   ...
   web_ui()->CallJavascriptFunctionUnsafe("loadShowDetailsFromPrefs", show_details);
}
...
void PluginsDOMHandler::HandleSetPluginAlwaysAllowed(
  ...
  Profile* profile = Profile::FromWebUI(web_ui());
  ProfileIOData* io_data = ProfileIOData::FromResourceContext(profile->GetResourceContext());
  io_data->GetHostContentSettingsMap()->SetContentSettingCustomScope(
      ContentSettingsPattern::Wildcard(), ContentSettingsPattern::Wildcard(),
      ContentSettingsType::PLUGINS, plugin,
...
void PluginsDOMHandler::LoadPlugins() {
  ...
  PluginService::GetInstance()->GetPlugins(base::Bind(
      &PluginsDOMHandler::PluginsLoaded, weak_ptr_factory_.GetWeakPtr()));
}
  ...
  for (PluginGroups::const_iterator it = groups.begin(); it != groups.end();
    ...
    for (size_t j = 0; j < group_plugins.size(); ++j) {
      ...
      if (group_plugin.is_pepper_plugin() &&
        ...
        base::PathService::Get(chrome::DIR_PEPPER_FLASH_SYSTEM_PLUGIN, &system_path);
      ...
      const std::vector<content::WebPluginMimeType>& plugin_mime_types = group_plugin.mime_types;
      for (size_t k = 0; k < plugin_mime_types.size(); ++k) {
        ...
        const std::vector<std::string>& mime_file_extensions = plugin_mime_types[k].file_extensions;
        for (size_t l = 0; l < mime_file_extensions.size(); ++l) {
          base::StringValue* file_extension = new base::StringValue(mime_file_extensions[l]);
          file_extensions->Append(std::unique_ptr<base::Value>(file_extension));
          mime_type->SetString("fileExtensions", mime_file_extensions[l]);
        }
        //mime_type->Set("fileExtensions", std::unique_ptr<base::Value>(file_extensions));

        mime_types->Append(std::unique_ptr<base::Value>(mime_type));
      }
      plugin_file->Set("mimeTypes", std::unique_ptr<base::Value>(mime_types));
      ...
      plugin_files->Append(std::unique_ptr<base::Value>(plugin_file));
    }
    base::DictionaryValue* group_data = new base::DictionaryValue();

    group_data->Set("plugin_files", std::unique_ptr<base::Value>(plugin_files));
    ...
    plugin_groups_data->Append(std::unique_ptr<base::Value>(group_data));
  }
  base::DictionaryValue results;
  results.Set("plugins", std::unique_ptr<base::Value>(plugin_groups_data));
  web_ui()->CallJavascriptFunctionUnsafe("returnPluginsData", results);
}
...
PluginsUI::PluginsUI(content::WebUI* web_ui) : WebUIController(web_ui) {
  web_ui->AddMessageHandler(std::make_unique<PluginsDOMHandler>());
