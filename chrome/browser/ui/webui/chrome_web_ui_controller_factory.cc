...
bool IsAboutUI(const GURL& url) {
  return (url.host_piece() == chrome::kChromeUIAboutHost /*||*/
...
WebUIFactoryFunction GetWebUIFactoryFunction(WebUI* web_ui,
  ...
  // wangjuna: disable chrome://local-state, 2020/08/22 --start
#if 0
  if (url.host_piece() == chrome::kChromeUILocalStateHost)
    return &NewWebUI<LocalStateUI>;
#endif
  // wangjuna: disable chrome://local-state, 2020/08/22 --end
  ...
  //wangjuna: disable chrome://ntp-tiles-internals, 2020/08/06 --start
#if 0
  if (url.host_piece() == chrome::kChromeUINTPTilesInternalsHost)
    return &NewWebUI<NTPTilesInternalsUI>;
#endif
  //wangjuna: disable chrome://ntp-tiles-internals, 2020/08/06 --end
  ...
  //wangjuna: disable chrome://safe-browsing, chrome://sync-internals chrome://signin-internals --start
#if 0
...
#endif
  //wangjuna: disable chrome://safe-browsing, chrome://sync-internals chrome://signin-internals --end
  ...
  if (url.host_piece() == chrome::kChromeUIAppLauncherPageHost && profile &&
    ...
    //wangjuna: disable chrome://apps, 20200804 --start
#if 0
  ...
  if (profile->IsGuestSession() &&
      ...
      (
#endif
    //wangjuna: disable chrome://apps, 20200804 --end
  ...
  // add by hanll, 新标签页地址进入正常首页还是隐私首页, 2020/07/28 --start
  if ((url.host_piece() == chrome::kChromeUINewTabHost) &&
      (profile->IsIncognitoProfile()))
    return &NewWebUI<NewTabUI>;
  if ((url.host_piece() == chrome::kChromeUINewTabHost) &&
      (!profile->IsIncognitoProfile()))
    return &NewWebUI<CustomizeNewTabUI>;
  // add by hanll, 新标签页地址进入正常首页还是隐私首页, 2020/07/28 --end

  // delete by hanll, 禁用new-tab-page, 2020/08/19, start
  #if 0
  if (url.host_piece() == chrome::kChromeUINewTabPageHost)
    return &NewWebUI<NewTabPageUI>;
  #endif
  // delete by hanll, 禁用new-tab-page, 2020/08/19, end
  ...
  // delete by hanll, 禁用system, 2020/08/19, start
  #if 0
  if (url.host_piece() == chrome::kChromeUISystemInfoHost)
    return &NewWebUI<SystemInfoUI>;
  #endif
  // delete by hanll, 禁用system, 2020/08/19, end
  ...
  // delete by hanll, 禁用policy, 2020/08/19, start
  #if 0
  if (url.host_piece() == chrome::kChromeUIPolicyHost)
    return &NewWebUI<PolicyUI>;
  #endif
  // delete by hanll, 禁用policy, 2020/08/19, end
#if !defined(OS_ANDROID)
//wangjuna: disable chrome://management, 20200814 --start
#if 0
  if (url.host_piece() == chrome::kChromeUIManagementHost)
    return &NewWebUI<ManagementUI>;
#endif
//wangjuna: disable chrome://management, 20200814 --end
#endif
...
#endif
#if BUILDFLAG(ENABLE_SERVICE_DISCOVERY)
  // delete by hanll, 禁用devices, 2020/08/19, start
  #if 0
  if (url.host_piece() == chrome::kChromeUIDevicesHost) {
    return &NewWebUI<LocalDiscoveryUI>;
  }
  #endif
  // delete by hanll, 禁用devices, 2020/08/19, end
#endif
