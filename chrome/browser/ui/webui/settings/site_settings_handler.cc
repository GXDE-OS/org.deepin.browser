...
void SiteSettingsHandler::HandleSetCategoryPermissionForPattern(
  ...
  ContentSettingsPattern primary_pattern =
      ContentSettingsPattern::FromString(primary_pattern_string);
  if ((primary_pattern.GetScheme() == ContentSettingsPattern::SCHEME_HTTP
      || primary_pattern.GetScheme() == ContentSettingsPattern::SCHEME_HTTPS)
      && primary_pattern.GetPort().empty()) {
    primary_pattern = ContentSettingsPattern::FromStringUOS(primary_pattern_string);
  }
  ContentSettingsPattern secondary_pattern =
      secondary_pattern_string.empty()
          ? ContentSettingsPattern::Wildcard()
