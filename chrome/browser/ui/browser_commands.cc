...
//  浏览器应用自己更新了主题
void UosThemeChange(int command) {
  std::string sTheme = "";
  if(command == IDC_UOS_LIGHT_THEME) {
    sTheme = dbus::uos::kAppearanceGtkThemeLight;
  } else if (command == IDC_UOS_DARK_THEME) {
    sTheme = dbus::uos::kAppearanceGtkThemeDark;
  } else {
    sTheme = dbus::uos::kAppearanceGtkThemeSystem;
  }
  dbus::uos::DBusThreadManager::Get()->GetDdeAppearanceThemeClient()->setAppTheme(sTheme);
  dbus::uos::DBusThreadManager::Get()->GetDdeAppearanceThemeClient()->updateAppThemeColor(sTheme);
}
//add by xiepinze uos theme, 2020/07/24 end


//add by xiepinze, add page zoom submenu, 2020/08/03, start
void SetPageZoomLevel(const Browser* browser, const double zoom_level) {
  auto webcontent = browser->tab_strip_model()->GetActiveWebContents();
  content::HostZoomMap *zoomMap = content::HostZoomMap::GetForWebContents(webcontent);
  zoomMap->SetZoomLevel(webcontent, zoom_level);
}
...
void BookmarkCurrentTab(Browser* browser) {
  ...
  // add by hanll, 当点击地址栏星号时URL已经是书签，弹出修改书签对话框, 2020/08/17, start
  if(!was_bookmarked_by_user){
    ShowBookmarkCurrentDlg(browser);
    return ;
  }
  // add by hanll, 当点击地址栏星号时URL已经是书签，弹出修改书签对话框, 2020/08/17, end
...
// add by hanll, 增加修改书签对话框显示, 2020/08/17, start
void ShowBookmarkCurrentDlg(Browser* browser) {
  LOG(INFO)<<"ShowBookmarkCurrentDlg0";
  base::RecordAction(UserMetricsAction("StarDlg"));
  size_t index = 0;
  BookmarkModel* model =
      BookmarkModelFactory::GetForBrowserContext(browser->profile());
  if (!model || !model->loaded())
  ...
  gfx::NativeWindow web_window = web_contents->GetTopLevelNativeWindow();
  const bookmarks::BookmarkNode* selectnode = bookmarks::GetParentForNewNodes(model);
  const bookmarks::BookmarkNode* parent = selectnode;
  ...
  BookmarkEditor::Show(web_window,
                      browser->profile(),
                      BookmarkEditor::EditDetails::AddNodeInFolder(
                      parent, index, url, title),
                      BookmarkEditor::SHOW_TREE);

  LOG(INFO)<<"ShowBookmarkCurrentDlg10";
}
// add by hanll, 增加修改书签对话框显示, 2020/08/17, end
