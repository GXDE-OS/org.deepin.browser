...
bool JavaScriptTabModalDialogViewViews::ShouldShowWindowIcon() const {
  return true;
}

bool JavaScriptTabModalDialogViewViews::ShouldShowWindowTitle() const {
  return false;
}

gfx::ImageSkia JavaScriptTabModalDialogViewViews::GetWindowIcon() {
  return *ui::ResourceBundle::GetSharedInstance().GetNativeImageNamed(IDR_JAVASCRIPT_DIALOG_ICON).ToImageSkia();
}
...
JavaScriptTabModalDialogViewViews::JavaScriptTabModalDialogViewViews(
  ...
  // modify by xiaohuyang, Set the single button style of the JS dialog.  2021/2/2
  // For specific modifications, view the modifications of lines 99-102
#if UNUSED
  ...
  // modify by hanll, Set the single button style of the JS dialog.  2021/3/10,begin
  if(is_alert)
    DialogDelegate::SetDefaultButton(ui::DIALOG_BUTTON_NONE);
  else
    DialogDelegate::SetDefaultButton(ui::DIALOG_BUTTON_OK);
  // modify by hanll, Set the single button style of the JS dialog.  2021/3/10,end
