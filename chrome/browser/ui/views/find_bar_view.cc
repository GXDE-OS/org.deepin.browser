...
void SetCommonButtonAttributes(views::ImageButton* button) {
  //modify by xiaohuyang, Set the button style of the find bar view, 2020/10/16  --start
  //Rounded rectangle button
#if 0
  views::ConfigureVectorImageButton(button);
  views::InstallCircleHighlightPathGenerator(button);
#else
  views::ConfigureVectorImageButtonFindbarView(button);
  views::InstallRoundRectHighlightPathGenerator(button,gfx::Insets(),8);
#endif
  //modify by xiaohuyang, Set the button style of the find bar view, 2020/10/16  --end
...
void FindBarView::OnThemeChanged() {
  ...
  views::SetImageFromVectorIcon(close_button_, vector_icons::kCloseRoundedIcon,
                                base_foreground_color);
#else
  views::SetImageFromVectorIcon(find_previous_button_, kFindPreviousIcon,
                                base_foreground_color);
  views::SetImageFromVectorIcon(find_next_button_, kFindNextIcon,
                                base_foreground_color);
  views::SetImageFromVectorIcon(close_button_, vector_icons::kFindbarViewCloseIcon,
                                base_foreground_color);
#endif
  //modify by xiaohuyang, Set the button style of the find bar view, 2020/10/16  --end
}
