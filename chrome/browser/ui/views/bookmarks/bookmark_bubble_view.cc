...
std::unique_ptr<views::View> CreateEditButton(views::ButtonListener* listener) {
  ...
  // modify by xiaohuyang, Set the text color of button under different themes. 2021/1/21  --start
#if 1
  SkColor text_color;
  ...
  edit_button->SetEnabledTextColors(text_color);
#endif
  // modify by xiaohuyang, Set the text color of button under different themes. 2021/1/21  --end
  return edit_button;
}
...
#if defined(USE_UNIONTECH_NPAPI)
  views::View *ancestor = anchor_view->GetAncestorWithClassName("BrowserView");
  DCHECK(ancestor);
  BrowserView* browser_view = static_cast<BrowserView* > (ancestor);

  std::set<views::NpapiPluginMaskObserver*> set;
  if (browser_view) {
    set.insert(browser_view->GetNpapiPluginMaskObserver(bookmark_bubble_->GetWidget()));
    for (auto* observer_ : browser_view->GetNpapiPluginMaskObserverForChildFrame(bookmark_bubble_->GetWidget())) {
      if (observer_) {
        set.insert(observer_);
      }
    }
  }
  views::Widget* bubble_widget = views::BubbleDialogDelegateView::CreateBubble(bookmark_bubble_, set);
#else
...
void BookmarkBubbleView::Init() {
  ...
  AddTitleRow(layout,l10n_util::GetStringUTF16(IDS_BOOKMARK_BUBBLE_PAGE_BOOKMARKED),kColumnId);
  // add by hanll, 增加标题, 2020/08/17, start
...
BookmarkBubbleView::BookmarkBubbleView(
  ...
  //delete by hanll, 去掉书签弹框中同步部分, 2020/07/21 --start
  //DialogDelegate::SetFootnoteView(CreateSigninPromoView());
  //delete by hanll, 去掉书签弹框中同步部分, 2020/07/21 --end
