...
void BookmarkBarView::Layout() {
  ...
  //modify by xiaohuyang, Set the position of the 'emptybookmark_tooltip_label_', 2020/09/24
  emptybookmark_tooltip_label_->SetVisible(CheckBookmarksIsEmpty());
  if(emptybookmark_tooltip_label_->GetVisible()){
    emptybookmark_tooltip_label_->SetBounds(x + 2, y, 300,
                                       button_height);
  }
  //modify by xiaohuyang, Set the position of the 'emptybookmark_tooltip_label_', 2020/09/24
...
bool BookmarkBarView::CanDrop(const ui::OSExchangeData& data) {
  ...
  bool value = (drop_info_->data.Read(data) && drop_info_->data.size() == 1);
  LOG(INFO)<<">>>>>CanDrop3:"<<value;
...
void BookmarkBarView::BookmarkModelLoaded(BookmarkModel* model,
                                          bool ids_reassigned) {
  LOG(INFO)<<"BookmarkBarView::BookmarkModelLoaded";
...
void BookmarkBarView::BookmarkModelBeingDeleted(BookmarkModel* model) {
  LOG(INFO)<<"BookmarkBarView::BookmarkModelBeingDeleted";
  NOTREACHED();
...
void BookmarkBarView::BookmarkNodeMoved(BookmarkModel* model,
                                        ...
                                        size_t new_index) {
  LOG(INFO)<<"BookmarkBarView::BookmarkNodeMoved";
  bool was_throbbing =
      throbbing_view_ &&
...
void BookmarkBarView::BookmarkNodeAdded(BookmarkModel* model,
                                        ...
                                        size_t index) {
  LOG(INFO)<<"BookmarkBarView::BookmarkNodeAdded";
...
void BookmarkBarView::BookmarkNodeRemoved(BookmarkModel* model,
        ...
        LOG(INFO)<<"BookmarkBarView::BookmarkNodeRemoved";
...
void BookmarkBarView::BookmarkAllUserNodesRemoved(
    BookmarkModel* model,
    const std::set<GURL>& removed_urls) {
      LOG(INFO)<<"BookmarkBarView::BookmarkAllUserNodesRemoved";
  UpdateOtherAndManagedButtonsVisibility();
...
void BookmarkBarView::BookmarkNodeFaviconChanged(BookmarkModel* model,
                                                 const BookmarkNode* node) {
  LOG(INFO)<<"BookmarkBarView::BookmarkNodeFaviconChanged";
  BookmarkNodeChangedImpl(model, node);
}
...
void BookmarkBarView::Init() {
  ...
  // add by hanll, 增加创建空提示控件, 2020/08/18, start
  emptybookmark_tooltip_label_ = AddChildView(CreateEmptyBookmarkLabel());
  // add by hanll, 增加创建空提示控件, 2020/08/18, end
...
// add by hanll, 增加显示您的收藏夹是空的提示, 2020/08/18, start
//modify by xiaohuyang, Set the text color and position of the label, 2020/09/23
std::unique_ptr<views::Label> 
BookmarkBarView::CreateEmptyBookmarkLabel(){
  auto emptybookmark_tooltip_label = std::make_unique<views::Label>();
  //base::string16 t = base::ASCIIToUTF16("your bookmarks is empty");
  emptybookmark_tooltip_label->SetText(l10n_util::GetStringUTF16(IDS_EMPTY_BOOKMARK));
  emptybookmark_tooltip_label->SetVisible(true);
  emptybookmark_tooltip_label->SetEnabledColor(SkColorSetRGB(0x70, 0x70, 0x70));
  // int preferred_height = GetLayoutConstant(BOOKMARK_BAR_HEIGHT);
  emptybookmark_tooltip_label->SetHorizontalAlignment(gfx::ALIGN_LEFT);
  emptybookmark_tooltip_label->SetLineHeight(GetLayoutConstant(BOOKMARK_BAR_HEIGHT));
  // emptybookmark_tooltip_label->SetBounds(-45,5,200,20);

  return emptybookmark_tooltip_label;
}
// add by hanll, 增加显示您的收藏夹是空的提示, 2020/08/18, end
...
void BookmarkBarView::ConfigureButton(const BookmarkNode* node,
  ...
  if (node->is_url()) {
    ...
    // modify by fangzhouyang, Use theme color when favicon is null, 2021/4/1 --start
    SkColor iconColorMask = SkColorSetRGB(0x00, 0x81, 0xFF);
    if (favicon.isNull()) {
      if (ui::TouchUiController::Get()->touch_ui() && GetThemeProvider()) {
        ...
        iconColorMask = tp->GetColor(ThemeProperties::COLOR_TOOLBAR_BUTTON_ICON);
        // modify by fangzhouyang, Use theme color when favicon is null, 2021/4/1 --start
    ...
    if (themify_icon && tp &&
      ...
      favicon = gfx::ImageSkiaOperations::CreateColorMask(
          // modify by fangzhouyang, Use same color in both dark and light theme, 2021/3/18 --start
          #if 0
          favicon, tp->GetColor(ThemeProperties::COLOR_TOOLBAR_BUTTON_ICON));
          #endif
          favicon, iconColorMask);
          // modify by fangzhouyang, Use same color in both dark and light theme, 2021/3/18 --end
    }
...
// add by hanll, 增加显示您的收藏夹是空的提示, 2020/08/18, start
bool BookmarkBarView::CheckBookmarksIsEmpty(){

  if (model_->loaded()&&(model_->HasNoUserCreatedBookmarksOrFolders()))
    return true;//没有用户创建的书签
