...
ExtensionInstalledBubbleView::ExtensionInstalledBubbleView(
  ...
  // modify by xiaohuyang, Set the new style of ExtensionInstalledBubbleView，Add OK button， 2020/10/27 --start
#if 0
  ...
  DialogDelegate::SetButtonLabel(
      ui::DIALOG_BUTTON_OK, l10n_util::GetStringUTF16(IDS_OK));
#endif
  // modify by xiaohuyang, Set the new style of ExtensionInstalledBubbleView，Add OK button， 2020/10/27 --end
...
// modify by xiaohuyang, Set the new style of ExtensionInstalledBubbleView, do not display title at the top position.
...
// modify by xiaohuyang, Set the new style of ExtensionInstalledBubbleView, set new window icon.
gfx::ImageSkia ExtensionInstalledBubbleView::GetWindowIcon() {
#if 0
  return icon_;
#else
  return *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_EXTENSION_INSTALL_DIALOG_VIEW_ICON));
#endif
}
...
void ExtensionInstalledBubbleView::Init() {
  ...
  // modify by xiaohuyang, Set the new style of ExtensionInstalledBubbleView， 2020/10/27 --start
#if 0
  ...
  constexpr int kContentChildSpacing = 5;
  constexpr int kContentMarginTop = 0;
  constexpr int kContentMargins = 16;
  SetLayoutManager(std::make_unique<views::BoxLayout>(
      views::BoxLayout::Orientation::kVertical, gfx::Insets(),
      kContentChildSpacing));
  set_margins(gfx::Insets(kContentMarginTop, kContentMargins, kContentMargins, kContentMargins));
#endif
  
  base::string16 extension_name = base::UTF8ToUTF16(model_->extension_name());
  base::i18n::AdjustStringForLocaleDirection(&extension_name);
  base::string16 window_title = l10n_util::GetStringFUTF16(IDS_EXTENSION_INSTALLED_HEADING,
                                    extension_name);
  auto dialog_title_label =
        new views::Label(window_title, CONTEXT_BODY_TEXT_LARGE);
  dialog_title_label->SetMultiLine(true);
  // modify by xiaohuyang, Set size for ‘SetMultiLine’.
  dialog_title_label->SizeToFit(kRightColumnWidth);
  ...
  AddChildView(dialog_title_label);
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, 2020/10/27 --end
