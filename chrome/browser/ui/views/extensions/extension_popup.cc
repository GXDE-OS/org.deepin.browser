...
void ExtensionPopup::ShowPopup(
...
#ifdef USE_UNIONTECH_NPAPI
  BrowserView* _browser_view = BrowserView::GetBrowserViewForBrowser(host->browser());
#endif
  ...
  set.insert(_browser_view->GetNpapiPluginMaskObserver(popup->GetWidget()));
  for (auto* observer_ : _browser_view->GetNpapiPluginMaskObserverForChildFrame(popup->GetWidget())) {
    if (observer_) {
      set.insert(observer_);
    }
  }
  views::BubbleDialogDelegateView::CreateBubble(popup, set);
#else
