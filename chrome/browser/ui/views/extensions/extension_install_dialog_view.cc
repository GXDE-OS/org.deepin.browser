...
ExtensionInstallDialogView::ExtensionInstallDialogView(
  ...
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, set default button.
  // int default_button = ui::DIALOG_BUTTON_CANCEL;
  int default_button = ui::DIALOG_BUTTON_OK;
...
void ExtensionInstallDialogView::AddedToWidget() {
  ...
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, set new window icon， 2020/10/26 --start
#if 0
  ...
  icon->SetImage(*image);
#else
  const gfx::ImageSkia image = *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_EXTENSION_INSTALL_DIALOG_VIEW_ICON));
  auto icon = std::make_unique<views::ImageView>();
  icon->SetImage(image);
#endif
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, set new window icon， 2020/10/26 --end
  ...
  if (prompt_->has_webstore_data()) {
    ...
    // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, do not display title at the top position.
...
//wangfeng, del, (bug: 53479), 2020/12/28 --start
//设置ok为默认按钮时，焦点设置失败问题
#if UNUSE
  if (button == ui::DIALOG_BUTTON_OK)
    return install_button_enabled_;
#endif
//wangfeng, del, (bug: 53479), 2020/12/28 --end
  return true;
}
...
void ExtensionInstallDialogView::CreateContents() {
  ...
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, title centered display， 2020/10/26 --start
  //title 
  auto dialog_title_label =
        new views::Label(title_, CONTEXT_BODY_TEXT_LARGE);
  dialog_title_label->SetMultiLine(true);
  dialog_title_label->SetHorizontalAlignment(gfx::ALIGN_CENTER);
  dialog_title_label->SizeToFit(content_width);
  dialog_title_label->SetFontList(gfx::FontList().DeriveWithWeight(gfx::Font::Weight::BOLD));
  extension_info_container->AddChildView(dialog_title_label);
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, title centered display， 2020/10/26 --end
  ...
  //modify by xiaohuyang, Not return here, because there must be a label of title be created and add to content.
#if 0
  if (sections.empty()) {
  ...
  constexpr int kContentMargins = 0;
  constexpr int kContentMarginBottom = 16;
  // set_margins(gfx::Insets(content_insets.top(), 0, content_insets.bottom(), 0));
  set_margins(gfx::Insets(kContentMargins, kContentMargins, kContentMarginBottom, kContentMargins));
  ...
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, do not set scroll area， 2020/10/26 --start
  //content
#if 0
...
#else 
  AddChildView(std::move(extension_info_container));
#endif
  // modify by xiaohuyang, Set the new style of ‘ExtensionInstallDialogView’, do not set scroll area， 2020/10/26 --end
}
