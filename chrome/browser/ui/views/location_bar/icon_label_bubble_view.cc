...
SkPath IconLabelBubbleView::GetHighlightPath() const {
  ...
  // modify by xiaohuyang, Set a round rect background for locationbar button when it state is focused, 2020/08/07 --start
  #if 0
  const float corner_radius = highlight_bounds.height() / 2.f;
  #else
  const float corner_radius = 8;
  #endif
  // modify by xiaohuyang, Set a round rect background for locationbar button when it state is focused, 2020/08/07 --end
