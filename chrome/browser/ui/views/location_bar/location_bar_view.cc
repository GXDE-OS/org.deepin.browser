...
LocationBarView::LocationBarView(Browser* browser,
  ...
  if (!is_popup_mode_) {
    ...
    // modify by xiaohuyang, Set the border color of locationbar when it is in focus, 2020/08/26 --start
    if(!browser_->profile()->IsIncognitoProfile()){
      focus_ring_->SetLocationBarBorderColor(gfx::kGoogleBlue500);
    } else{
      focus_ring_->SetLocationBarBorderColor(gfx::kGoogleGrey900);
    }
    // modify by xiaohuyang, Set the border color of locationbar when it is in focus, 2020/08/26 --end

    // modify by xiaohuyang, Draw a rounded rectangle background when the button state is hovered or pressed, 2020/08/07 --start
#if 0
    ...
    focus_ring_->SetPathGenerator(
      std::make_unique<views::RoundRectHighlightPathGenerator>(
          gfx::Insets(), 8));
#endif
    // modify by xiaohuyang, Draw a rounded rectangle background when the button state is hovered or pressed, 2020/08/07 --end
  }
}
...
std::unique_ptr<views::Background> LocationBarView::CreateRoundRectBackground(
  ...
  // modify by xiaohuyang, Draw a rounded rectangle background, 2020/08/07 --start
  ...
  // modify by xiaohuyang, Draw a rounded rectangle background, 2020/08/07 --end
...
void LocationBarView::Layout() {
  ...
  // Set the cursor in the address bar to center.
#if UNUSED
  ...
  gfx::Rect location_bounds(0, vertical_padding,
                            ...
                            location_height);
#else
  gfx::Rect location_bounds(0, vertical_padding + 3,
                            std::max(width(), width() - entry_width),
                            location_height);
#endif
...
gfx::Rect LocationBarView::GetLocalBoundsWithoutEndcaps() const {
  ...
  bounds_without_endcaps.set_height(32);
