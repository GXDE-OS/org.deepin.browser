...
void ParentToBrowser(Browser* browser,
  ...
  views::Widget* zoom_bubble_widget = views::BubbleDialogDelegateView::CreateBubble(zoom_bubble);
  ...
  zoom_bubble_widget->GetNpapiPuginMask()->AddObserver(
    _browser_view->GetNpapiPluginMaskObserver(zoom_bubble_widget));

  for (auto* observer_ : _browser_view->GetNpapiPluginMaskObserverForChildFrame(zoom_bubble_widget)) {
    if (observer_) {
      zoom_bubble_widget->GetNpapiPuginMask()->AddObserver(observer_);
    }
  }
#endif
}
