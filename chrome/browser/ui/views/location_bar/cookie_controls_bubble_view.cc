...
void CookieControlsBubbleView::ShowBubble(views::View* anchor_view,
  ...
  if (_browser_view) {
    bubble_widget->GetNpapiPuginMask()->AddObserver(
      _browser_view->GetNpapiPluginMaskObserver(bubble_widget));

    for (auto* observer_ : _browser_view->GetNpapiPluginMaskObserverForChildFrame(bubble_widget)) {
      if (observer_) {
        bubble_widget->GetNpapiPuginMask()->AddObserver(observer_);
      }
    }
  }
#endif
