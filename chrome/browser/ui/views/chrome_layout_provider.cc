...
constexpr int kMediumSnapPoint = 348;
...
int ChromeLayoutProvider::GetSnappedDialogWidth(int min_width) const {
  //modify by xiaohuyang, Set a fixed size dialog,  2020/09/27 --start
#if 0
  ...
  return ((min_width + kHarmonyLayoutUnit - 1) / kHarmonyLayoutUnit) *
         kHarmonyLayoutUnit;
#else
  for (int snap_point : {385, 448, 522}) {
    if (min_width <= snap_point)
      return snap_point;
  }
  ...
  return ((min_width + kHarmonyLayoutUnit - 1) / kHarmonyLayoutUnit) *
         kHarmonyLayoutUnit;
#endif
  //modify by xiaohuyang, Set a fixed size dialog,  2020/09/27 --end
}
