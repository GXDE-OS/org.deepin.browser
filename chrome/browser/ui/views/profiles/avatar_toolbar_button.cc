...
void AvatarToolbarButton::UpdateText() {
  ...
  switch (delegate_->GetState()) {
    case State::kIncognitoProfile: {
      //wangjuna: 不显示隐私窗口计数 2020/08/03 --start
#if 0
...
#endif
      //wangjuna: 不显示隐私窗口计数 2020/08/03 --end
      break;
    }
...
void AvatarToolbarButton::OnMouseExited(const ui::MouseEvent& event) {
  ...
  ToolbarRightButton::OnMouseExited(event);
}

void AvatarToolbarButton::OnBlur() {
  delegate_->OnBlur();
  ToolbarRightButton::OnBlur();
}

void AvatarToolbarButton::OnThemeChanged() {
  ToolbarRightButton::OnThemeChanged();
  UpdateIcon();
  UpdateText();
}
...
gfx::ImageSkia AvatarToolbarButton::GetAvatarIcon(
  ...
  switch (delegate_->GetState()) {
    case State::kIncognitoProfile:
    // modify by xiaohuyang, Set incognito mode window icon, 2020/08/27 --start
    #if 0
      return gfx::CreateVectorIcon(kIncognitoIcon, icon_size, icon_color);
    #else
      return *ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_PRIVACY_MODE_NORMAL_ICON);
    #endif
    // modify by xiaohuyang, Set incognito mode window icon, 2020/08/27 --end
    case State::kGuestSession:
