...
void ProfileMenuViewBase::ShowBubble(
  ...
  set.insert(browser_view->GetNpapiPluginMaskObserver(bubble->GetWidget()));
  for (auto* observer_ : browser_view->GetNpapiPluginMaskObserverForChildFrame(bubble->GetWidget())) {
    if (observer_) {
      set.insert(observer_);
    }
  }
  views::BubbleDialogDelegateView::CreateBubble(bubble, set)->Show();
#else
...
// wangjuna: disable scroll view in avatar button menu, 2020/08/14 --start
#if 0
...
#endif
// wangjuna: disable scroll view in avatar button menu, 2020/08/14 --end
...
#else
  layout->AddView(std::move(components));
#endif
// wangjuna: disable scroll view in avatar button menu, 2020/08/14 --end
}
