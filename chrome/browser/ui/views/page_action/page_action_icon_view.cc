...
void PageActionIconView::NotifyClick(const ui::Event& event) {
  ...
  // add by hanll, 添加点击图标后，变为正常状态, 2020/08/25, start
  SetHighlighted(false);
  // add by hanll, 添加点击图标后，变为正常状态, 2020/08/25, end
}
...
void PageActionIconView::UpdateIconImage() {
  ...
  //modify by xiaohuyang, Set the bookmark status icon to yellow, 2020/09/01 --start
#if 0
...
#else
  const SkColor icon_color =
      active_ ? gfx::kGoogleYellow600 : icon_color_;
#endif
  //modify by xiaohuyang, Set the bookmark status icon to yellow, 2020/09/01 --end
