...
std::unique_ptr<views::Widget> CreatePopupWidget(
  ...
  if(browser_view->GetNpapiPluginMaskObserver(popup.get())) {
    params.npapi_plugin_mask_observers.push_back(
      browser_view->GetNpapiPluginMaskObserver(popup.get()));
  }

  for (auto* observer_ : browser_view->GetNpapiPluginMaskObserverForChildFrame(popup.get())) {
    if (observer_) {
