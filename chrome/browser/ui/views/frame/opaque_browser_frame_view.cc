...
void OpaqueBrowserFrameView::InitViews() {
  ...
  logo_button_ =
        CreateImageButton(IDR_LOGO, IDR_LOGO_H, IDR_LOGO_P,
                          IDR_LOGO_BUTTON_MASK, VIEW_ID_LOGO_BUTTON);
  logo_button_->SetState(views::Button::ButtonState::STATE_DISABLED);
  InitWindowCaptionButton(logo_button_, IDS_ACCNAME_LOGO,
                          VIEW_ID_LOGO_BUTTON);
...
int OpaqueBrowserFrameView::NonClientHitTest(const gfx::Point& point) {
  ...
  if (close_button_ && close_button_->GetVisible() &&
    ...
    return HTCLOSE;
  if (logo_button_ && logo_button_->GetVisible() &&
      logo_button_->GetMirroredBounds().Contains(point))
    return HTNOWHERE;
...
gfx::Size OpaqueBrowserFrameView::GetBrowserViewMinimumSize() const {
  // modify by hanll, 修改浏览器最小尺寸大小, 2020/10/12, start
  return gfx::Size(550,400);
  // modify by hanll, 修改浏览器最小尺寸大小, 2020/10/12, end
