...
BrowserView::BrowserView(std::unique_ptr<Browser> browser)
  ...
  //wangjuna:fix bug#40671,disable status bubble, 2020/08/01 --start
  //InitStatusBubble();
  //wangjuna:fix bug#40671,disable status bubble, 2020/08/01 --end
...
views::NpapiPluginMaskObserver* BrowserView::GetNpapiPluginMaskObserver(views::Widget *widget) {
  auto* web_contents = GetActiveWebContents();
  if(web_contents && web_contents->GetRenderWidgetHostView())
      return web_contents->GetRenderWidgetHostView()->GetNpapiPluginMaskObserver(widget);

  return nullptr;
}

std::set<views::NpapiPluginMaskObserver*> BrowserView::GetNpapiPluginMaskObserverForChildFrame(views::Widget *widget) {
  std::set<views::NpapiPluginMaskObserver*> set;

  auto* web_contents = GetActiveWebContents();
  if (web_contents) {
    for (auto* view : web_contents->GetRenderWidgetChildFrameView()) {
      if (view) {
        set.insert(view->GetNpapiPluginMaskObserver(widget));
      }
    }
  }
