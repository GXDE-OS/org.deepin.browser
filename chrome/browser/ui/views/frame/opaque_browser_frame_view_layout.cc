...
const int OpaqueBrowserFrameViewLayout::kCaptionLogoTopSpacingY = 7;
const int OpaqueBrowserFrameViewLayout::kCaptionLogoY = 12;
const int OpaqueBrowserFrameViewLayout::kCaptionLogoPaddingSpacingX = 2;
...
OpaqueBrowserFrameViewLayout::OpaqueBrowserFrameViewLayout()
    : available_space_leading_x_(0),
      ...
      leading_buttons_{views::FrameButton::kLogo},
...
int OpaqueBrowserFrameViewLayout::CaptionButtonY(views::FrameButton button_id,
                                                 bool restored) const {
  if (button_id != views::FrameButton::kLogo) {
    return DefaultCaptionButtonY(restored);
  } else {
    return kCaptionLogoY;
  }
}
...
void OpaqueBrowserFrameViewLayout::LayoutWindowControls() {
  ...
  buttons_not_shown.push_back(views::FrameButton::kLogo);
...
#else
  int size = delegate_->GetIconSize() - 2;
#endif
  // modify by fangzhouyang, modify web app style, 2021/4/2 --end
...
void OpaqueBrowserFrameViewLayout::ConfigureButton(views::FrameButton button_id,
  ...
  switch (button_id) {
    ...
    case views::FrameButton::kLogo: {
      //modify by xiepinze, don't show browser logo if it's app browser, 20201028, start
      Browser* browser = nullptr;
      if (web_app_frame_toolbar_ && web_app_frame_toolbar_->GetBrowserView())
        browser = web_app_frame_toolbar_->GetBrowserView()->browser();

      if (browser && browser->is_type_app())
        SetViewVisibility(logo_button_, false);
      else
        SetViewVisibility(logo_button_, true);

      SetBoundsForButton(button_id, logo_button_, alignment);
      break;
      //modify by xiepinze, don't show browser logo if it's app browser, 20201028, end
    }
  }
}
...
void OpaqueBrowserFrameViewLayout::SetBoundsForButton(
  ...
  if (delegate_->GetFrameButtonStyle() ==
      OpaqueBrowserFrameViewLayoutDelegate::FrameButtonStyle::kMdButton && button_id != views::FrameButton::kLogo) {
    DCHECK_EQ(std::string(views::FrameCaptionButton::kViewClassName),
              button->GetClassName());
    //modify by xiaohuyang, Set the size of 'FrameCaptionButton', 2020/09/19  --start
#if 0
...
#else
    const int height = delegate_->GetTopAreaHeight();
    button_size = gfx::Size(height, height);
    button->SetPreferredSize(button_size);
#endif
    //modify by xiaohuyang, Set the size of 'FrameCaptionButton', 2020/09/19  --end
  } else if (delegate_->GetFrameButtonStyle() ==
             OpaqueBrowserFrameViewLayoutDelegate::FrameButtonStyle::
                 kImageButton || button_id == views::FrameButton::kLogo) {
    ...
    case ALIGN_LEADING: {
      ...
      if (button_id != views::FrameButton::kLogo) {
        ...
        button->SetBounds(
            available_space_leading_x_ - (start_spacing_clickable
                                              ? button_start_spacing + extra_width
                                              : 0) + kCaptionLogoPaddingSpacingX,
            top_spacing_clickable ? kCaptionLogoTopSpacingY : caption_y,
            button_size.width() + (start_spacing_clickable
                                       ? button_start_spacing + extra_width
                                       : 0),
            button_size.height());
...
void OpaqueBrowserFrameViewLayout::SetView(int id, views::View* view) {
  ...
  switch (id) {
    ...
    case VIEW_ID_CLOSE_BUTTON:
      ...
      break;
    case VIEW_ID_LOGO_BUTTON:
      logo_button_ = static_cast<views::Button*>(view);
      break;
