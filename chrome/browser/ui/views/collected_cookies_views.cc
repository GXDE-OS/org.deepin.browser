...
// modify by xiaohuyang, Only used to set the text color when theme changed. 2021/2/19  --start
void CollectedCookiesViews::OnThemeChanged() {
  ...
  if(GetNativeTheme() && GetNativeTheme()->ShouldUseDarkColors()) {
    ...
    text_color = SkColorSetRGB(65, 77, 104);
  }
  block_allowed_button_->SetEnabledTextColors(text_color);
  allow_blocked_button_->SetEnabledTextColors(text_color);
  for_session_blocked_button_->SetEnabledTextColors(text_color);
}
// modify by xiaohuyang, Only used to set the text color when theme changed. 2021/2/19  --end
...
std::unique_ptr<views::View> CollectedCookiesViews::CreateButtonsPane() {
...

  {
    ...
    // modify by xiaohuyang, Set the text color and preferred size for the button. 2021/2/19
#if 1
    block_allowed_button->SetPreferredSize(gfx::Size(80, 36));
    delete_allowed_button->SetPreferredSize(gfx::Size(80, 36));
    delete_allowed_button->SetEnabledTextColors(SkColorSetRGB(255, 87, 54));
#endif
...

  {
    ...
    // modify by xiaohuyang, Set the preferred size for the button. 2021/2/19
#if 1 
    allow_blocked_button->SetPreferredSize(gfx::Size(80, 36));
    for_session_blocked_button->SetPreferredSize(gfx::Size(120, 36));
#endif
