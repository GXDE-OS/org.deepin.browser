...
PageInfoHoverButton::PageInfoHoverButton(views::ButtonListener* listener,
  ...
  // modify by xiaohuyang, Set the background and border of the page info hover button under different themes. 2021/1/20  --start
#if UNUSED
  SetBorder(views::CreateEmptyBorder(vert_spacing, horz_spacing, vert_spacing,
                                     horz_spacing));
#else
  SetBorder(views::CreateEmptyBorder(vert_spacing-5, horz_spacing, vert_spacing-5,
                                     horz_spacing));

  if (GetNativeTheme()->ShouldUseDarkColors()) {
    SetBackground(views::CreateRoundedRectBackground(
      SkColorSetRGB(0x44, 0x44, 0x44), 8));
  } else {
    SetBackground(views::CreateRoundedRectBackground(
      SkColorSetRGB(0xF0, 0xF0, 0xF0), 8));
  }
#endif
  // modify by xiaohuyang, Set the background color of the page info hover button under different themes. 2021/1/20  --end
...
int PageInfoHoverButton::GetHeightForWidth(int w) const {
  //modify by xiaohuyang, Set the height of page info hover button, 2020/10/22  --start
#if 0
  return Button::GetHeightForWidth(w);
#else
  return 36;
#endif
  //modify by xiaohuyang, Set the height of page info hover button, 2020/10/22  --end
}
