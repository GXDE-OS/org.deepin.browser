...
//modify by xiaohuyang, Set the width of the page info dialog,  2020/09/27 --start
// constexpr int kMinBubbleWidth = 320;
constexpr int kMinBubbleWidth = 348;
//modify by xiaohuyang, Set the width of the page info dialog,  2020/09/27 --end
[[maybe_unused]] constexpr int kMaxBubbleWidth = 1000;
...
std::unique_ptr<views::View> CreateSiteSettingsLink(
  ...
  //modify by xiaohuyang, Set the icon of setting button, 2020/10/22  --start
#if 0
...
#else    
  return std::make_unique<PageInfoHoverButton>(
      listener, *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(resId)),
      IDS_PAGE_INFO_SITE_SETTINGS_LINK, base::string16(),
      PageInfoBubbleView::VIEW_ID_PAGE_INFO_LINK_OR_BUTTON_SITE_SETTINGS,
      tooltip, base::string16());
#endif
  //modify by xiaohuyang, Set the icon of setting button, 2020/10/22  --end
}
...
void BubbleHeaderView::SetDetails(const base::string16& details_text) {
  ...
  //modify by xiaohuyang, Remove the head logo pop-up window "Learn more", 2020/07/27 --start
  // subst.push_back(l10n_util::GetStringUTF16(IDS_LEARN_MORE));
  //modify by xiaohuyang, Remove the head logo pop-up window "Learn more", 2020/07/27 --end
  ...
  //modify by xiaohuyang, Website security detail display is set to gray, 2020/07/29 --start
  security_details_label_->SetDefaultTextStyle(4102);
  //modify by xiaohuyang, Website security detail display is set to gray, 2020/07/29 --end
...
#ifdef USE_UNIONTECH_NPAPI
  Browser* _browser = chrome::FindBrowserWithWebContents(web_contents);
  BrowserView* _browser_view = BrowserView::GetBrowserViewForBrowser(_browser);

  std::set<views::NpapiPluginMaskObserver*> set;
  set.insert(_browser_view->GetNpapiPluginMaskObserver(GetWidget()));
  for (auto* observer_ : _browser_view->GetNpapiPluginMaskObserverForChildFrame(GetWidget())) {
    if (observer_) {
      set.insert(observer_);
    }
  }
  views::BubbleDialogDelegateView::CreateBubble(this, set);
#else
...
PageInfoBubbleView::PageInfoBubbleView(
  ...
  //modify by xiaohuyang, Set window title margins.
  set_title_margins(gfx::Insets(18, 20, 0, 10));
  ...
  //modify by xiaohuyang, Adjust window style, margins, fonts, separators, etc, 2020/07/28  --start
  //Set content margins.
#if 0
  const int bottom_margin = hover_list_spacing;
  set_margins(gfx::Insets(margins().top(), 0, bottom_margin, 0));
#else
  set_margins(gfx::Insets(0, 20, 10, 20));
#endif
  ...
  header_ = layout->AddView(
      std::make_unique<BubbleHeaderView>(this, this, 0));
  ...
  if (!profile->IsGuestSession()) {
    int resId = IDR_SETTING_NORMAL_ICON;
    if(View::GetNativeTheme() && GetNativeTheme()->ShouldUseDarkColors()){
      resId = IDR_SETTING_DARK_ICON;
    }
    else{
      resId = IDR_SETTING_NORMAL_ICON;
    }

    layout->StartRowWithPadding(views::GridLayout::kFixedSize, kColumnId,
                                views::GridLayout::kFixedSize, 10);
    layout->AddView(CreateSiteSettingsLink(side_margin, this, resId));
  }
  //modify by xiaohuyang, Adjust window style, margins, fonts, separators, etc, 2020/07/28  --end
...
gfx::Size PageInfoBubbleView::CalculatePreferredSize() const {
  ...
  //modify by xiaohuyang, Adjust size of bubble view , 2020/08/12  --start
  #if 0
  ...
  #endif
  //modify by xiaohuyang, Adjust size of bubble view , 2020/08/12  --end
  ...
  if (cookie_button_ == nullptr) {
    ...
    info.is_incognito =
        Profile::FromBrowserContext(web_contents()->GetBrowserContext())
            ->IsOffTheRecord();
    //modify by xiaohuyang, Set the icon of cookie button, 2020/10/22  --start
#if 0
    const gfx::ImageSkia icon =
        PageInfoUI::GetPermissionIcon(info, GetRelatedTextColor());
#else
    gfx::ImageSkia icon;
    if(GetNativeTheme() && GetNativeTheme()->ShouldUseDarkColors())
    {
      icon = *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_COOKIE_DARK_ICON));
    }
    else
    {
      icon = *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_COOKIE_NORMAL_ICON));
    }
#endif
    //modify by xiaohuyang, Set the icon of cookie button, 2020/10/22  --end
...
void PageInfoBubbleView::SetPermissionInfo(
  ...
  //modify by xiaohuyang, The Permission view is not displayed and it is not necessary to read the data, 2020/08/12  --start
  #if 1
  return ;
  ...
  //modify by xiaohuyang, Remove the space between security title and detail, 2020/07/29 --start
  // layout->AddPaddingRow(views::GridLayout::kFixedSize, list_item_padding);
  //modify by xiaohuyang, Remove the space between security title and detail, 2020/07/29 --end

  layout->Layout(permissions_view_);
  SizeToContents();
  #endif
  //modify by xiaohuyang, The Permission view is not displayed and it is not necessary to read the data, 2020/08/12  --end
}
...
void PageInfoBubbleView::SetIdentityInfo(const IdentityInfo& identity_info) {
  ...
  //modify by xiaohuyang, Website security summary display is set to bold and black, 2020/11/27 --start
#if 0
  ...
  static_cast<views::Label*>(GetBubbleFrameView()->title())
    ->SetEnabledColor(views::style::GetColor(
        *this, views::style::CONTEXT_DIALOG_TITLE, views::style::STYLE_HIGHLIGHTED));
  static_cast<views::Label*>(GetBubbleFrameView()->title())->SetFontList(gfx::FontList().DeriveWithWeight(gfx::Font::Weight::BOLD));
#endif
  //modify by xiaohuyang, Website security summary display is set to bold and black, 2020/11/27 --end
  ...
  if (identity_info.certificate) {
    ...
    //modify by xiaohuyang, Set the icon of certificate button, 2020/10/22  --start
#if 0
    const gfx::ImageSkia icon =
    ...
    if(GetNativeTheme() && GetNativeTheme()->ShouldUseDarkColors())
    {
      icon = *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_CERTIFICATE_DARK_ICON));
    }
    else{
      icon = *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_CERTIFICATE_NORMAL_ICON));
    }
#endif
    //modify by xiaohuyang, Set the icon of certificate button, 2020/10/22  --end
...
std::unique_ptr<views::View> PageInfoBubbleView::CreateSiteSettingsView() {
  ...
  auto* box_layout =
      site_settings_view->SetLayoutManager(std::make_unique<views::BoxLayout>(
          views::BoxLayout::Orientation::kVertical));
  box_layout->set_between_child_spacing(10);
