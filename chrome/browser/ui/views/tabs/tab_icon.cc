...
void TabIcon::PaintLoadingAnimation(gfx::Canvas* canvas, gfx::Rect bounds) {
  ...
  if (network_state_ == TabNetworkState::kWaiting) {
    gfx::PaintThrobberWaiting(
        ...
        #endif
  // modify by fangzhouyang, Use same color in both dark and light theme, 2021/3/17 --start
        SkColorSetRGB(0x00, 0x81, 0xFF),
    ...
    gfx::PaintThrobberSpinningAfterWaiting(
        canvas, bounds,
        SkColorSetRGB(0x00, 0x81, 0xFF),
  // modify by fangzhouyang, Use same color in both dark and light theme, 2021/3/17 --end
...
gfx::ImageSkia TabIcon::ThemeImage(const gfx::ImageSkia& source) {
  if (!GetThemeProvider()->HasCustomColor(
    ...
    return source;
  // modify by xiaohuyang, Use color to set a new style of favicon, 2021/3/6 --start
  // Under the dark theme, darken the color of the favorite icon. 
  // Although i think it is not necessary, the tester raised this question.
#if UNUSED
  ...
  return gfx::ImageSkiaOperations::CreateColorMask(
      ...
      SkColorSetRGB(0x00, 0x81, 0xFF));
#endif
  // modify by xiaohuyang, Use color to set a new style of favicon, 2021/3/6 --end
}
