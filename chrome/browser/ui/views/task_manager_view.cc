...
task_manager::TaskManagerTableModel* TaskManagerView::Show(Browser* browser) {
  ...
  if ((!xdg_session_type || strncmp(xdg_session_type, "wayland", 7) != 0)) {
    ...
    std::string wm_class = shell_integration_linux::GetProgramClassClass();
    std::string wm_name = shell_integration_linux::GetProgramClassName();
    DialogDelegate::CreateDialogWidgetForTaskManagerWayland(g_task_manager_view, context, nullptr, wm_class, wm_name);
  }
  ...
  g_task_manager_view->SelectTaskOfActiveTab(browser);
  // modify by xiaohuyang, The style attribute of the button has been set in MdTextButton. 2021/3/9
  if(g_task_manager_view->GetOkButton()){
    (g_task_manager_view->GetOkButton())->SetPreferredSize(gfx::Size(88,36));
#if UNUSED
    static_cast<views::MdTextButton*>(g_task_manager_view->GetOkButton())->SetProminent(true);
    (g_task_manager_view->GetOkButton())->SetTextColor(views::Button::STATE_NORMAL,
      SkColorSetRGB(0xFF,0xFF,0xFF));
    (g_task_manager_view->GetOkButton())->SetTextColor(views::Button::STATE_HOVERED,
      SkColorSetRGB(0xFF,0xFF,0xFF));
    (g_task_manager_view->GetOkButton())->SetTextColor(views::Button::STATE_PRESSED,
      SkColorSetRGB(0xFF,0xFF,0xFF));
#endif
  }
...
void TaskManagerView::SetSortDescriptor(const TableSortDescriptor& descriptor) {
  LOG(INFO)<<"TaskManagerView::SetSortDescriptor";
...
bool TaskManagerView::IsDialogButtonEnabled(ui::DialogButton button) const {
  LOG(INFO)<<"TaskManagerView::IsDialogButtonEnabled";
...
void TaskManagerView::OnSelectionChanged() {
  LOG(INFO)<<"TaskManagerView::OnSelectionChanged";
  DialogModelChanged();
  // modify by xiaohuyang, The style attribute of the button has been set in MdTextButton. 2021/3/9
  if(g_task_manager_view->GetOkButton()){
    (g_task_manager_view->GetOkButton())->SetPreferredSize(gfx::Size(88,36));
