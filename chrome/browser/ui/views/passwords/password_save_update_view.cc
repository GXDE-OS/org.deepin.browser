...
constexpr int kContentHeight = 36;
constexpr int kViewButtonWidth = 40;
constexpr int kPasswordEditableComboboxWidth = 300;
constexpr int kUsernameEditableComboboxWidth = 354;
...
//modify by xiaohuyang, Set password to save and update the style of pop-up window, 2020/10/30  --start
#if 0
...
#else 
void BuildColumnSetWithTitle(views::GridLayout* layout,
                    PasswordSaveUpdateViewColumnSetType type) {
  views::ColumnSet* column_set = layout->AddColumnSet(type);
  switch (type) {
    case DOUBLE_VIEW_COLUMN_SET_USERNAME:
...
#else
void BuildCredentialRowsWithTitle(
    views::GridLayout* layout,
  ...
  int child_height = std::max(username_field->GetPreferredSize().height(),
                               password_field->GetPreferredSize().height());

  //title
  BuildColumnSetWithTitle(layout, DOUBLE_VIEW_COLUMN_SET_USERNAME);
  layout->StartRow(views::GridLayout::kFixedSize,
                   DOUBLE_VIEW_COLUMN_SET_USERNAME);
  std::unique_ptr<views::Label> title_label(new views::Label(
      window_title,
      views::style::CONTEXT_LABEL, views::style::STYLE_PRIMARY));
  title_label->SetHorizontalAlignment(gfx::HorizontalAlignment::ALIGN_CENTER);

  layout->AddView(std::move(title_label), 1, 1, views::GridLayout::FILL,
                  views::GridLayout::FILL, 0, 20);
  ...
  layout->AddView(std::move(username_field), 1, 1, views::GridLayout::FILL,
                  views::GridLayout::FILL, 0, child_height);
  layout->AddPaddingRow(views::GridLayout::kFixedSize,
                        ChromeLayoutProvider::Get()->GetDistanceMetric(
                            DISTANCE_CONTROL_LIST_VERTICAL));
  ...
  PasswordSaveUpdateViewColumnSetType type =
      password_view_button ? TRIPLE_VIEW_COLUMN_SET
                           : DOUBLE_VIEW_COLUMN_SET_PASSWORD;
  BuildColumnSetWithTitle(layout, type);
  layout->StartRow(views::GridLayout::kFixedSize, type);
  layout->AddView(std::move(password_field), 1, 1, views::GridLayout::FILL,
                  views::GridLayout::FILL, 0, kContentHeight);
  // The eye icon is also added to the layout if it was passed.
  if (password_view_button) {
    // layout->AddView(std::move(password_view_button));
    layout->AddView(std::move(password_view_button));
...
#endif
//modify by xiaohuyang, Set password to save and update the style of pop-up window, 2020/10/30  --end
...
std::unique_ptr<views::ToggleImageButton> CreatePasswordViewButton(
  ...
  button->SetImage(views::ImageButton::STATE_NORMAL,
                   *rb.GetNativeImageNamed(IDR_ENTRY_PASSWORD_ICON).ToImageSkia());
  button->SetToggledImage(
      views::ImageButton::STATE_NORMAL,
      rb.GetNativeImageNamed(IDR_ENTRY_UNPASSWORD_ICON).ToImageSkia());
  button->SetMinimumImageSize(gfx::Size(kViewButtonWidth, kContentHeight));
#endif
...
std::unique_ptr<views::EditableCombobox> CreateUsernameEditableCombobox(
  ...
  //Add by xiaohuyang, Set the size of 'UsernameEditableCombobox' to make it centered.    --2020/10/22
  combobox->SetPreferredSize(gfx::Size(kUsernameEditableComboboxWidth, kContentHeight));
  return combobox;
}
...
std::unique_ptr<views::EditableCombobox> CreatePasswordEditableCombobox(
  ...
  //modify by xiaohuyang, Password save pop-up window password drop-down box data display, 2020/10/13  --start
  //‘Form.password_value’ is always correct
#if 0
  std::vector<base::string16> passwords =
...
#else
  std::vector<base::string16> passwords = std::vector<base::string16>(/*n=*/1, form.password_value);
#endif
  //modify by xiaohuyang, Password save pop-up window password drop-down box data display, 2020/10/13  --end
  ...
  combobox->SetAccessibleName(
      l10n_util::GetStringUTF16(IDS_PASSWORD_MANAGER_PASSWORD_LABEL));
  //Add by xiaohuyang, Set the size of 'PasswordEditableCombobox' to make it and 'PasswordViewButton' centered.    --2020/10/22
  combobox->SetPreferredSize(gfx::Size(kPasswordEditableComboboxWidth, kContentHeight));
  return combobox;
}
...
PasswordSaveUpdateView::PasswordSaveUpdateView(
  ...
  if (password_form.IsFederatedCredential()) {
    ...
    // modify by xiaohuyang, Set new layout of this view, 2020/10/22 --start
#if 0
    BuildCredentialRows(layout, std::move(username_dropdown),
                        std::move(password_dropdown),
                        std::move(password_view_button));
#else
    BuildCredentialRowsWithTitle(layout, PasswordBubbleViewBase::GetWindowTitle(),
                        std::move(username_dropdown),
                        std::move(password_dropdown),
                        std::move(password_view_button));
#endif
    // modify by xiaohuyang, Set new layout of this view, 2020/10/22 --end
  }
...
// modify by xiaohuyang, Used to set window title not to dispaly, 2020/10/22 --start
#if 1
base::string16 PasswordSaveUpdateView::GetWindowTitle() const {
  return base::string16();
}

bool PasswordSaveUpdateView::ShouldShowWindowTitle() const {
  return false;
}
#endif
// modify by xiaohuyang, Used to set window title not to dispaly, 2020/10/22 --end
...
// modify by xiaohuyang, Used to set window icon to show, 2020/10/22 --start
#if 0
...
#endif
// modify by xiaohuyang, Used to set window icon to show, 2020/10/22 --end
...
void PasswordSaveUpdateView::OnThemeChanged() {
  // delete by hanll, 去掉密码框中钥匙的部分, 2020/08/17, start
#if 0
  PasswordBubbleViewBase::OnThemeChanged();
  [[maybe_unused]] int id = color_utils::IsDark(GetBubbleFrameView()->GetBackgroundColor())
               ? IDR_SAVE_PASSWORD_DARK
               : IDR_SAVE_PASSWORD;
  GetBubbleFrameView()->SetHeaderView(CreateHeaderImage(id));
#endif
  // delete by hanll, 去掉密码框中钥匙的部分, 2020/08/17, end
}
