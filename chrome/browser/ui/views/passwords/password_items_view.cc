...
[[maybe_unused]] const gfx::Insets kContentMargins = gfx::Insets(0, 16, 16, 16);
...
enum PasswordItemsViewColumnSetType { PASSWORD_COLUMN_SET, UNDO_COLUMN_SET,  TITLE_COLUMN_SET };
...
void BuildColumnSet(views::GridLayout* layout,
  ...
  // modify by xiaohuyang, Build colum set for title label.
#if 1
  if(TITLE_COLUMN_SET==type_id){
...
// modify by xiaohuyang, Create title label.
std::unique_ptr<views::Label> CreateTitleLabel(base::string16 title) {
  auto label = std::make_unique<views::Label>(title, CONTEXT_BODY_TEXT_LARGE);
  label->SetMultiLine(true);
  label->SetHorizontalAlignment(gfx::ALIGN_CENTER);
  label->SetFontList(gfx::FontList().DeriveWithWeight(gfx::Font::Weight::BOLD));
  return label;
}
...
// modify by xiaohuyang, Set the new style of password item view, 2020/11/19 --start
#if 0
...
#else
void PasswordItemsView::PasswordRow::AddUndoRow(views::GridLayout* layout) {
  std::unique_ptr<views::Label> title_label = CreateTitleLabel(parent_->GetTitle());
  StartRow(layout, TITLE_COLUMN_SET);
...
#endif
// modify by xiaohuyang, Set the new style of password item view, 2020/11/19 --end
...
PasswordItemsView::PasswordItemsView(content::WebContents* web_contents,
  ...
  if (controller_.local_credentials().empty()) {
    // modify by xiaohuyang, Set layout manage for the new style of password item view.
#if 0
    ...
    std::unique_ptr<views::Label> title_label = CreateTitleLabel(GetTitle());
    StartRow(grid_layout, TITLE_COLUMN_SET);
    grid_layout->AddView(std::move(title_label));
#endif
  } else {
  ...
  // modify by xiaohuyang, Set content margins.
...
// modify by xiaohuyang, Display window icon, do not show title and get window title for title label.  2020/11/19 --start
#if 1
gfx::ImageSkia PasswordItemsView::GetWindowIcon() {
  return *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_PERMISSION_PROMPT_BUBBLE_VIEW_ICON));
}
bool PasswordItemsView::ShouldShowWindowIcon() const {
  return true;
}
base::string16 PasswordItemsView::GetWindowTitle() const {
  return base::string16();
}
bool PasswordItemsView::ShouldShowWindowTitle() const {
  return true;
}

base::string16 PasswordItemsView::GetTitle(){
  return PasswordBubbleViewBase::GetWindowTitle();
}
#endif
// modify by xiaohuyang, Display window icon, do not show title and get window title for title label.  2020/11/19 --end
