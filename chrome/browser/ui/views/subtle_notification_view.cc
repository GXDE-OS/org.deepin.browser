...
//modify by xiaohuyang, Set the background color of 'SubtleNotificationView', the text color of notification label and the border color of 'key_view'.
#if 0
...
const SkColor kKeyViewBorderColor =
    SkColorSetRGB(0xC6, 0xC6, 0xC6);
#endif
...
void SubtleNotificationView::InstructionView::AddTextSegment(
  ...
  [[maybe_unused]] constexpr SkColor kForegroundColor = SK_ColorWHITE;

  views::Label* label = new views::Label(text, kInstructionTextContext);
  //modify by xiaohuyang, Set the text color of notification label.
#if 1
  label->SetEnabledColor(kForegroundColor);
#else
  label->SetEnabledColor(kLabelTextColor);
#endif
  ...
  //modify by xiaohuyang, Set the border color of 'key_view'.
#if 0
...
#else
  std::unique_ptr<views::Border> border(views::CreateRoundedRectBorder(
      kKeyNameBorderPx, kKeyNameCornerRadius, kKeyViewBorderColor));
#endif
...
#ifdef USE_UNIONTECH_NPAPI
  BrowserView *browser_view = (BrowserView *)(parent_view->GetNativeWindowProperty("__BROWSER_VIEW__"));
  if(browser_view->GetNpapiPluginMaskObserver(popup)) {
    params.npapi_plugin_mask_observers.push_back(
      browser_view->GetNpapiPluginMaskObserver(popup));
  }

  for (auto* observer_ : browser_view->GetNpapiPluginMaskObserverForChildFrame(popup)) {
    if (observer_) {
      params.npapi_plugin_mask_observers.push_back(observer_);
    }
  }
#endif
