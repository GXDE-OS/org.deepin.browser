...
std::unique_ptr<views::InkDropHighlight> WebAppMenuButton::CreateInkDropHighlight() const {
  auto highlight = std::make_unique<views::InkDropHighlight>(
      size(), 0, gfx::RectF(GetMirroredRect(GetLocalBounds())).CenterPoint(),
      GetInkDropBaseColor());
    highlight->set_visible_opacity(0.175f);
  return highlight;
}

std::unique_ptr<views::InkDropMask> WebAppMenuButton::CreateInkDropMask()
    const {
