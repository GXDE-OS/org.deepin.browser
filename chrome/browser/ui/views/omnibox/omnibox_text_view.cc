...
// Set the text color according to UI requirements.
void OmniboxTextView::ApplyTextColor(const SkColor& value) {
#if 1
  if (!render_text_) {
    return;
  }
...
void OmniboxTextView::ReapplyStyling() {
  ...
  for (size_t i = 0; i < cached_classifications_->size(); ++i) {
    ...
    render_text_->ApplyColor(result_view_->GetColorUOS(part), current_range);
  }
