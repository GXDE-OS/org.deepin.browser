...
void OmniboxViewViews::InstallPlaceholderText() {
  ...
  if (default_provider) {
    // modify by xiaohuyang, Set the  placeholder text of locationbar, 2020/08/26 --start
#if 0
    SetPlaceholderText(l10n_util::GetStringFUTF16(
        IDS_OMNIBOX_PLACEHOLDER_TEXT, default_provider->short_name()));
#else
    SetPlaceholderText(l10n_util::GetStringUTF16(IDS_OMNIBOX_PLACEHOLDER_TEXT_UOS));
#endif
    // modify by xiaohuyang, Set the placeholder text of locationbar, 2020/08/26 --end
  } else {
...
void OmniboxViewViews::OnTemplateURLServiceChanged() {
  //wangfeng: add TextChanged, search icon switch in the omnibox, 2020/08/19 --start
  TextChanged();
  //wangfeng: add TextChanged, search icon switch in the omnibox, 2020/08/19 --end
  InstallPlaceholderText();
}
