...
OmniboxResultView::OmniboxResultView(
  ...
  // modify by xiaohuyang, fix bug#89652.  2021/08/04
#if UNUSED
...
// modify by xiaohuyang, fix bug#89862.  2021/08/04
SkColor OmniboxResultView::GetColorUOS(OmniboxPart part) const {
  return GetThemeState() == OmniboxPartState::SELECTED ? SK_ColorWHITE : GetOmniboxColor(GetThemeProvider(), part, GetThemeState());
}
...
void OmniboxResultView::ApplyThemeAndRefreshIcons(bool force_reapply_styles) {
  // modify by xiaohuyang, handle task#81099.  2021/07/20
#if UNUSED
  bool high_contrast =
...
#else
  // Set the background and text colors in different states according to UI requirements.
  // You should pay attention to these fuctions,such as 'OnSelectionStateChanged','UpdateHoverState' and 'OnThemeChanged'.
  UpdateStyleOfOmniboxResultView(force_reapply_styles);
#endif
}
...
gfx::Image OmniboxResultView::GetIcon() const {
  // modify by xiaohuyang, handle task#81099.  2021/07/20  --start
  // Set the icon colors in different states according to UI requirements.
#if UNUSED
  return popup_contents_view_->GetMatchIcon(
      match_, GetColor(OmniboxPart::RESULTS_ICON));
#else
  bool use_dark = 
      GetNativeTheme() && GetNativeTheme()->ShouldUseDarkColors();
  SkColor icon_color;
  if (use_dark || GetThemeState() == OmniboxPartState::SELECTED) {
    icon_color = gfx::kGoogleGrey100;;
  } else {
    icon_color = gfx::kGoogleGrey700;
  }

  return popup_contents_view_->GetMatchIcon(
      match_, icon_color);
#endif
  // modify by xiaohuyang, handle task#81099.  2021/07/20  --end
}
...
// modify by xiaohuyang, handle task#81099.  2021/07/20  --start
// Set the background and text colors in different states according to UI requirements.
// The fuction should be excuted after 'SetMatch(Init)', 'OnSelectionStateChanged', 'UpdateHoverState' and 'OnThemeChanged'.
void OmniboxResultView::UpdateStyleOfOmniboxResultView(bool force_reapply_styles) {
  bool high_contrast =
      GetNativeTheme() && GetNativeTheme()->UsesHighContrastColors();
  bool use_dark = 
      GetNativeTheme() && GetNativeTheme()->ShouldUseDarkColors();
  ...
  if (GetThemeState() == OmniboxPartState::SELECTED) {
    background_color = gfx::kGoogleBlue600;
  } else if (GetThemeState() == OmniboxPartState::HOVERED && !use_dark) {
    background_color = gfx::kGoogleGrey300;
  } else if (GetThemeState() == OmniboxPartState::HOVERED && use_dark) {
    background_color = gfx::kGoogleGrey800;
  } else {
  ...
  SkColor text_color;
  if (use_dark || GetThemeState() == OmniboxPartState::SELECTED) {
    text_color = gfx::kGoogleGrey100;;
  } else {
    text_color = gfx::kGoogleGrey900;
  }

  //separator
  suggestion_view_->separator()->ApplyTextColor(text_color);
  keyword_view_->separator()->ApplyTextColor(text_color);

  //suggestion tab switch button
  if (suggestion_tab_switch_button_->GetVisible())
    suggestion_tab_switch_button_->UpdateBackground();
  ...
  keyword_view_->icon()->SetImage(gfx::CreateVectorIcon(
      omnibox::kKeywordSearchIcon, GetLayoutConstant(LOCATION_BAR_ICON_SIZE),
      (GetThemeState() == OmniboxPartState::SELECTED || use_dark) ? SK_ColorWHITE : gfx::kGoogleGrey600));
  ...
  if (high_contrast || force_reapply_styles) {
    ...
    suggestion_view_->content()->ApplyTextColor(text_color);
    suggestion_view_->description()->ApplyTextColor(text_color);
  } 
  if (keyword_view_->GetVisible()) {
    keyword_view_->description()->ApplyTextColor(text_color);
  }

  //remove suggestion button
  views::SetImageFromVectorIconUOS(remove_suggestion_button_,
                                vector_icons::kOmniboxResultViewCloseIcon,
                                GetLayoutConstant(LOCATION_BAR_ICON_SIZE),
                                (GetThemeState() == OmniboxPartState::SELECTED || use_dark) ? SK_ColorWHITE : gfx::kGoogleGrey600);
}
// modify by xiaohuyang, handle task#81099.  2021/07/20  --end
