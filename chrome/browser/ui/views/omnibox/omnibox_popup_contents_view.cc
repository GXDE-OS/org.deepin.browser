...
#ifdef USE_UNIONTECH_NPAPI
    BrowserView *browser_view = (BrowserView *)(parent_widget->GetNativeView()->GetNativeWindowProperty("__BROWSER_VIEW__"));
    if(browser_view->GetNpapiPluginMaskObserver(this)) {
      params.npapi_plugin_mask_observers.push_back(
        browser_view->GetNpapiPluginMaskObserver(this));
    }

    for (auto* observer_ : browser_view->GetNpapiPluginMaskObserverForChildFrame(this)) {
      if (observer_) {
