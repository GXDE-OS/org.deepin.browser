...
#ifndef CHROME_BROWSER_UI_VIEWS_TOOLBAR_RIGHT_TOOLBAR_BUTTON_H_
#define CHROME_BROWSER_UI_VIEWS_TOOLBAR_RIGHT_TOOLBAR_BUTTON_H_
...
//modify by xiaohuyang, Used to implement a rectangular rounded menu button without a background, 2020/08/12  --start
// This class provides basic drawing and mouse-over behavior for app menu button, Refer to the class ToolbarButton to implementation.
// In order not to introduce risks, only the radius of rounded rect border was modified when button state is hovered.
class ToolbarRightButton : public views::LabelButton,
                      public views::ContextMenuController {
 public:
  // More convenient form of the ctor below, when |model| and |tab_strip_model|
  // are both nullptr.
  explicit ToolbarRightButton(views::ButtonListener* listener);
  ...
  // shutdown.
  ToolbarRightButton(views::ButtonListener* listener,
                std::unique_ptr<ui::MenuModel> model,
                TabStripModel* tab_strip_model,
                bool trigger_menu_on_long_press = true);
  ToolbarRightButton(const ToolbarRightButton&) = delete;
  ToolbarRightButton& operator=(const ToolbarRightButton&) = delete;
  ~ToolbarRightButton() override;
  ...
  //xiaohuyang 设置toolbar右侧按钮与主窗口之间的间距
 ...
 private:
  ...
  class HighlightColorAnimation : gfx::AnimationDelegate {
   public:
    explicit HighlightColorAnimation(ToolbarRightButton* parent);
   ...
   private:
    ...
    ToolbarRightButton* const parent_;
    ...
    // set, uses the default ToolbarRightButton ink drop.
  ...
  // Layout insets to use. This is used when the ToolbarRightButton is not actually
  ...
  // that use smaller or larger icons than regular ToolbarRightButton instances.
  ...
  base::WeakPtrFactory<ToolbarRightButton> show_menu_factory_{this};
};
//modify by xiaohuyang, Used to implement a rectangular rounded menu button without a background, 2020/08/12  --end

#endif  // CHROME_BROWSER_UI_VIEWS_TOOLBAR_RIGHT_TOOLBAR_BUTTON_H_