...
[[maybe_unused]] constexpr int kBorderThicknessDpWithLabel = 1;
[[maybe_unused]] constexpr int kBorderThicknessDpWithoutLabel = 2;
...
void ToolbarButton::UpdateColorsAndInsets() {
  // modify by xiaohuyang, Set the background and borders of the navigation buttons under different themes. 2021/1/22  --start
  ...
  // modify by xiaohuyang, Use this to set background.  2021/1/19
  ...
  if (!border() || target_insets != border()->GetInsets() ||
    ...
    if (border_color) {
      ...
      // modify Use this to set border.  2021/1/19 
      // std::unique_ptr<views::Border> border = views::CreateRoundedRectBorder(
      //     1, 8, paint_insets, *border_color);
  ...
  if (GetNativeTheme()->ShouldUseDarkColors()) {
    background_color = SkColorSetRGB(0x44, 0x44, 0x44);
    border_color = SkColorSetRGB(0x4D, 0x4D, 0x4D);
  } else {
    background_color = SkColorSetRGB(0xE4, 0xE4, 0xE4);
    border_color = SkColorSetRGB(0xDC, 0xDC, 0xDC);
  }
  SetBackground(views::CreateRoundedRectBackground(
      background_color, highlight_radius));

  std::unique_ptr<views::Border> border = views::CreateRoundedRectBorder(
          thickness, highlight_radius, paint_insets, border_color);
  const gfx::Insets extra_insets = target_insets - border->GetInsets() - gfx::Insets(1, 1, 1, 1);
  SetBorder(views::CreatePaddedBorder(std::move(border), extra_insets));
  ...
  last_paint_insets_ = paint_insets;
#endif
  // modify by xiaohuyang, Set the background and borders of the navigation buttons under different themes. 2021/1/22  --end
...
void ToolbarButton::OnThemeChanged() {
  ...
  std::unique_ptr<views::Border> border = views::CreateRoundedRectBorder(
          1, highlight_radius, paint_insets, border_color);
  const gfx::Insets extra_insets = target_insets - border->GetInsets() - gfx::Insets(1, 1, 1, 1);
  SetBorder(views::CreatePaddedBorder(std::move(border), extra_insets));
