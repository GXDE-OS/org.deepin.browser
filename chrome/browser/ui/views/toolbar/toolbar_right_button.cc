...
#include "chrome/browser/ui/views/toolbar/toolbar_right_button.h"
...
ToolbarRightButton::ToolbarRightButton(views::ButtonListener* listener)
    : ToolbarRightButton(listener, nullptr, nullptr) {}
...
ToolbarRightButton::~ToolbarRightButton() {}

void ToolbarRightButton::Init() {
  SetFocusBehavior(FocusBehavior::ACCESSIBLE_ONLY);
}

void ToolbarRightButton::SetHighlight(const base::string16& highlight_text,
...
void ToolbarRightButton::SetText(const base::string16& text) {
  LabelButton::SetText(text);
  UpdateColorsAndInsets();
}

void ToolbarRightButton::ClearHighlight() {
  highlight_color_animation_.Hide();
  ShrinkDownThenClearText();
}

void ToolbarRightButton::UpdateColorsAndInsets() {
...
SkColor ToolbarRightButton::GetForegroundColor(ButtonState state) const {
...
void ToolbarRightButton::UpdateIconsWithColors(const gfx::VectorIcon& icon,
...
void ToolbarRightButton::UpdateIconsWithStandardColors(const gfx::VectorIcon& icon) {
...
void ToolbarRightButton::SetLabelSideSpacing(int spacing) {
...
void ToolbarRightButton::SetLayoutInsetDelta(const gfx::Insets& inset_delta) {
...
void ToolbarRightButton::SetTrailingMargin(int margin) {
...
void ToolbarRightButton::ClearPendingMenu() {
  show_menu_factory_.InvalidateWeakPtrs();
}

bool ToolbarRightButton::IsMenuShowing() const {
  return menu_showing_;
}

void ToolbarRightButton::SetLayoutInsets(const gfx::Insets& insets) {
...
void ToolbarRightButton::OnBoundsChanged(const gfx::Rect& previous_bounds) {
  if (size() != previous_bounds.size())
    UpdateColorsAndInsets();
  LabelButton::OnBoundsChanged(previous_bounds);
}

void ToolbarRightButton::OnThemeChanged() {
  LabelButton::OnThemeChanged();
  if (installable_ink_drop_)
    installable_ink_drop_->SetConfig(GetToolbarInstallableInkDropConfig(this));
}

gfx::Rect ToolbarRightButton::GetAnchorBoundsInScreen() const {
...
bool ToolbarRightButton::OnMousePressed(const ui::MouseEvent& event) {
  if (trigger_menu_on_long_press_ && IsTriggerableEvent(event) &&
    ...
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
        FROM_HERE,
        base::BindOnce(&ToolbarRightButton::ShowDropDownMenu,
...
bool ToolbarRightButton::OnMouseDragged(const ui::MouseEvent& event) {
...
void ToolbarRightButton::OnMouseReleased(const ui::MouseEvent& event) {
...
void ToolbarRightButton::OnMouseCaptureLost() {}

void ToolbarRightButton::OnMouseExited(const ui::MouseEvent& event) {
...
void ToolbarRightButton::OnGestureEvent(ui::GestureEvent* event) {
  if (menu_showing_) {
...
void ToolbarRightButton::GetAccessibleNodeData(ui::AXNodeData* node_data) {
  Button::GetAccessibleNodeData(node_data);
  if (model_)
    node_data->SetHasPopup(ax::mojom::HasPopup::kMenu);
}

std::unique_ptr<views::InkDrop> ToolbarRightButton::CreateInkDrop() {
...
std::unique_ptr<views::InkDropHighlight> ToolbarRightButton::CreateInkDropHighlight()
    const {
  // Ensure this doesn't get called when InstallableInkDrops are enabled.
  DCHECK(!base::FeatureList::IsEnabled(views::kInstallableInkDropFeature));
...
SkColor ToolbarRightButton::GetInkDropBaseColor() const {
  // Ensure this doesn't get called when InstallableInkDrops are enabled.
  DCHECK(!base::FeatureList::IsEnabled(views::kInstallableInkDropFeature));
...
views::InkDrop* ToolbarRightButton::GetInkDrop() {
  if (installable_ink_drop_)
    return installable_ink_drop_.get();
  return views::LabelButton::GetInkDrop();
}

void ToolbarRightButton::ShowContextMenuForViewImpl(View* source,
...
// static
SkColor ToolbarRightButton::AdjustHighlightColorForContrast(
...
// static
SkColor ToolbarRightButton::GetDefaultBackgroundColor(
    const ui::ThemeProvider* theme_provider) {
  return color_utils::GetColorWithMaxContrast(
...
// static
SkColor ToolbarRightButton::GetDefaultBorderColor(views::View* host_view) {
  return SkColorSetA(GetToolbarInkDropBaseColor(host_view),
                     kToolbarButtonBackgroundAlpha);
}

bool ToolbarRightButton::ShouldShowMenu() {
  return model_ != nullptr;
}

void ToolbarRightButton::ShowDropDownMenu(ui::MenuSourceType source_type) {
  if (!ShouldShowMenu())
    return;
  ...
  // Exit if the model is null. Although ToolbarRightButton::ShouldShowMenu()
  ...
  menu_model_adapter_ = std::make_unique<views::MenuModelAdapter>(
      model_.get(), base::BindRepeating(&ToolbarRightButton::OnMenuClosed,
...
void ToolbarRightButton::OnMenuClosed() {
...
const char* ToolbarRightButton::GetClassName() const {
  return "ToolbarRightButton";
}
...
ToolbarRightButton::HighlightColorAnimation::HighlightColorAnimation(
    ToolbarRightButton* parent)
...
void ToolbarRightButton::HighlightColorAnimation::Show(
    base::Optional<SkColor> highlight_color) {
...
void ToolbarRightButton::HighlightColorAnimation::Hide() {
  highlight_color_animation_.Hide();
}

base::Optional<SkColor> ToolbarRightButton::HighlightColorAnimation::GetTextColor()
    const {
...
base::Optional<SkColor> ToolbarRightButton::HighlightColorAnimation::GetBorderColor()
    const {
  if (!IsShown() || !parent_->GetThemeProvider()) {
    return base::nullopt;
  }
  ...
  if (highlight_color_) {
    ...
    border_color = ToolbarRightButton::GetDefaultBorderColor(parent_);
  }
  return FadeWithAnimation(border_color, highlight_color_animation_);
}

base::Optional<SkColor>
ToolbarRightButton::HighlightColorAnimation::GetBackgroundColor() const {
  if (!IsShown() || !parent_->GetThemeProvider())
    return base::nullopt;
  SkColor bg_color =
...
base::Optional<SkColor>
ToolbarRightButton::HighlightColorAnimation::GetInkDropBaseColor() const {
  if (!highlight_color_)
    return base::nullopt;
  return *highlight_color_;
}

void ToolbarRightButton::HighlightColorAnimation::AnimationEnded(
...
void ToolbarRightButton::HighlightColorAnimation::AnimationProgressed(
    const gfx::Animation* animation) {
  parent_->UpdateColorsAndInsets();
}

bool ToolbarRightButton::HighlightColorAnimation::IsShown() const {
  return highlight_color_animation_.is_animating() ||
         highlight_color_animation_.GetCurrentValue() == 1.0f;
}

void ToolbarRightButton::HighlightColorAnimation::ClearHighlightColor() {
