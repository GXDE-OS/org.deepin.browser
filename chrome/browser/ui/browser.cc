...
void Browser::OnTabStripModelChanged(TabStripModel* tab_strip_model,
  ...
  if (!selection.active_tab_changed()){
    //fix bug#80467. 在当前活跃标签页未发生变化，同时关闭其他标签页的时候，更新标签页状态 --start
    //@xudongxu 2021/05/20
    command_controller_->TabStateChanged();
    //fix bug#80467. 在当前活跃标签页未发生变化，同时关闭其他标签页的时候，更新标签页状态 --end
    return;
  }
...
void Browser::SetAsDelegate(WebContents* web_contents, bool set_delegate) {
  ...
  //modify by xiepinze, disable google translate, 2020/07/08 --start
  ...
  //modify by xiepinze, disable google translate, 2020/07/08 --end
}
