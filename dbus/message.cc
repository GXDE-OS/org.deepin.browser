...
void OutputStringHeader(const std::string& header_name,
                        const std::string& header_value) {
  if (!header_value.empty()) {
    VLOG(1) << header_name << ": " << header_value;
  }
}
...
void OutputUint32Header(const std::string& header_name,
                        uint32_t header_value) {
  if (header_value != 0) {
    VLOG(1) << header_name + ": " + base::NumberToString(header_value);
  }
}
...
void Message::Output() {
  ...
  OutputStringHeader("message_type", GetMessageTypeAsString());
  OutputStringHeader("destination", GetDestination());
  OutputStringHeader("path", GetPath().value());
  OutputStringHeader("interface", GetInterface());
  OutputStringHeader("member", GetMember());
  OutputStringHeader("error_name", GetErrorName());
  OutputStringHeader("sender", GetSender());
  OutputStringHeader("signature", GetSignature());
  OutputUint32Header("serial", GetSerial());
  OutputUint32Header("reply_serial", GetReplySerial());
}
