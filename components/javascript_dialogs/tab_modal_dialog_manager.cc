...
void TabModalDialogManager::RunBeforeUnloadDialogForBankAndIRC(
    content::WebContents* web_contents,
    content::RenderFrameHost* render_frame_host,
    bool is_reload,
    bool is_tab_close,
  ...
  return GetAppModalDialogManager()->RunBeforeUnloadDialogWithOptionsForBankAndIRC(
      web_contents, render_frame_host, is_reload, is_tab_close, bank, message, delegate_->IsApp(),
      std::move(callback));
}
