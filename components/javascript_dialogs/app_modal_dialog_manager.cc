...
//modify by xiepinze, js dialog modify, 20201027, start
    ...
    return base::i18n::GetDisplayStringInLTRDirectionality(url_string);
...
void AppModalDialogManager::RunBeforeUnloadDialogForBankAndIRC(
    content::WebContents* web_contents,
    content::RenderFrameHost* render_frame_host,
    bool is_reload,
    bool is_tab_close,
    ...
    DialogClosedCallback callback) {
  RunBeforeUnloadDialogWithOptionsForBankAndIRC(web_contents, render_frame_host, is_reload,
                                                is_tab_close, bank, message,
                                                false, std::move(callback));
}
...
//modify by xiaohuyang, Handle the task of #82352.
void AppModalDialogManager::RunBeforeUnloadDialogWithOptionsForBankAndIRC(
    content::WebContents* web_contents,
    content::RenderFrameHost* render_frame_host,
    bool is_reload,
    bool is_tab_close,
  ...
  if (!bank.empty() && !is_reload && is_tab_close) {
  ...
  AppModalDialogQueue::GetInstance()->AddDialog(new AppModalDialogController(
      web_contents, &javascript_dialog_extra_data_, title,
      content::JAVASCRIPT_DIALOG_TYPE_CONFIRM, dialog_message,
      base::string16(),  // default_prompt_text
      (!bank.empty() && !is_reload && is_tab_close) ? false : ShouldDisplaySuppressCheckbox(extra_data),
      true,  // is_before_unload_dialog
      is_reload,
      is_tab_close,
      bank,
