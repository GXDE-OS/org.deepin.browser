...
bool isGMCertificateType(scoped_refptr<net::X509Certificate> cert_chain)
{
  net::ScopedCERTCertificateList certs =
      net::x509_util::CreateCERTCertificateListFromX509Certificate(cert_chain.get());

  CERTCertificate* cert = certs.front().get();
  std::string strSignatureAlgorithm = CERT_GetOidString(&(cert->signature.algorithm));
  //LOG(INFO) << "SignatureAlgorithm in certificate:" << strSignatureAlgorithm;
  if(!strSignatureAlgorithm.compare("OID.1.2.156.10197.1.501"))
  {
    return true;
  }     
...
SecurityLevel GetSecurityLevel(
  ...
  //GMTLS
  #ifndef OPENSSL_NO_GMTLS
  if(isGMCertificateType(visible_security_state.certificate))
  {
    return GM_SECURE;
  }
  #endif
