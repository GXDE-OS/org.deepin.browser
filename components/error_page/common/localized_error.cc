...
//modify by xiaohuyang, Obtain the 'class' used for the css file according to the error code network disconnected, 404, cann't access,  2020/08/20 --start
const int net_connection_problem_use_style[] = {
  net::ERR_TIMED_OUT,
  net::ERR_CONNECTION_RESET,
  net::ERR_CONNECTION_TIMED_OUT,
  net::ERR_CONNECTION_CLOSED,
...
bool IsUseNetConnectionProblemStyle(const int maps[],
                                int error_code) {
  for (size_t i = 0; i < base::size(net_connection_problem_use_style); ++i) {
    if (maps[i] == error_code)
      return true;
  }
  return false;
}
...
const char* GetIconClassForError(const std::string& error_domain,
  ...
  if(IsOfflineError(error_domain, error_code)){
    return "icon-offline-uos";
  } else if(IsUseNetConnectionProblemStyle(net_connection_problem_use_style, error_code)){
    return "icon-generic-uos";
...
// Gets the Main Frame Error class for a given |error_domain| and |error_code|.
const char* GetClassForMainFrameError(const std::string& error_domain,
                                 int error_code, int heading_id) {
  if(IsOfflineError(error_domain, error_code)){
    return "interstitial-wrapper-disconnected-uos";
  } else if(IsUseNetConnectionProblemStyle(net_connection_problem_use_style, error_code)){
    return "normal-error-uos";
  } else if (error_code == 404 ){
    return "interstitial-wrapper-404-uos";
  } else {
    return "interstitial-wrapper-invalid-uos";
  }
}
//modify by xiaohuyang, Obtain the 'class' used for the css file according to the error code network disconnected, 404, cann't access,  2020/08/20 --end
  ...
  switch (error_code) {
    case net::ERR_TOO_MANY_REDIRECTS:
      ...
      suggestion_string = l10n_util::GetStringUTF16(
          IDS_ERRORPAGES_SUGGESTION_CLEAR_COOKIES_SUMMARY_UOS);
      break;
    default:
      NOTREACHED();
      break;
...
//modify by xiepinze, delete proxy config, 2020/08/18, start
  if (suggestions & SUGGEST_PROXY_CONFIG) {
    ...
    suggest_proxy_config->SetString("body",
        l10n_util::GetStringUTF16(IDS_ERRORPAGES_SUGGESTION_PROXY_CONFIG_BODY_UOS));
  }
//modify by xiepinze, delete proxy config, 2020/08/18, end
#endif
...
LocalizedError::PageState LocalizedError::GetPageState(
  ...
  //modify by xiaohuyang, Set data for 'mainFrameErrorClass' applied to 'neterror.html',  2020/08/20 --start
  std::string main_frame_error_class = GetClassForMainFrameError(error_domain, error_code, options.heading_resource_id);
  result.strings.SetString("mainFrameErrorClass", main_frame_error_class);
  //modify by xiaohuyang, Set data for 'mainFrameErrorClass' applied to 'neterror.html',  2020/08/20 --end
