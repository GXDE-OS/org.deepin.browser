...
VerifierResult VerifyCrx3(
  ...
  if (GetUOSDefaultExtensionProperty(UOSExtensionProperty::ID_EXIST, declared_crx_id)) {
    require_publisher_key = false;
  }
...
std::vector<UOSDefaultExtension> GetUOSDefaultExtensionPath()
{
  static const UOSDefaultExtension kUOSDefaultExtension[] = {
      {"/usr/share/browser-extensions",
       {"glbejanjnaknkiideapodijpcgeemhfj", "eamchncaadaikjedddikpgopndpgfdma"},
       "consumer"
      },
      {"/usr/share/downloader/extension",
       {"ojlicckikdkkaclkpdddijgehekpmmbg"},
       "downloader"
      },
  };

  std::vector<UOSDefaultExtension> uos_default_extension_group;
  for (const UOSDefaultExtension& path : kUOSDefaultExtension) {
    uos_default_extension_group.push_back(path);
  }
  return uos_default_extension_group;
}

bool GetUOSDefaultExtensionProperty(const UOSExtensionProperty property, const std::string& crx_id)
{
  bool result = true;
  bool id_exist_result = false;
  for (const UOSDefaultExtension& path : GetUOSDefaultExtensionPath()) {
    std::string input;
    base::ReadFileToString(base::FilePath(path.pathname).AppendASCII("info.json"), &input);
    std::unique_ptr<base::DictionaryValue> root = base::DictionaryValue::From(base::JSONReader::ReadDeprecated(input));

    if (std::find(path.allow_list.begin(), path.allow_list.end(), crx_id) != path.allow_list.end() && root) {
      for (auto it = root->begin(); it != root->end(); ++it) {
        if (crx_id == it->first) {
          if (property == UOSExtensionProperty::TOOLBAR_VISIABLE) {
            base::Value *value = it->second->FindKey("in_toolbar_visiable");
            result = value ? value->GetBool() : result;
          } else if (property == UOSExtensionProperty::TOOLBAR_CONTEXT_MENU) {
            base::Value *value = it->second->FindKey("in_toolbar_context_menu");
            result = value ? value->GetBool() : result;
          } else if (property == UOSExtensionProperty::WEBUI_VISIABLE) {
            base::Value *value = it->second->FindKey("in_webui_visiable");
            result = value ? value->GetBool() : result;
          } else if (property == UOSExtensionProperty::INCOGNITO_DISABLE) {
            base::Value *value = it->second->FindKey("incognito_disable");
            result = value ? value->GetBool() : result;
          } else if (property == UOSExtensionProperty::CAN_BE_DELETE) {
            base::Value *value = it->second->FindKey("can_be_delete");
            result = value ? value->GetBool() : result;
          } else if (property == UOSExtensionProperty::CAN_BE_DISABLE) {
            base::Value *value = it->second->FindKey("can_be_disable");
            result = value ? value->GetBool() : result;
          } else if (property == UOSExtensionProperty::ID_EXIST) {
            id_exist_result = true;
          }
        }
      }
    }
  ...
  if (property == UOSExtensionProperty::ID_EXIST) {
    return id_exist_result;
  } else {
    return result;
  }
}
