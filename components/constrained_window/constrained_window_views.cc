...
views::Widget* ShowWebModalDialogViews(
  ...
  auto* web_widget_view = web_contents->GetRenderWidgetHostView();
  views::NpapiPluginMaskObserver *plugin_mask_observer = nullptr;
  if(web_widget_view &&
      (plugin_mask_observer = web_widget_view->GetNpapiPluginMaskObserver(widget)))
    widget->GetNpapiPuginMask()->AddObserver(plugin_mask_observer);

  if (web_contents) {
    for (auto* view : web_contents->GetRenderWidgetChildFrameView()) {
      if (view && view->GetNpapiPluginMaskObserver(widget)) {
        widget->GetNpapiPuginMask()->AddObserver(view->GetNpapiPluginMaskObserver(widget));
      }
    }
  }
#endif
