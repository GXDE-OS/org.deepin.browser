  ...
  *write_url = false;
  ...
  if (!user_input_in_progress_ &&
    ...
    *url_from_text = controller()->GetLocationBarModel()->GetURL();
    #if 0
    //modify by xiaohuyang, Adjust text as 'uos://about' when url scheme is 'chrome' for copy, 2020/08/14 --start
    if(url_from_text->SchemeIs(content::kChromeUIScheme)){
      *url_from_text = GURL(("uos://") + url_from_text->GetContent());
    }
    #endif
    //modify by xiaohuyang, Adjust text as 'uos://about' when url scheme is 'chrome' for copy, 2020/08/14 --end
    *write_url = true;
