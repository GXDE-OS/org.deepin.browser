...
base::string16 LocationBarModelImpl::GetFormattedURL(
  ...
  #if 0 
  //modify by xiaohuyang, Added uos:xxx access to the built-in browser page, 2020/07/23-2020/08/14 --start
  //Display: Replace'chrome:xxx' with'uos:xxx' in the address bar to display
  if(url.SchemeIs(content::kChromeUIScheme)){
      url = GURL(("uos://") + url.GetContent());
  }
...
const gfx::VectorIcon& LocationBarModelImpl::GetVectorIcon() const {
  ...
  switch (security_level) {
    ...
    case security_state::GM_SECURE:
      return vector_icons::kGmSecureIcon;
    #endif
...
base::string16 LocationBarModelImpl::GetSecureDisplayText() const {
  ...
  switch (GetSecurityLevel()) {
    ...
    case security_state::GM_SECURE:
    {
      return l10n_util::GetStringUTF16(IDS_NATIONAL_CIPHER_VERBOSE_STATE);
    }
    #endif
    default:
      return base::string16();
