...
void GetBookmarksMatchingPropertiesImpl(
  ...
  while (iterator.has_next()) {
    ...
    //Fix bug#54679. Modify the rule for bookmarks matching. --start
    //@xudongxu 2021/04/30
    // if ((!query_words.empty() &&
    //   !DoesBookmarkContainWords(node->GetTitle(), node->url(),    
    base::string16* keyword = query.word_phrase_query.get();
    if (keyword && node->GetTitledUrlNodeTitle().find(base::i18n::ToLower(*keyword))
                                            != base::string16::npos) {      
      nodes->push_back(node);
    } else if (!query_words.empty()) {
      if (!DoesBookmarkContainWords(node->GetTitle(), node->url(),
    //Fix bug#54679. Modify the rule for bookmarks matching. --end
    //@xudongxu 2021/04/30
                                    query_words) ||
