...
bool QueryNodeWord::Matches(const base::string16& word, bool exact) const {
  if (exact ||
      !QueryParser::IsWordLongEnoughForPrefixSearch(word_, matching_algorithm_))
    //modify by hanll, 修改中文历史匹配规则, 2020/08/15 --start
    //return word == word_;
    return word.find(word_) != base::string16::npos;
    //modify by hanll, 修改中文历史匹配规则, 2020/08/15 --end

  //modify by xudongxu, 修改URL历史匹配规则, 2021/04/27 --START
  // return word.size() >= word_.size() &&
  //        (word_.compare(0, word_.size(), word, 0, word_.size()) == 0);
  return word.size() >= word_.size() &&
         (word.find(word_) != base::string16::npos);
  //modify by xudongxu, 修改URL历史匹配规则, 2022/04/27 --END
}
