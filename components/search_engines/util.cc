...
void GetSearchProvidersUsingKeywordResult(
  ...
  // modify by xiaohuyang, fix bug#84630, 2021/07/14  --start
  // Modify the value of keyword version so that the function 'MergeEnginesFromPrepopulateData' is executed every time the browser starts,
  // so as to ensure that the browser version 1040 and 1050 can get the corresponding search engine collection.
#if UNUSED
  *new_resource_keyword_version = keyword_result.builtin_keyword_version;
#else
  *new_resource_keyword_version = 0;
#endif
  // modify by xiaohuyang, fix bug#84630, 2021/07/14  --end
