...
// static
// wangjuna: 增加一个接口，用于设置页面添加https网站时，自动补上443端口，2020/08/27 --start
ContentSettingsPattern ContentSettingsPattern::FromStringUOS(
    base::StringPiece pattern_spec) {
  ContentSettingsPattern::Builder builder;
  content_settings::PatternParser::Parse(pattern_spec, &builder);
  if (pattern_spec.starts_with("https:")) {
    builder.WithPort("443");
  } else if (pattern_spec.starts_with("http:")) {
    builder.WithPort("80");
  }
  return builder.Build();
}
// wangjuna: 增加一个接口，用于设置页面添加https网站时，自动补上443端口，2020/08/27 --end
...
const std::string& ContentSettingsPattern::GetPort() const {
  return parts_.port;
}
