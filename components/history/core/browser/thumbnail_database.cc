...
// modify by xiaohuyang, fix bug#85314,  2021/07/01
// Get application Locale and define some favicon url constants.
#if 1
#include "base/i18n/rtl.h"
#include "chrome/browser/browser_process.h"

const base::StringPiece bing_favicon_url = "https://www.bing.com/sa/simg/bing_p_rr_teal_min.ico";
const base::StringPiece bing_cn_favicon_url = "https://cn.bing.com/sa/simg/favicon-2x.ico";
...
favicon_base::FaviconID ThumbnailDatabase::GetFaviconIDForFaviconURL(
  ...
  // modify by xiaohuyang, fix bug#85314,  2021/07/01  --start
  // For "chrome://settings/searchEngines", because the favicon url corresponding to the "bing" search engine is redirected, 
  // special processing is required when reading the data.
#if UNUSED
  statement.BindString(0, URLDatabase::GURLToDatabaseURL(icon_url));
#else
  GURL favicon_url = icon_url;
 
  if (icon_url.spec() == bing_favicon_url && GetApplicationLocale() == "zh-CN") {
    favicon_url = GURL(bing_cn_favicon_url);
  }

  statement.BindString(0, URLDatabase::GURLToDatabaseURL(favicon_url));
#endif
  // modify by xiaohuyang, fix bug#85314,  2021/07/01  --end
