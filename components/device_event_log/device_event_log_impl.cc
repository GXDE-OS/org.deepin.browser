...
std::string LogEntryToStringWithoutTime(const DeviceEventLogImpl::LogEntry& log_entry,
                             ShowTime show_time,
                             bool show_file,
                             bool show_type,
                             bool show_level) {
  std::string line;
  // Delete time info because it have been shown in LOG or VLOG
  if (show_type)
    line += GetLogTypeString(log_entry.log_type) + ": ";
  if (show_level) {
    const char* kLevelDesc[] = {"ERROR", "USER", "EVENT", "DEBUG"};
...
void SendLogEntryToVLogOrErrorLog(
  ...
  std::string output =
      LogEntryToStringWithoutTime(log_entry, show_time, show_file, show_type, show_level);
