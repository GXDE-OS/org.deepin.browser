...
// wangjuna: 禁用GM, 不再加载 uos_nssdb
#if 0
    // GMTLS
#ifndef OPENSSL_NO_GMTLS
      //加载uos_nssdb
    PK11SlotInfo* db_slot = SECMOD_OpenUserDB("configDir='sql:/etc/browser/.pki/uos_nssdb' tokenDescription='' flags=readOnly");
    if (db_slot) {
      if (PK11_NeedUserInit(db_slot))
        PK11_InitPin(db_slot, nullptr, nullptr);
    } else {
      LOG(ERROR) << "Error initializing NSS with a persistent database (/etc/browser/.pki/uos_nssdb) : " << GetNSSErrorMessage();
    }
#endif
#endif
