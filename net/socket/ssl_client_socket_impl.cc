...
#ifndef OPENSSL_NO_GMTLS 
std::map<std::string,std::map<std::string,int>> SSLClientSocketImpl::connects_try_;
#endif
...
SSLClientSocketImpl::SSLClientSocketImpl(
  ...
  {
    version_type_="";
    if(connects_try_.find(host_and_port_.ToString()) == connects_try_.end())
    {
      std::map<std::string,int> map;
      map.insert(std::make_pair("GM",0));
      map.insert(std::make_pair("NM",0));
      connects_try_.insert(std::make_pair(host_and_port_.ToString(),map));
    }
  }
  #endif
}
...
// GMTLS
#ifndef OPENSSL_NO_GMTLS
  cert_request_info->is_gm_ = ( SSL_version(ssl_.get()) == GMTLS_VERSION);
#endif
...
int SSLClientSocketImpl::Init() {
  ...
  if(SSL_get_gm_mode() == USE_GM_MODE){
    std::map<std::string,std::map<std::string,int>>::iterator itr = connects_try_.find(host_and_port_.ToString());
    if(itr == connects_try_.end()){
      //connect process,new connects_try_ item!
      std::map<std::string,int> map;
      map.insert(std::make_pair("GM",0));
      map.insert(std::make_pair("NM",0));
      connects_try_.insert(std::make_pair(host_and_port_.ToString(),map));
    ...
    if((itr->second)["NM"]<=(itr->second)["GM"]){
      //normal version is set.
      version_min = 
      ...
      //gm version is set.
      version_min = GMTLS_VERSION;
      version_max = GMTLS_VERSION;
      version_type_="GM";
    }
  }
  else{
  ...
  // OpenSSL默认一些选项为on，其他选项为off。为了避免模棱两可，把我们关心的一切都设为绝对值
  ...
  // 使用BoringSSL默认值，但在ECDSA中禁用HMAC-SHA1密码。这些是其余的CBC模式ECDSA密码。
  ...
  // //配置BoringSSL以允许重新协商。一旦初始握手完成，如果不允许重新协商，将恢复默认拒绝值。这样做是为了允许BoringSSL优化。使用ssl_renegotiate_explicit，而不是ssl_renegotiate_freely，这样DoPeek（）就不会触发重新协商。
...
#ifndef OPENSSL_NO_GMTLS  
    if((net_error == ERR_CONNECTION_CLOSED || net_error == ERR_SSL_PROTOCOL_ERROR || 
        net_error == ERR_CONNECTION_RESET || net_error == ERR_SSL_VERSION_OR_CIPHER_MISMATCH) 
        && SSL_get_gm_mode() == USE_GM_MODE){
      std::map<std::string,std::map<std::string,int>>::iterator itr = connects_try_.find(host_and_port_.ToString());
      if(itr == connects_try_.end()){
        next_handshake_state_ = STATE_NONE;
        return ERR_UNEXPECTED;
        ...
        //LOG(ERROR)<<"connects encountered error,will retry handshake,"<<host_and_port_.ToString()<<" was being visited."
        //<<"returned:"<<rv<<",ssl error:"<<ssl_error<<",net_error:"<<net_error;
        //LOG(ERROR)<<version_type_<<"'s count is added.";
        connects_try_[itr->first][version_type_]++;
      }
      next_handshake_state_ = STATE_NONE;
      return net_error;
    }
...
int SSLClientSocketImpl::DoHandshakeComplete(int result) {
  ...
  // #ifndef OPENSSL_NO_GMTLS 
  // std::map<std::string,int>::iterator itr = connects_try_.find(host_and_port_.ToString());
  // if(itr!=connects_try_.end())
  // {
  //   connects_try_.erase(itr);
  // }
  // #endif
  return OK;
}
...
ssl_verify_result_t SSLClientSocketImpl::VerifyCert() {
  ...
  // 在这个配置中，BoringSSL将只执行一个证书验证，因此不能有来自先前验证的状态。
  CHECK(!server_cert_);
  // 获取服务器证书
  ...
  // OpenSSL解码了证书，但X509Certificate无法实现。这将被视为致命的SSL级协议错误，而不是证书错误。
  if (!server_cert_) {
    OpenSSLPutNetError(FROM_HERE, ERR_SSL_SERVER_CERT_BAD_FORMAT);
    LOG(ERROR)<<"VerifyCert,ERR_SSL_SERVER_CERT_BAD_FORMAT";
...
// GMTLS
#ifndef OPENSSL_NO_GMTLS	
      SSL_version(ssl_.get()) != GMTLS_VERSION &&
#endif   
  ...
  // LOG(ERROR)<<"HandleVerifyResult,ssl_verify_invalid,result:"<<result;
  return ssl_verify_invalid;
}
...
int SSLClientSocketImpl::ClientCertRequestCallback(SSL* ssl) {
  ...
  if (!send_client_cert_) {
    ...
    // 第一步：我们知道需要一个客户端证书，但是我们手头没有。暂停握手。SSL_get_error将返回SSL_error_WANT_X509_LOOKUP。
    // LOG(ERROR)<<"a client certificate is needed, but we do not have one at hand. Suspend the handshake.";
    return -1;
  }
