...
class NET_EXPORT Parser {
 public:
  ...
  // to be 0x30 (SEQUENCE).
  // 一种更具体的ReadConstructed形式，它期望当前标记为0x30（SEQUENCE）
  ...
  // next TLV.
  // 低级方法。将上一个TLV的指针与下一个TLV的内部方法一起读取较低级别的方法将这两个步骤分离为从当前TLV读取的方法和一个将内部指针推进到下一个TLV
  ...
  // 从输入读取当前TLV，将标记放入| tag |，将原始值放入| out |，但不推进输入。如果成功读取标记和长度并且输出存在，则返回true
