...
CertPathBuilder::Result TryBuildPath(
  ...
  // 路径构建将要求候选路径至少符合|user_initial_policy_set|中的一个策略。
...
int AssignVerifyResult(X509Certificate* input_cert,
  ...
  if (!best_path_possibly_invalid) {
    ...
    // 要传达哪些错误？也许路径生成器应该总是返回一些部分路径（即使只是包含目标），那么有一个certferrors要测试。
  ...
  if (!path_is_valid) {
    ...
    partial_path.errors.OutputDebugString(partial_path.certs);
  }
