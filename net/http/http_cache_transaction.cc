...
HttpCache::Transaction::disableCache * HttpCache::Transaction::disableCache::instance_ = nullptr;
...
int HttpCache::Transaction::DoSuccessfulSendRequest() {
  ...
  if(disableCache::get_instance()->urls_disablecahce_.count(request_->url.host())){
    if(NonErrorResponse(new_response_->headers->response_code()) 
        && (entry_ && !entry_->doomed)){
      LOG(ERROR)<<"cache_->DoomMainEntryForUrl";
      int ret = cache_->DoomEntry(cache_key_, nullptr);
      DCHECK_EQ(OK, ret);
    }
    DoneWithEntry(true);
...
int HttpCache::Transaction::RestartNetworkRequestWithCertificate(
  ...
  disableCache::get_instance()->urls_disablecahce_.insert(request_->url.host());
  if (rv != ERR_IO_PENDING)
    return DoLoop(rv);
  return rv;
}
