...
class NET_EXPORT_PRIVATE HttpCache::Transaction : public HttpTransaction {
 ...
 private:
  ...
  class disableCache{
  public:
    static disableCache *get_instance(){
      if(!instance_){
        instance_ = new disableCache();
      }
      return instance_;
    }
    std::set<std::string> urls_disablecahce_;
  private:
    static disableCache * instance_;
  };

  // static std::string url_disablecahce_;
