...
//wangfeng, add, 2020/12/15 --start
//添加os-version文件读取分析功能
bool ReadEdtionFromOSVersionFile(const char* file) {
  static const char kEditionName[] = "EditionName";

  std::string os_edtion_content;
  if (!ReadFileToString(FilePath(file), &os_edtion_content))
    return false;

  std::string edition_name =
      GetKeyValueFromOSReleaseFile(os_edtion_content, kEditionName);
  if (edition_name.empty())
    return false;

  SetUosEdition(edition_name);
  return true;
}

// https://www.freedesktop.org/software/systemd/man/os-release.html
class EditionNameGetter {
 public:
  EditionNameGetter() {
    static const char* kFilesVersion= "/etc/os-version";
    if (ReadEdtionFromOSVersionFile(kFilesVersion))
      return;
  }
};
//wangfeng, add, 2020/12/15 --end
#endif  // !defined(OS_CHROMEOS)

// Account for the terminating null character.
constexpr int kDistroSize = 128 + 1;

//wangfeng, add, 2020/12/15
constexpr int kEdtionSize = 128 + 1;
...
//wangfeng, add, 2020/12/15
// uos edition
char g_uos_edition[kEdtionSize] = "Unknown";
...
//wangfeng, add, 2020/12/15
std::string GetUosEdition() {
#if !defined(OS_CHROMEOS)
  // We do this check only once per process. If it fails, there's
  // little reason to believe it will work if we attempt to run it again.
  static NoDestructor<EditionNameGetter> edition_name_getter;
#endif
  return g_uos_edition;
}
...
//wangfeng, add, 2020/12/15
void SetUosEdition(const std::string& edition) {
  std::string trimmed_edition;
  TrimWhitespaceASCII(edition, TRIM_ALL, &trimmed_edition);
  strlcpy(g_uos_edition, trimmed_edition.c_str(), kEdtionSize);
}
