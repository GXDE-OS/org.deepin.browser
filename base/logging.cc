...
#define UOS_LOG_FILE_SIZE   (1024 * 1024 * 20) //20M
#define UOS_LOG_FILES_COUNT 5

enum {
  FILE_SIZE_NOT_EXCEED_LIMIT = 0,
...
const char* const log_severity_names[] = {"Debug", "Info", "Warning", "Error", "Fatal"};
static_assert(LOG_NUM_SEVERITIES == base::size(log_severity_names),
              "Incorrect number of log_severity_names");

const char* log_severity_name(int severity) {
  if (abs(severity) < LOG_NUM_SEVERITIES)
    return log_severity_names[abs(severity)];
  return "Unknown";
}
...
PathString GetDefaultLogFile() {
  ...
  return log_name;
#elif defined(OS_POSIX) || defined(OS_FUCHSIA)
  // On other platforms we just use the UOS application directory.
  base::FilePath log_path = base::GetHomeDir().Append(".cache/deepin/org.deepin.browser");
  if (!base::PathExists(log_path) ) {
    base::CreateDirectory(log_path);
  }

  return log_path.Append("org.deepin.browser.log").value();
#endif
}
...
void RenameLogFile(base::Time::Exploded ctime) {
  char new_path[MAX_PATH];
  memset(new_path, 0, sizeof(new_path));
  sprintf(new_path, "%s.%4d-%02d-%02d-%02d-%02d-%02d-%03d", g_log_file_name->c_str(), 
          ctime.year, ctime.month, ctime.day_of_month, ctime.hour, ctime.minute, ctime.second, ctime.millisecond);
  rename(g_log_file_name->c_str(), new_path);
}

int CheckLogFileSize(uint file_size_limit, uint append_msg_size) {
  if (g_log_file) {
    CloseFile(g_log_file);
    g_log_file = nullptr;
  }
  ...
  base::FilePath log_path(g_log_file_name->c_str());
  if (base::GetFileInfo(log_path, &file_info)) {
    if (file_info.size + append_msg_size > file_size_limit) {
      base::Time::Exploded exploded;
      file_info.creation_time.LocalExplode(&exploded);
      RenameLogFile(exploded);
      return FILE_SIZE_EXCEED_LIMIT;
    }
  }

  return FILE_SIZE_NOT_EXCEED_LIMIT;
}

void CleanExpiredLogFiles(uint files_count) {
  PathString log_file_name = GetDefaultLogFile();
  base::FilePath path(log_file_name.c_str());

  DIR *fc_dir = opendir(path.DirName().value().c_str());

  std::vector<std::string> files;
  struct dirent *result;
  const char *pattern = "org.deepin.browser.log.";
  int pattern_len = strlen(pattern);
  while ((result = readdir(fc_dir))) {
    if (strncmp(result->d_name, pattern, pattern_len) != 0) {
      continue;
    }
    if (files.empty()) {
      files.push_back(result->d_name);
    } else {
      if (std::string(result->d_name).compare(files.back().c_str()) < 0) {
        files.push_back(result->d_name);
      } else {
        for (uint i = 0; i < files.size(); i++) {
          if (std::string(result->d_name).compare(files.at(i).c_str()) > 0) {
            files.insert(files.begin() + i, result->d_name);
            break;
          }
        }
      }
  ...
  for (uint i = files.size() - 1; i >= files_count - 1; i--) {
    DeleteFile(path.DirName().Append(files.at(i)), false);
  }
}
...
bool BaseInitLoggingImpl(const LoggingSettings& settings) {
  ...
  // Disable for apply UOS log file rules
  /*
  if (!g_log_file_name)
...
LogMessage::LogMessage(const char* file, int line, LogSeverity severity)
    : severity_(severity), file_(file), line_(line), file_size_limit_(UOS_LOG_FILE_SIZE), files_count_limit_(UOS_LOG_FILES_COUNT) {
  Init(file, line);
}

LogMessage::LogMessage(const char* file, int line, const char* condition)
    : severity_(LOG_FATAL), file_(file), line_(line), file_size_limit_(UOS_LOG_FILE_SIZE), files_count_limit_(UOS_LOG_FILES_COUNT) {
  Init(file, line);
...
#undef ASL_LEVEL_STR_X
  ...
  if ((g_logging_destination & LOG_TO_FILE) != 0) {
    ...
    LoggingLock logging_lock;
#endif
    if (CheckLogFileSize(file_size_limit_, str_newline.size()) == FILE_SIZE_EXCEED_LIMIT) {
      CleanExpiredLogFiles(files_count_limit_);
    }
...
void LogMessage::SetFileSize(int size)
{
  file_size_limit_ = size;
}

void LogMessage::SetFilesCount(int count)
{
  files_count_limit_ = count;
}
...
void LogMessage::Init(const char* file, int line) {
  ...
  // Diable for apply UOS log format
  /*
...
#error Unsupported platform
  ...
  */
    ...
    localtime_r(&t, &local_time);
    struct tm* tm_time = &local_time;
    stream_ << std::setw(4) << tm_time->tm_year + 1900
            << '-'
            << std::setfill('0') << std::setw(2) << 1 + tm_time->tm_mon
            << '-'
            << std::setw(2) << tm_time->tm_mday
            << ", "
            << std::setw(2) << tm_time->tm_hour
...
#endif
  // Severity level
  stream_ << "[" << std::setfill(' ') << std::left << std::setw(7) << log_severity_name(severity_) << "] ";

  // File name and line number
  stream_ << "[" << std::setw(20) << filename.data() << " " << line << "] ";
