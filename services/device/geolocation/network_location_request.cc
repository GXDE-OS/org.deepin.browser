...
//add by xiepinze, add baidu map location service, 20201225, start
bool baidu_map_location_enabled = true;
const char kBaiduMapLocationUrl[] = "https://api.map.baidu.com/location/ip";
const char kBaiduApiKey[] = "18ljLK18g1dfF6fSoyZRWWYq0MHswMqV";
const char kCoorType[] = "bd09ll";
//add by xiepinze, add baidu map location service, 20201225, end
...
//add by xiepinze, add baidu map location service, 20201225, start
//Returns a URL for a request to the baidu maps geolocation API.
GURL FormBaiduMapLocationRequestURL();

//Parses the baidu map location server response body. Returns true if parsing was successful.
bool ParseBaiduMapLocationServerResponse(const std::string& response_body,
                         const base::Time& wifi_timestamp,
                         mojom::Geoposition* position);
//add by xiepinze, add baidu map location service, 20201225, end
...
bool NetworkLocationRequest::MakeRequest(
  ...
  //modify by xiepinze, add baidu map location service, 20201225, start
  //resource_request->url = FormRequestURL(api_key_);
  if (baidu_map_location_enabled)
    resource_request->url = FormBaiduMapLocationRequestURL();
  else
    resource_request->url = FormRequestURL(api_key_);
  //modify by xiepinze, add baidu map location service, 20201225, end
...
//modify by xiepinze, add baidu map location service, 20201225, start
  ...
  //}
  if (baidu_map_location_enabled) {
    if (!ParseBaiduMapLocationServerResponse(*response_body, wifi_timestamp, position)) {
      // We failed to parse the repsonse.
      FormatPositionError(server_url, "Baidu map location service Response was malformed", position);
      RecordUmaEvent(NETWORK_LOCATION_REQUEST_EVENT_RESPONSE_MALFORMED);
      return;
      }
  } else {
...
//add by xiepinze, add baidu map location service, 20201225, start
GURL FormBaiduMapLocationRequestURL() {
  std::string url(kBaiduMapLocationUrl);
  url.append("?ak=");
  url.append(kBaiduApiKey);
  url.append("&coor=");
  url.append(kCoorType);
  return GURL(url);
}
...
bool ParseBaiduMapLocationServerResponse(const std::string& response_body,
  ...
  std::unique_ptr<base::Value> response_value =
      base::JSONReader::ReadAndReturnErrorDeprecated(
          response_body, base::JSON_PARSE_RFC, NULL, &error_msg);
  if (response_value == NULL) {
    LOG(ERROR) << "ParseServerResponse() : JSONReader failed : " << error_msg;
    return false;
  }

  if (!response_value->is_dict()) {
    LOG(ERROR) << "ParseServerResponse() : Unexpected response type "
            << response_value->type();
    return false;
  }
  const base::DictionaryValue* response_object =
  ...
  double latitude;
  double longitude;
  const base::Value* content_value = nullptr;
  response_object->Get("content", &content_value);
  if (content_value) {
    const base::DictionaryValue* contente_object =
      static_cast<base::DictionaryValue*>(const_cast<base::Value*>(content_value));

      const base::Value* point_value = nullptr;
      contente_object->Get("point", &point_value);
      const base::DictionaryValue* point_object =
      static_cast<base::DictionaryValue*>(const_cast<base::Value*>(point_value));

      const base::Value* x_value = nullptr;
      const base::Value* y_value = nullptr;
      point_object->Get("x", &x_value);
      point_object->Get("y", &y_value);
      auto xstr = x_value->GetString();
      auto ystr = y_value->GetString();
      longitude = atof(xstr.c_str());
      latitude = atof(ystr.c_str());
  } else {
    LOG(ERROR) << "content_value is null";
    return false;
  }
