...
bool SyncChannel::Send(Message* message) {
  ...
  base::WaitableEvent* pump_messages_event = sync_msg->pump_messages_event();
#endif
  ChannelProxy::SendInternal(message);
...
#if defined(USE_UNIONTECH_NPAPI)
  WaitForReply(registry.get(), context.get(), pump_messages, pump_messages_event);
#else
...
void SyncChannel::WaitForReply(mojo::SyncHandleRegistry* registry,
  ...
  if (pump_messages) {
    if (pump_messages_event_) {
      pump_messages_event = pump_messages_event_;
    } else {
  ...
  while (true) {
    ...
    //@wangjun: if incoming pump_messages_event is not null, should pump message directly, 2021/04/29
    bool should_pump_messages = pump_messages_event_ != nullptr ? true : false;
    ...
    if (!pump_messages_event_ && pump_messages_event) {
