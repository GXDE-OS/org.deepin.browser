...
//add by xiepinze, set drag image background, 20201126, start
//set drag image background
void SetDragImageBackground(const GURL& url,
                            const base::string16& title,
                            const gfx::ImageSkia& icon,
                            const float radius,
                            const gfx::Point* press_pt,
  ...
  views::LabelButton button(
      nullptr, title.empty() ? base::UTF8ToUTF16(url.spec()) : title);
  button.SetTextSubpixelRenderingEnabled(true);
  const ui::NativeTheme* theme = widget.GetNativeTheme();
  button.SetTextColor(
      views::Button::STATE_NORMAL,
      theme->GetSystemColor(ui::NativeTheme::kColorId_TextfieldDefaultColor));
  ...
  if (widget.IsTranslucentWindowOpacitySupported()) {
    button.SetTextShadows(gfx::ShadowValues(
        0, gfx::ShadowValue(gfx::Vector2d(0, 0), 0.0f, bg_color)));
    button.SetBackground(views::CreateRoundedRectBackground(SkColorSetRGB(123, 188, 251), radius));
  } else {
    button.SetBackground(views::CreateRoundedRectBackground(SkColorSetRGB(123, 188, 251), radius));
    button.SetBorder(button.CreateDefaultBorder());
  }
