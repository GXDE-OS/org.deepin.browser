...
Widget::~Widget() {
#ifdef USE_UNIONTECH_NPAPI
  if(npapi_plugin_mask_) {
    npapi_plugin_mask_->DeleteWidgetObserver(this);
    npapi_plugin_mask_.reset();
  }
#endif
  DestroyRootView();
...
#ifdef USE_UNIONTECH_NPAPI
  npapi_plugin_mask_ = std::make_unique<NpapiPluginMask>();
  for(auto* observer: params.npapi_plugin_mask_observers) {
    npapi_plugin_mask_->AddObserver(observer);
    observer->AddWidgetObserver(this);
  }
#endif
...
#ifdef USE_UNIONTECH_NPAPI
  gfx::Rect content_rect = GetNativeWindow()->GetBoundsInRootWindow();
  content_rect = gfx::ScaleToRoundedRect(content_rect, gtk::GtkUi::GetRawDeviceScaleFactor());
  if(!visible)
      npapi_plugin_mask_->HideRect(this, content_rect);
  else
      npapi_plugin_mask_->ShowRect(this, content_rect);
#endif
}
...
void Widget::OnNativeWidgetMove() {
  ...
  gfx::Rect content_rect = GetNativeWindow()->GetBoundsInRootWindow();
  content_rect = gfx::ScaleToRoundedRect(content_rect, gtk::GtkUi::GetRawDeviceScaleFactor());
  npapi_plugin_mask_->MoveRect(this, content_rect);
#endif
}
