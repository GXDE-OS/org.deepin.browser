...
//wangfeng, add, Send a move cancel message when a menu pops up(bug: 45155), 2020/10/17 --start
#if defined(USE_X11)
  if (!host_)
    return;
  gfx::Point location_in_dip = event->location();
  int flags = event->flags();
  host_->GetRootTransform().TransformPointReverse(&location_in_dip);
  auto hit_test_code = content_window()->delegate()->GetNonClientComponent(location_in_dip);
  if (hit_test_code != HTCLIENT && hit_test_code != HTNOWHERE)
    flags |= ui::EF_IS_NON_CLIENT;
  event->set_flags(flags);

  if (hit_test_code == HTCAPTION) {
    if (event->type() == ui::ET_GESTURE_LONG_PRESS || event->type() == ui::ET_GESTURE_LONG_TAP ||
         event->type() == ui::ET_GESTURE_TWO_FINGER_TAP) {
      auto bounds_in_px = host_->GetBoundsInPixels();
      auto screen_point_in_px = event->location();
      screen_point_in_px.Offset(bounds_in_px.x(), bounds_in_px.y());

      ui::X11Window *xwindow = ui::X11WindowManager::GetInstance()->GetWindow(host_->GetAcceleratedWidget());
      ui::PlatformWindow * platform_window = static_cast<ui::PlatformWindow *>(xwindow);
      if (!platform_window)
        return;
      ui::GetWmMoveResizeHandler(*platform_window)->DispatchHostWindowDragMovement(HTMOVECANCEL, screen_point_in_px);
    }
  }
#endif
//wangfeng, add, Send a move cancel message when a menu pops up(bug: 45155), 2020/10/17 --end
