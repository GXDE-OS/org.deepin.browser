...
//wangfeng: add touch event handler, 2020/08/11 --start
void WindowEventFilterLinux::HandleTouchEventWithHitTest(
    int hit_test,
    ui::TouchEvent* event) {
  if (event->type() != ui::ET_TOUCH_MOVED) //wangfeng: modify, implement move bt ET_TOUCH_MOVED(bug: 45155), 2020/10/17
    return;

  if (hit_test == HTCAPTION) {
    MaybeDispatchHostWindowDragMovement(HTCAPTION, event);
  }
} //wangfeng: add touch event handler, 2020/08/11 --end
...
void WindowEventFilterLinux::OnClickedCaption(ui::MouseEvent* event,
  ...
  switch (action) {
    ...
    case LinuxUI::WindowFrameAction::kMenu:
      ...
      //add by wangfeng, bug(49111) 2020/11/26 --start
      // Controller requires locations to be in DIP, while |this| receives the
      // location in px.
      desktop_window_tree_host_->GetRootTransform().TransformPointReverse(&location);
      //add by wangfeng, bug(49111) 2020/11/26 --end
...
//wangfeng: add touch event dispatch, 2020/08/11 --start
void WindowEventFilterLinux::MaybeDispatchHostWindowDragMovement(
    int hittest,
    ui::TouchEvent* event) {
  // Some platforms (eg X11) may require last pointer location not in the
  ...
  event->StopPropagation();
  return;
} //wangfeng: add touch event dispatch, 2020/08/11 --end
