...
void DesktopWindowTreeHostLinux::DispatchEvent(ui::Event* event) {
  ...
  //wangfeng: add touch event handler(drag the window), 2020/08/11 --start
  if (event->IsTouchEvent()) {
    ui::TouchEvent* touch_event = event->AsTouchEvent();
    if (GetContentWindow() && GetContentWindow()->delegate()) {
      int flags = touch_event->flags();
      gfx::Point location_in_dip = touch_event->location();
      GetRootTransform().TransformPointReverse(&location_in_dip);
      hit_test_code = GetContentWindow()->delegate()->GetNonClientComponent(
          location_in_dip);
      if (hit_test_code != HTCLIENT && hit_test_code != HTNOWHERE)
        flags |= ui::EF_IS_NON_CLIENT;
      touch_event->set_flags(flags);
    }
  }

  if (event->IsTouchEvent() && non_client_window_event_filter_) {
    non_client_window_event_filter_->HandleTouchEventWithHitTest(
        hit_test_code, event->AsTouchEvent());
  }
  //wangfeng: add touch event handler(drag the window), 2020/08/11 --end
