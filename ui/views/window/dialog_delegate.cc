...
//static 
Widget* DialogDelegate::CreateDialogWidgetForTaskManagerWayland(WidgetDelegate* delegate,
                                    gfx::NativeWindow context,
                                    gfx::NativeView parent,
                                    std::string& wm_class,
                                    std::string& wm_name) {
  ...
  views::Widget::InitParams params =
      GetDialogWidgetInitParams(delegate, context, parent, gfx::Rect());
  params.wm_class_class = wm_class;
  params.wm_class_name = wm_name;
  widget->Init(std::move(params));
  return widget;
}
...
View* DialogDelegate::GetInitiallyFocusedView() {
  ...
  if (default_button == ui::DIALOG_BUTTON_NONE)
    //Fix bug#81395. @xudongxu 2021/05/26 --start
    // return nullptr;
    return dcv->ok_button();
    //Fix bug#81395. @xudongxu 2021/05/26 --end
...
NonClientFrameView* DialogDelegate::CreateDialogFrameView(Widget* widget) {
  ...
  //modify by xiaohuyang, Set the margins of 'BubbleFrameView' to solve the problem of rounded corners at the bottom of the certificate dialog, 2020/11/05  --start
#if 0
  BubbleFrameView* frame = new BubbleFrameView(
      provider->GetInsetsMetric(INSETS_DIALOG_TITLE), gfx::Insets());
#else
  BubbleFrameView* frame = new BubbleFrameView(
      provider->GetInsetsMetric(INSETS_DIALOG_TITLE), gfx::Insets(0, 0, 15, 0));
  // base::debug::StackTrace().Print();
#endif
  //modify by xiaohuyang, Set the margins of 'BubbleFrameView' to solve the problem of rounded corners at the bottom of the certificate dialog, 2020/11/05  --end
