...
//modify by xiaohuyang, Set ink drop ripple of ‘FrameCaptionButton’, 2020/11/03  --start
#if 0
...
#endif
//modify by xiaohuyang, Set ink drop ripple of ‘FrameCaptionButton’, 2020/11/03  --end
...
void FrameCaptionButton::PaintButtonContents(gfx::Canvas* canvas) {
  ...
  if (highlight_alpha != SK_AlphaTRANSPARENT) {
    ...
    //modify by xiaohuyang, Draw 'FrameCaptionButton' as a rectangular button with a width and height value of 50px, 2020/09/19  --start
#if 0
    canvas->DrawCircle(center, ink_drop_corner_radius_, flags);
#else
    canvas->DrawRoundRect(GetMirroredRect(GetContentsBounds()), 0, flags);
#endif
    //modify by xiaohuyang, Draw 'FrameCaptionButton' as a rectangular button with a width and height value of 50px, 2020/09/19  --end
  }
