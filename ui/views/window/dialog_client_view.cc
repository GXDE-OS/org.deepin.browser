...
void DialogClientView::OnThemeChanged() {
  ...
  // modify by baohaifeng, 修改主题切换时的弹框按钮的文案颜色 2021/03/25, start
  const bool use_dark_theme = dbus::uos::DBusThreadManager::Get()->GetDdeAppearanceThemeClient()->isUseDarkColor();
  ...
  if (cancel_button_)
    cancel_button_->SetEnabledTextColors(text_color);
  // modify by baohaifeng, 修改主题切换时的弹框按钮的文案颜色 2021/03/25, end
}
...
void DialogClientView::SetupLayout() {
  ...
  [[maybe_unused]] const int button_spacing = (ok_button_ && cancel_button_)
                                 ? layout_provider->GetDistanceMetric(
                                       DISTANCE_RELATED_BUTTON_HORIZONTAL)
                                 : 0;
  //modify by xiaohuyang, Modify the style of the browser dialog,  2020/09/27 --start
  //Set button size, layout, spacing
  int first_view_spacing = 0;
  int second_view_spacing = 0;
  if(extra_view_ && cancel_button_ && ok_button_){
    first_view_spacing = 10;
    second_view_spacing = 10;
  } else if(!extra_view_ && cancel_button_ && ok_button_){
    first_view_spacing = 0;
    second_view_spacing = 20;
  } else if(extra_view_ && !cancel_button_ && ok_button_){
    first_view_spacing = 20;
    second_view_spacing = 0;
  } else if((extra_view_ || ok_button_) && !cancel_button_ ){
    first_view_spacing = 0;
    second_view_spacing = 0;
  }
  ...
  column_set->AddPaddingColumn(kStretchy, first_view_spacing);
  column_set->AddColumn(GridLayout::FILL, GridLayout::FILL, kFixed,
                        GridLayout::USE_PREF, 0, 0);
  // column_set->AddPaddingColumn(kFixed, button_spacing);
  column_set->AddPaddingColumn(kFixed, second_view_spacing);
  column_set->AddColumn(GridLayout::FILL, GridLayout::FILL, kFixed,
                        GridLayout::USE_PREF, 0, 0);
...
#else
  //The size of the dialog box can be determined, so the button size can be set to a fixed type
  int valid_view_count = kNumButtons - std::count(views.begin(), views.end(), nullptr);
  for (size_t view_index = 0; view_index < kNumButtons; ++view_index) {
    if (views[view_index]) {
      if(valid_view_count == 1){
        views[view_index]->SetPreferredSize(gfx::Size(353, 36));
      } else if(valid_view_count == 2){
        views[view_index]->SetPreferredSize(gfx::Size(166, 36));
      } else if(valid_view_count == 3){
        views[0]->SetPreferredSize(gfx::Size(88, 36));
        views[1]->SetPreferredSize(gfx::Size(88, 36));
        views[2]->SetPreferredSize(gfx::Size(157, 36));
      }
      layout->AddExistingView(views[view_index]);
      if (should_link(views[view_index]))
        columns_to_link.push_back(kViewToColumnIndex[view_index]);
  ...
  layout->AddPaddingRow(kFixed, 10);
  //modify by xiaohuyang, Modify the style of the browser dialog,  2020/09/27 --end
