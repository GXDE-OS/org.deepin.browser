...
//modify by xiaohuyang, Set the width of the message prompt dialog,  2020/09/27 --start
// constexpr int kDefaultMessageWidth = 400;
constexpr int kDefaultMessageWidth = 385;
//modify by xiaohuyang, Set the width of the message prompt dialog,  2020/09/27 --end
...
void MessageBoxView::Init(const InitParams& params) {
  ...
  // add by hanll,新增标题一行, 2020/11/02, start
  //添加标题
  if(!params.dialog_title.empty()){
    auto titlelable = std::make_unique<Label>(params.dialog_title,CONTEXT_BODY_TEXT_LARGE,views::style::STYLE_PRIMARY);
    titlelable->SetMultiLine(false);
    titlelable->SetAllowCharacterBreak(true);
    titlelable->SetHorizontalAlignment(gfx::ALIGN_CENTER);
    auto headlabel = message_contents->AddChildView(std::move(titlelable));
    headlabel->SetFontList(gfx::FontList().DeriveWithWeight(gfx::Font::Weight::BOLD));
    message_labels_.push_back(headlabel);
  }
  // add by hanll,新增标题一行, 2020/11/02, end
  ...
  //modify by xiepinze, js dialog modify, 20201027, start
  if (params.options & DETECT_DIRECTIONALITY) {
    ...
    for (const auto& text : texts) {
      ...
      add_label(text, !text.empty(), gfx::ALIGN_CENTER);
    }
  } else {
    //add_label(params.message, true, gfx::ALIGN_LEFT);
    add_label(params.message, true, gfx::ALIGN_CENTER);
  }
  //modify by xiepinze, js dialog modify, 20201027, end
...
void MessageBoxView::ResetLayoutManager() {
  ...
  if (prompt_field_) {
    ...
    // modify by hanll,控件居中,2020/11/02, start
    prompt_field_->SetHorizontalAlignment(gfx::ALIGN_CENTER);  
    // modify by hanll,控件居中,2020/11/02, end
  }
  ...
  if (checkbox_) {
    ...
    // modify by hanll,控件居中,2020/11/02, start
    checkbox_->SetHorizontalAlignment(gfx::ALIGN_CENTER);
    // modify by hanll,控件居中,2020/11/02, end
  }
