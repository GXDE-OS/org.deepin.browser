...
gfx::ImageSkia Checkbox::GetImage(ButtonState for_state) const {
  int icon_state = 0;
  //modify by xiaohuyang, Set the style of checked state with texture as required by UI designer. 2021/3/11
#if UNUSED
  if (GetChecked())
    icon_state |= IconState::CHECKED;
#else
  if (GetChecked()) {
    return *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_CHECKBOX_CHECKED_NORMAL));
  }
#endif
