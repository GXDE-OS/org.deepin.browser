...
//Add by xiaohuyang, Only used to set the button style of the find bar view.
void ConfigureVectorImageButtonFindbarView(ImageButton* button) {
  button->SetInkDropMode(Button::InkDropMode::ON);
  button->set_has_ink_drop_action_on_click(true);
  button->SetImageHorizontalAlignment(ImageButton::ALIGN_CENTER);
  button->SetImageVerticalAlignment(ImageButton::ALIGN_MIDDLE);
  button->SetBorder(CreateEmptyBorder(gfx::Insets(8,13)));
}
...
void SetImageFromVectorIconUOS(ImageButton* button,
                            const gfx::VectorIcon& icon,
                            int dip_size,
                            SkColor related_text_color) {
  const SkColor icon_color = (related_text_color == SK_ColorWHITE) ? SK_ColorWHITE 
      : color_utils::DeriveDefaultIconColor(related_text_color);
  SetImageFromVectorIconWithColor(button, icon, dip_size, icon_color);
}
