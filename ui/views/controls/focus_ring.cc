...
double GetCornerRadius() {
  // modify by xiaohuyang, Set a round rect background for locationbar when it state is focused, 2020/08/07 --start
  #if 0
  ...
  return 8;
  #endif
  // modify by xiaohuyang, Set a round rect background for locationbar when it state is focused,, 2020/08/07 --end
}
...
void FocusRing::SetLocationBarBorderColor(const SkColor color){
  if(color == locationbar_border_color_){
    return ;
  }

  locationbar_border_color_ = color;
  SchedulePaint();
}
...
#else
  paint.setColor(locationbar_border_color_);
#endif
