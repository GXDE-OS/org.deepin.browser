...
class Arrow : public Button {
 ...
 private:
  void PaintButtonContents(gfx::Canvas* canvas) override {
    ...
    //modify by xiaohuyang, Set the style of the drop-down button of ‘EditableCombobox’, 2021/3/6  --start
#if UNUSED
    PaintComboboxArrow(color_, arrow_bounds, canvas);
#else
    SkColor color = color_;
    gfx::ImageSkia icon = *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_COMBOBOX_ARROW));
    // No password saving function in privacy mode.
    if(GetNativeTheme()->ShouldUseDarkColors()) {
      color = SK_ColorWHITE;
      icon = *(ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_COMBOBOX_ARROW_DARK)); 
    }
    PaintComboboxArrow(color, arrow_bounds, canvas, icon);
#endif
    //modify by xiaohuyang, Set the style of the drop-down button of ‘EditableCombobox’, 2021/3/6  --end
  }
