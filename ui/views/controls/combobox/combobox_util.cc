...
//modify by xiaohuyang, Set the style of the drop-down button of ‘combobox’, 2021/3/6
#if 1
void PaintComboboxArrow(SkColor color,
                        const gfx::Rect& bounds,
                        gfx::Canvas* canvas,
  ...
  float dsf = canvas->UndoDeviceScaleFactor();
  SkScalar x = std::ceil(bounds.x() * dsf) + 2;
  SkScalar y = std::ceil(bounds.y() * dsf);
  constexpr SkScalar kEpsilon = 0.0001f;
  ...
  canvas->DrawImageInt(icon, x - kEpsilon, y, flags);
}
#endif
