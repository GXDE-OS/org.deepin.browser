...
void TreeView::PaintRow(gfx::Canvas* canvas,
...
#else
  if (!model_->GetChildren(node->model_node()).empty())
    PaintExpandControlUOS(canvas, bounds, node);
#endif
  ...
  if (!PlatformStyle::kTreeViewSelectionPaintsEntireRow &&
    ...
    // modify by xiaohuyang, Paint a round rect background for the selected row（No aliasing）, 2021/1/28 --start
#if UNUSED
    canvas->FillRect(text_bounds, selected_row_bg_color);
#else
    cc::PaintFlags flags;
    ...
    flags.setColor(selected_row_bg_color);
    canvas->DrawRoundRect(GetBoundsForNode(node), 8, flags);
#endif
    // modify by xiaohuyang, Paint a round rect background for the selected row（No aliasing）, 2021/1/28 --end
  }
  ...
  // modify by xiaohuyang, Paint arrow icon. 2021/2/5
#if 1 
  if (!model_->GetChildren(node->model_node()).empty())
    PaintExpandControlUOS(canvas, bounds, node);
  if (drawing_provider()->ShouldDrawIconForNode(this, node->model_node()))
...
// modify by xiaohuyang, Paint arrow icon. 2021/2/5
#if 1
void TreeView::PaintExpandControlUOS(gfx::Canvas* canvas,
                          const gfx::Rect& node_bounds,
                          InternalNode* node) {
  gfx::ImageSkia arrow = gfx::CreateVectorIcon(
      kSubmenuArrowIcon,
      (node == selected_node_ || GetNativeTheme()->ShouldUseDarkColors()) 
