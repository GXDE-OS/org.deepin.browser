...
gfx::Rect ScrollBarViews::GetTrackBounds() const {
  //modify by xiaohuyang, Handle the scroll bar issue. 2021/3/25  --start
  //Set the size of the arrow button to 0, so that the scroll bar track can be displayed completely, 
  //and there is no gap between the top and bottom or left and right positions.
#if UNUSED
  ...
  if (IsHorizontal()) {
    ...
    bounds.set_width(std::max(0, bounds.width()));
    ...
    bounds.set_height(std::max(0, bounds.height()));
