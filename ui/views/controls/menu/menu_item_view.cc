...
void MenuItemView::SetTitle(const base::string16& title) {
  // add by hanll, 去掉快捷键后的文本, 2020/08/27, start
  base::string16 tmptitle = title;
  int len = title.length();

  std::size_t i = tmptitle.find('&', 0);
  if (std::size_t(-1) != i) {
    int j =i-1;
    //英文系统下，没有左括号就去字符
    if((j>=0)&&(j<len)&&(tmptitle.at(j) == '('))
    {
      //LOG(INFO)<<"-----tmptitle-before----"<<tmptitle;
      tmptitle.erase(j); 
      //LOG(INFO)<<"-----tmptitle-after----"<<tmptitle;
    }
  }
 
  tmptitle_ = tmptitle;
  // add by hanll, 去掉快捷键后的文本, 2020/08/27, end
  title_ = title;
...
//modify by xiepinze, submenu arrow, 2020/08/27, start
  // Set the submenu indicator (arrow) image and color.
  //if (HasSubmenu())
  //  submenu_arrow_image_view_->SetImage(GetSubmenuArrowImage(icon_color));
  if (HasSubmenu()) {
    //modify by xiaohuyang, Set the color of submenu arrow, 2021/3/4
    icon_color = (IsSelected() || GetNativeTheme()->ShouldUseDarkColors()) ? SkColorSetARGB(0xFF, 0xFF, 0xFF, 0xFF) : SkColorSetARGB(0xFF, 0x00, 0x00, 0x00);
    submenu_arrow_image_view_->SetImage(GetSubmenuArrowImage(icon_color));
  }
//modify by xiepinze, submenu arrow, 2020/08/27, end
}
...
//delete by xiepinze, don't show minor text, 2020/08/04, start
  //base::string16 accel_text;
  //if (MenuConfig::instance().ShouldShowAcceleratorText(this, &accel_text))
  //  return accel_text;
//delete by xiepinze, don't show minor text, 2020/08/04, end
  return minor_text_;
}
