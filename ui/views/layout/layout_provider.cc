...
gfx::Insets LayoutProvider::GetInsetsMetric(int metric) const {
  ...
  switch (metric) {
    ...
    case InsetsMetric::INSETS_DIALOG_TITLE: {
      const gfx::Insets dialog_insets = GetInsetsMetric(INSETS_DIALOG);
      // modify by xiaohuyang, Set the inset of the dialog title,  2020/11/2  --start
#if 0
      return gfx::Insets(dialog_insets.top(), dialog_insets.left(), 0,
                         dialog_insets.right());
#else
      return gfx::Insets(10, 10, 0,
                         dialog_insets.right());
#endif
      // modify by xiaohuyang, Set the inset of the dialog title,  2020/11/2  --end
    }
