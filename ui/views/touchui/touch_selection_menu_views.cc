...
LabelButton* TouchSelectionMenuViews::CreateButton(const base::string16& title,
  ...
  //wangfeng, add, erase shortcut tips. 2020/09/03 --start
  base::string16::size_type begin, end;
  if ((begin = label.find('(')) != base::string16::npos && 
      (end = label.find(')')) != base::string16::npos)
    label.erase(begin, end);
  //wangfeng, add, erase shortcut tips. 2020/09/03 --end
