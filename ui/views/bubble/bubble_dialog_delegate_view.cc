...
Widget* CreateBubbleWidget(BubbleDialogDelegateView* bubble,
                           std::set<views::NpapiPluginMaskObserver*> npapi_mask_observer) {
#else
Widget* CreateBubbleWidget(BubbleDialogDelegateView* bubble) {
#endif
...
#if defined(USE_UNIONTECH_NPAPI)
  for (auto* observer_ : npapi_mask_observer) {
    if (observer_) {
      observer_->AddWidgetObserver(bubble_widget);
      bubble_params.npapi_plugin_mask_observers.push_back(observer_);
    }
  }
#endif
...
#if defined(USE_UNIONTECH_NPAPI)
  Widget* bubble_widget = CreateBubbleWidget(bubble_delegate, npapi_mask_observer);
#else
