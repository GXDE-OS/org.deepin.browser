...
std::unique_ptr<Button> BubbleFrameView::CreateCloseButton(
  ...
  // modify by hanll, 修改弹框叉号样式, 2020/10/21, start
#if 0
...
#else
  auto close_button = std::make_unique<views::ImageButton>(listener);
  ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();

  // modify by xiaohuyang, Set the close button style of the dialog under different themes.  2021/1/20
  const bool use_dark_theme = 
      dbus::uos::DBusThreadManager::Get()->GetDdeAppearanceThemeClient()->isUseDarkColor();
  if (use_dark_theme) {
      close_button->SetImage(
        views::Button::STATE_NORMAL,
        rb.GetNativeImageNamed(IDR_CUSCLOSE_N_DARK).ToImageSkia());
      close_button->SetImage(
        views::Button::STATE_HOVERED,
        rb.GetNativeImageNamed(IDR_CUSCLOSE_H_DARK).ToImageSkia());
      close_button->SetImage(
        views::Button::STATE_PRESSED,
        rb.GetNativeImageNamed(IDR_CUSCLOSE_P_DARK).ToImageSkia());
  } else {
      close_button->SetImage(
        views::Button::STATE_NORMAL,
        rb.GetNativeImageNamed(IDR_CUSCLOSE_N).ToImageSkia());
      close_button->SetImage(
        views::Button::STATE_HOVERED,
        rb.GetNativeImageNamed(IDR_CUSCLOSE_H).ToImageSkia());
      close_button->SetImage(
        views::Button::STATE_PRESSED,
        rb.GetNativeImageNamed(IDR_CUSCLOSE_P).ToImageSkia());
  }
  close_button->SetTooltipText(l10n_util::GetStringUTF16(IDS_APP_CLOSE));
  close_button->SizeToPreferredSize();
  close_button->SetFocusForPlatform();
#endif
  // modify by hanll, 修改弹框叉号样式, 2020/10/21, end
  return close_button;
}
...
void BubbleFrameView::OnThemeChanged() {
  ...
  // modify by baohaifeng, 修改关闭按钮文案颜色 2021/03/26, start
  ImageButton * image_close = (ImageButton*)close_;
  if (image_close == nullptr)
        return;

  ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();
  const bool use_dark_theme = dbus::uos::DBusThreadManager::Get()->GetDdeAppearanceThemeClient()->isUseDarkColor();
  if (use_dark_theme) {
    image_close->SetImage(
      views::Button::STATE_NORMAL,
      rb.GetNativeImageNamed(IDR_CUSCLOSE_N_DARK).ToImageSkia());
    image_close->SetImage(
      views::Button::STATE_HOVERED,
   ...
   // modify by baohaifeng, 修改关闭按钮文案颜色 2021/03/26, end
