...
//modify by xiaohuyang, Set the hover state color for the page info window button, 2020/07/31  --start
//It will affect the color of some buttons in the hover state and needs improvement
...
std::unique_ptr<InkDropHighlight> InkDropHostView::CreateInkDropHighlight()
  ...
  //     GetInkDropBaseColor());
  auto highlight = std::make_unique<views::InkDropHighlight>(
      size(), 8, gfx::RectF(GetMirroredRect(GetLocalBounds())).CenterPoint(),
      GetInkDropBaseColor());
...
//modify by xiaohuyang, Set the hover state color for the page info window button, 2020/07/31  --end
