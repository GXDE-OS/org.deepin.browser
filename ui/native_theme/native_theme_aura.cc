...
void NativeThemeAura::PaintMenuPopupBackground(
  ...
  if (menu_background.corner_radius > 0) {
    ...
    // modify by xiaohuyang, set popup menu opacity. 2021/1/20  --start
#if UNUSED
    flags.setColor(color);
#else
    flags.setColor(SkColorSetARGB(245, SkColorGetR(color), SkColorGetG(color), SkColorGetB(color)));
#endif
    // modify by xiaohuyang, set popup menu opacity. 2021/1/20  --end
...
void NativeThemeAura::PaintScrollbarTrack(
  ...
  //modify by xiaohuyang, Handle the scroll bar issue. 2021/3/25  --start
  //Set the color of the track to suit different themes.
#if UNUSED
  flags.setColor(kTrackColor);
#else
  SkColor track_color;
  if(ShouldUseDarkColors()){
    track_color = SkColorSetRGB(0x29, 0x2A, 0x2D);
  } else {
    track_color = gfx::kGoogleGrey100;
  }
  flags.setColor(track_color);
#endif
  //modify by xiaohuyang, Handle the scroll bar issue. 2021/3/25  --end
  canvas->drawIRect(gfx::RectToSkIRect(rect), flags);
}
...
void NativeThemeAura::PaintScrollbarThumb(cc::PaintCanvas* canvas,
  ...
  if (use_overlay_scrollbars_) {
    ...
    //modify by xiaohuyang, Handle the scroll bar issue. 2021/3/25  --start
    //Make the width or height of thumb is equal to the track.
#if UNUSED
    ...
    //modify by xiaohuyang, Handle the scroll bar issue. 2021/3/25  --start
    //Set the color of the thumb and make it equal to the track width or height.
#if UNUSED
...
void NativeThemeAura::PaintScrollbarCorner(cc::PaintCanvas* canvas,
  ...
  DCHECK(!use_overlay_scrollbars_);
  //modify by xiepinze, don't paint a scrollbar corner, 2020/11/03, start
  //cc::PaintFlags flags;
  //flags.setColor(SkColorSetRGB(0xDC, 0xDC, 0xDC));
  //canvas->drawIRect(RectToSkIRect(rect), flags);
  //modify by xiepinze, don't paint a scrollbar corner, 2020/11/03, end
}
