...
using GdkDrawable = struct _GdkDrawable;
...
class COMPONENT_EXPORT(GTK) GtkUiDelegate {
 public:
  ...
  virtual GdkDrawable* GetGdkWindow(gfx::AcceleratedWidget window_id) = 0;

  // Gtk dialog windows must be set transient for the browser window. This
  // function abstracts away such functionality.
  virtual bool SetGdkWindowTransientFor(GdkDrawable* window,
