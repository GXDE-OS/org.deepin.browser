...
#include "ui/gtk2/gtk_ui_delegate_x11.h"
...
#include "ui/gtk2/gtk_event_loop_x11.h"
...
GdkDrawable* GtkUiDelegateX11::GetGdkWindow(gfx::AcceleratedWidget window_id) {
  GdkDisplay* display = GetGdkDisplay();
  GdkDrawable* gdk_window = gdk_x11_window_lookup_for_display(display, window_id);
  if (gdk_window)
...
bool GtkUiDelegateX11::SetGdkWindowTransientFor(GdkDrawable* window,
...
int GtkUiDelegateX11::GetGdkKeyState() {
  XEvent* xevent =
      X11EventSource::GetInstance()->dispatching_event();

  if (!xevent)
    return ui::EF_NONE;
  
  return static_cast<int>(xevent->xkey.state);
}
}  // namespace ui
