...
XWindow::XWindow()
  ...
  has_window_keyboard_grab_ = false;
  has_child_keyboard_grab_ = false;
  uos_focus_window_ = 0;
}
...
#ifdef USE_UNIONTECH_NPAPI_NOUSE
  attribute_mask |= (NoEventMask | CWEventMask);
#endif
...
void XWindow::OnCrossingEvent(bool enter,
  ...
  LOG(INFO) << "[NPAPI] XWindow OnCrossingEvent " << enter << " " << focus_in_window_or_ancestor << " " << mode << " " << detail;
  if (detail == NotifyInferior) {
#ifdef USE_UNIONTECH_NPAPI_NOUSE
    if (has_window_focus_) {
      if (!enter && !has_window_keyboard_grab_ &&
                    !has_child_keyboard_grab_) {
        GrabOrUngrabKeyboardForParentWindow(true);
      } else if (enter && has_window_keyboard_grab_ &&
                         !has_child_keyboard_grab_) {
        GrabOrUngrabKeyboardForParentWindow(false);
      }
    }
#endif
    return;
  ...
  if (focus_in_window_or_ancestor && !has_window_focus_) {
    ...
    //@wangjun: comment this line for fix suwell plugin bug#56352, 2020/12/02
    // has_pointer_focus_ = has_pointer_;
    LOG(WARNING) << "[NPAPI] has_pointer_focus_ value need to be checked";
  }
...
void XWindow::OnFocusEvent(bool focus_in, int mode, int detail) {
  ...
  LOG(INFO) << "[NPAPI] XWindow::OnFocusEvent " << focus_in << " " << mode << " " << detail;
  if (detail == NotifyInferior)
    return;
...
void XWindow::ProcessEvent(XEvent* xev) {
  ...
  switch (xev->type) {
    ...
    case LeaveNotify: {
      ...
      if (xev->xcrossing.subwindow != 0) {
        LOG(INFO) << "[NPAPI] CHECK STATUS " << has_window_focus_ << " " << IsActive()
                  << " " << has_child_keyboard_grab_ << " " << has_window_keyboard_grab_;
        if (xev->type == LeaveNotify && !has_child_keyboard_grab_ && xev->xcrossing.detail != NotifyInferior) {
          LOG(INFO) << "[NPAPI] LeaveNotify LEAVE MAIN WINDOW";
          GrabOrUngrabKeyboardForParentWindow(false);
        }
      }
    #endif
    ...
    case x11::FocusOut:
      ...
      if (xev->xfocus.detail == NotifyNonlinearVirtual) {
        if (xev->type == x11::FocusOut && xev->xfocus.mode == NotifyNormal) {
          LOG(INFO) << "[NPAPI] Clear child keyboard grab in focusevent";
          has_child_keyboard_grab_ = false;
        }
      }
    #endif
    ...
    case GenericEvent: {
      ...
      if (!factory->ShouldProcessXI2Event(xev)) {
      ...
      #ifdef USE_UNIONTECH_NPAPI
        // XI_ButtonRelease will change keyboard grab
        // xienter_event->child == 0 means mouse event produce in xwindow_
        // xienter_event->child != 0 means mouse event produce in childwindow
        XIEvent* xi_event = static_cast<XIEvent*>(xev->xcookie.data);
        XIEnterEvent* xienter_event =
            static_cast<XIEnterEvent*>(xev->xcookie.data);

        // get window name to check whether is plugin container or not
        char* name = 0;
        bool in_plugin_area = false;
        static bool in_plugin_area_press = true;
        XFetchName(xdisplay_, xienter_event->child, &name);
        if (name != nullptr && strcmp(name, "Plugin Container") == 0) {
          in_plugin_area = true;
        }
        XFree(name);
        ...
        if (xi_event->evtype == XI_Motion) {
          if (in_plugin_area) {
            if (!has_child_keyboard_grab_ && has_window_focus_ && !has_window_keyboard_grab_) {
              GrabOrUngrabKeyboardForParentWindow(true);
            }
          } else {
            GrabOrUngrabKeyboardForParentWindow(false);
        ...
        if (xi_event->evtype == XI_ButtonPress) {
          in_plugin_area_press = in_plugin_area;
          LOG(INFO) << "[NPAPI] press start in plugin area " << in_plugin_area_press;
          if (!in_plugin_area_press && has_child_keyboard_grab_ && uos_focus_window_ != xwindow_) {
            OnXWindowIsActiveChanged(true);
          }
        }

        if (xi_event->evtype == XI_ButtonRelease) {
          // this case means mainwindow get focus
          if (!in_plugin_area && has_child_keyboard_grab_ && uos_focus_window_ != xwindow_) {
            LOG(INFO) << "[NPAPI] CLICK IN MAINWINDOW";
            Time timestamp = ui::X11EventSource::GetInstance()->GetTimestamp();
            has_window_keyboard_grab_ = true;
            has_child_keyboard_grab_ = false;
            uos_focus_window_ = xwindow_;
            XSetInputFocus(xdisplay_, xwindow_, RevertToParent, timestamp);
          }

          // this case means plugin container get focus
          if (in_plugin_area && in_plugin_area_press) {
            LOG(INFO) << "[NPAPI] CLICK IN PLUGIN CONTAINER";
            Time timestamp = ui::X11EventSource::GetInstance()->GetTimestamp();
            if (has_window_keyboard_grab_) {
              GrabOrUngrabKeyboardForParentWindow(false);
            }
            OnXWindowIsActiveChanged(false);
            //@wangjun: fix bug#78454, for some special case, don't set input focus, start, 2021/05/08
            Window focus_return;
            int focus_state;
            if (XGetInputFocus(xdisplay_, &focus_return, &focus_state)) {
              LOG(INFO) << "[NPAPI] XGetInputFocus focus_return " << focus_return << " focus_state " << focus_state;
              LOG(INFO) << "[NPAPI] xwindow: " << xwindow_ << " child window " << xienter_event->child;
              if (focus_return == xwindow_ || focus_state == RevertToParent) {
                XSetInputFocus(xdisplay_, xienter_event->child, RevertToParent, timestamp);
              }
            }
            //@wangjun: fix bug#78454, for some special case, don't set input focus, end, 2021/05/08
            has_child_keyboard_grab_ = true;
            has_window_keyboard_grab_ = false;
            uos_focus_window_ = xienter_event->child;
          }
          in_plugin_area_press = true;
        }
      #endif
        break;
      }
    ...
    case UnmapNotify: {
    #ifdef USE_UNIONTECH_NPAPI_NOUSE
      auto it = map_window_display_.find(xev->xunmap.window);
      if(it != map_window_display_.end())
        map_window_display_.erase(it);
      if (xwindow_ != xev->xunmap.window)
        break;
    #endif
  ...
  #ifdef USE_UNIONTECH_NPAPI_NOUSE
    case MapRequest: {
      LOG(INFO) << "[X11_WINDOW] MapRequest window:" << xev->xmaprequest.window;
      LOG(INFO) << "[X11_WINDOW] MapRequest parent:" << xev->xmaprequest.parent;
      XMapWindow(xev->xmaprequest.display, xev->xmaprequest.window);

      child_display_ = xev->xmaprequest.display;
      child_window_ = xev->xmaprequest.window;
      XWindowAttributes win_attributes;
      XGetWindowAttributes(xev->xmaprequest.display, child_window_, &win_attributes);
      if(win_attributes.bit_gravity == ForgetGravity)
        XLowerWindow(child_display_, child_window_);
        
      if(map_window_display_.find(child_window_) == map_window_display_.end()){
          map_window_display_.insert(std::make_pair(child_window_, child_display_));
      }
      break;
    }
  #endif
...
int XWindow::GrabOrUngrabKeyboardForParentWindow(bool grab){
  Time timestamp = ui::X11EventSource::GetInstance()->GetTimestamp();
  int ret = -1;
  if(grab){
    ret = XGrabKeyboard(xdisplay_, xwindow_, TRUE, GrabModeAsync,
    ...
    if (ret == GrabSuccess) {
      ...
      LOG(WARNING) << " XGrabKeyboard ERROR errno = " << ret;
    }
  }else{
    Time timestamp = ui::X11EventSource::GetInstance()->GetTimestamp();
    ret = XUngrabKeyboard(xdisplay_, timestamp);
    has_window_keyboard_grab_ = false;
    uos_focus_window_ = 0;
  }
  return ret;
...
#ifdef USE_UNIONTECH_NPAPI_NOUSE
  LOG(INFO) << "[NPAPI] XWindow::OnConfigureEvent";
  if (!map_window_display_.empty() &&
      map_window_display_.find(xev->xconfigure.window) != map_window_display_.end()){
        return;
  }
  if (xwindow_ !=  xev->xconfigure.window) {
    return;
  }
#endif
