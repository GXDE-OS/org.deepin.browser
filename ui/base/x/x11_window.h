...
class COMPONENT_EXPORT(UI_BASE_X) XWindow {
 ...
 private:
  ...
  // grab parent_window(|xwindow_|) keyboard
  int GrabOrUngrabKeyboardForParentWindow(bool grab);
  ...
  // add by hp for focus problem by plugin
  // Is |xwindow_| grab keyboard
  bool has_window_keyboard_grab_;
  // IS |child window| grab keyboard
  bool has_child_keyboard_grab_;
  // The grab focus window
  ::Window uos_focus_window_;
  ...
  std::map<::Window, ::Display*> map_window_display_;
