...
void X11Window::DispatchUiEvent(ui::Event* event, XEvent* xev) {
  ...
  if (coalesced) {
    ...
    if ((xdg_session_type && strncmp(xdg_session_type, "wayland", 7) == 0)) {
      X11Window* current_mouse_window = window_manager->window_mouse_currently_on();
      if (current_mouse_window && event) {
        ConvertEventLocationToTargetLocation(GetBounds(),
                                             current_mouse_window->GetBounds(),
                                             event->AsLocatedEvent());
      }
    }
  }
