...
void ResourceRequestInfo::AllocateForTesting(
  ...
  // the main frame.
  //DCHECK(resource_type != kMainFrame || is_main_frame);
...
ResourceRequestInfoImpl::GetWebContentsGetterForRequest() const {
  ...
  //return base::Bind(WebContents::FromRenderFrameHostID, render_process_host_id, render_frame_host_id);
  LOG(ERROR) << "[RES][TODO] ResourceRequestInfoImpl::GetWebContentsGetterForRequest()";
  return base::Bind(WebContents::FromFrameTreeNodeId, frame_tree_node_id_);
}
...
network::mojom::ReferrerPolicy ResourceRequestInfoImpl::GetReferrerPolicy() const {
