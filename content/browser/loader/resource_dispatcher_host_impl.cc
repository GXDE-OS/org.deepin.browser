...
#ifdef USE_UNIONTECH_FEATURE_GMSSL
#include "net/base/uniontech_ssl_exports.h"
#include "net/base/ukey_util.h"
...
void OnVPNSelectCompleteOnIO(void* ptr,
                         const std::string& scert,
                         const std::string& signature) {
  DCHECK_CURRENTLY_ON(BrowserThread::IO);

  VPNParams* p = static_cast<VPNParams*>(ptr);
  ResourceDispatcherHostImpl::Get()->KeepVPNLoginGoing(p, scert, signature);
}

void OnVPNSelectCompleteOnUI(void* ptr,
                             const std::string& scert,
                             const std::string& signature) {
  ...
  BrowserThread::PostTask(
      ...
      base::Bind(&OnVPNSelectCompleteOnIO, ptr, scert, signature));
}

void WillCallUIDialogOnUI(
    VPNParams* ptr,
    base::Callback<WebContents*(void)> getter) {

  net::LPUICALLBACK pUIFnPtr = nullptr;
  net::GetVPNDialogUICallBack(pUIFnPtr);

  (*pUIFnPtr)(std::move(getter), static_cast<void*>(ptr));
}

void WillCallUIDialogOnIO(
    VPNParams* ptr,
    base::Callback<WebContents*(void)> getter) {
  ...
  BrowserThread::PostTask(
    BrowserThread::UI,
    FROM_HERE,
    base::Bind(&WillCallUIDialogOnUI, ptr, std::move(getter)));
}
#endif
...
#if defined(USE_UNIONTECH_NPAPI)
  requester_info->GetContexts(static_cast<ResourceType>(request_data.resource_type),
                              request_data.plugin_child_id, &resource_context,
#else
...
void ResourceDispatcherHostImpl::ContinuePendingBeginRequest(
  ...
  if (handler) {
    ...
    DCHECK(0 == new_request->ssl_connect_type());
    new_request->set_ssl_connect_type(SSL_CONNECT_TYPE_SSL31CONNECT);
    // 1. Is A GMSSL Check URL
    // 2.   NONE - Query GMSSL Recorder
    // 3.   NONE - Query SSL31 Recorder
    // 4. Both NONE - Fire Checker
    // 5. Get Checker Result And Refresh
    if (request_data.is_main_frame) {
      if (content::SwitchSSLHost::Get().enabled()) {
        const GURL& url_ = new_request->url();
        if (url_.SchemeIs("https")) {

          // content::SwitchSSLHost::Get().IsGMSSLCheckerURL(&url_);
          // new_request->set_ssl_connect_type(SSL_CONNECT_TYPE_GMSSLCHECKER);
          // content::SwitchSSLHost::Get().IsSSL31CheckerURL(&url_);
          // new_request->set_ssl_connect_type(SSL_CONNECT_TYPE_SSL31CHECKER);

          content::ResourceRequestInfo* info = ResourceRequestInfo::ForRequest(new_request.get());
          if (info) {
            ResourceRequestInfo::WebContentsGetter web_contents_getter = info->GetWebContentsGetterForRequest();

            if (web_contents_getter) {
              std::string host_port_pair;
              if (content::SwitchSSLHost::MakeHostPortPair(&url_, &host_port_pair)) {
                ...
                if (content::SwitchSSLHost::RECORD_SSL31 == up) {
                  new_request->set_ssl_connect_type(SSL_CONNECT_TYPE_SSL31CONNECT);
                } else if (content::SwitchSSLHost::RECORD_GMSSL == up) {
                  new_request->set_ssl_connect_type(SSL_CONNECT_TYPE_GMSSLCONNECT);
                } else {

                  int flags = new_request->load_flags();
                  flags |= net::LOAD_BYPASS_CACHE;
                  flags |= net::LOAD_DISABLE_CACHE;
                  flags |= net::LOAD_DISABLE_CONNECTION_MIGRATION_TO_CELLULAR;
                  new_request->SetLoadFlags(flags);

                  content::SwitchSSLHost::Param p;
                  p.url            = new_request->url().spec();
                  p.host_port_pair = host_port_pair;
                  p.mainframe = request_data.is_main_frame;
                  p.frame_url = true;
                  p.parent_is_main_frame = false; // TODO: CHECK IT!
                  content::SwitchSSLHost::Get().OnWillStartChecker(p, std::move(web_contents_getter));
                }
              }
            }
          }
...
void ResourceDispatcherHostImpl::BeginNavigationRequest(
    ...
    if (content::SwitchSSLHost::Get().enabled()) {
      ...
      if (url_.SchemeIs("https")) {
        ...
        ResourceRequestInfo::WebContentsGetter web_contents_getter =
            ResourceRequestInfo::ForRequest(new_request.get())->GetWebContentsGetterForRequest();
        if (web_contents_getter) {
          std::string host_port_pair;
          if (content::SwitchSSLHost::MakeHostPortPair(&url_, &host_port_pair)) {
            content::SwitchSSLHost::URLProtocol up = content::SwitchSSLHost::RECORD_NONE;
            ...
            if (content::SwitchSSLHost::RECORD_SSL31 == up) {
              ...
              p.url = new_request->url().spec();
              p.host_port_pair = host_port_pair;
              p.mainframe = info.is_main_frame;
              p.frame_url = true;
              p.parent_is_main_frame = info.parent_is_main_frame;
              content::SwitchSSLHost::Get().OnWillStartChecker(p, std::move(web_contents_getter));
            }
          }
        }
...
#if defined(USE_UNIONTECH_FEATURE_GMSSL)
  bool vpn_block_url = false;
  if (SSL_CONNECT_TYPE_GMSSLCONNECT == new_request->ssl_connect_type()) {
    std::string vpn_path = info.common_params.url.path();
    // if (vpn_target.find("https://10.92.2.186:4430/admin/login.php") == 0) {
    if (vpn_path.find("/admin/login_action.php") == 0) {
      std::string vpn_param = info.common_params.url.query();
      if (vpn_param.find("id=0") == 0) {
        if (info.common_params.url.SchemeIs("https")) {
          if (info.common_params.method.compare("POST") == 0) {
            vpn_block_url = true;
          }
        }
      }
  ...
  if (vpn_block_url) {
    ResourceRequestInfo::WebContentsGetter web_contents_getter =
        ResourceRequestInfo::ForRequest(new_request.get())
            ->GetWebContentsGetterForRequest();

    DCHECK_CURRENTLY_ON(BrowserThread::IO);

    net::SetVPNDialogNetCallBack(OnVPNSelectCompleteOnUI);

    VPNParams* ptr = new VPNParams();
    ptr->new_request = std::move(new_request);
    ptr->handler = std::move(handler);
    ptr->is_initiated_by_fetch_api = false;
    ptr->throttling_token = std::move(throttling_token);
    ptr->post_data = info.common_params.post_data;

    WillCallUIDialogOnIO(ptr, std::move(web_contents_getter));
    return;
  }
#endif // #ifdef USE_UNIONTECH_FEATURE_GMSSL
...
#ifdef USE_UNIONTECH_FEATURE_GMSSL
void ResourceDispatcherHostImpl::KeepVPNLoginGoing(
    VPNParams* ptr,
    const std::string& scert,
  ...
  network::ResourceRequestBody* body = ptr->post_data.get();
  if (body) {
    ...
    data += "&sdata=";
    data += signatrue;
    body->AppendBytes(data.data(), data.size());

    ptr->new_request->set_upload(UploadDataStreamBuilder::Build(
        body, nullptr, nullptr,
        base::CreateSingleThreadTaskRunnerWithTraits(
            {base::MayBlock(), base::TaskPriority::USER_VISIBLE})
            .get()));

    BeginRequestInternal(std::move(ptr->new_request), std::move(ptr->handler),
                         ptr->is_initiated_by_fetch_api,
                         std::move(ptr->throttling_token));
  }
}
#endif
