...
void GpuDataManagerImpl::SetVideoDecodeAcceleration(bool video_decode) {
  base::AutoLock auto_lock(lock_);
  private_->SetVideoDecodeAcceleration(video_decode);
}

bool GpuDataManagerImpl::VideoDecodeAccelerationEnabled() {
  base::AutoLock auto_lock(lock_);
  return private_->VideoDecodeAccelerationEnabled();
}
