...
bool IsBlacklistedBySha1sumAndQuirks(const base::FilePath& path) {
  ...
  if (!base::GetFileSize(path, &size))
    return false;
  for (size_t i = 0; i < base::size(bad_entries); i++) {
...
bool IsUndesirablePlugin(const WebPluginInfo& info) {
  ...
  for (size_t i = 0; i < sizeof(kUndesiredPlugins)/sizeof(kUndesiredPlugins[0]); i++) {
...
bool IsBlacklistedPlugin(const base::FilePath& path) {
  ...
  for (size_t i = 0; i < sizeof(kBlackListedPlugins)/sizeof(kBlackListedPlugins[0]); i++) {
...
typedef const char* (*UOS_NP_GetMimeDescriptionType)();
typedef const char* (*UOS_NP_GetPluginVersion)();
typedef NPError (*UOS_NP_GetValueType)(void* unused, nsPluginVariable variable, void* value_out);

const std::string uos_plugin_sep = "~~";

std::string readMimeInfoFromHandle(void * handle) {
    UOS_NP_GetMimeDescriptionType NP_GetMIMEDescription = reinterpret_cast<UOS_NP_GetMimeDescriptionType>(dlsym(handle, "NP_GetMIMEDescription"));
    const char * mime_description = NULL;
    if (NP_GetMIMEDescription) {
        mime_description = NP_GetMIMEDescription();
        return std::string(mime_description);
    } else {
        return "";
    }
}

std::string readPluginVersionFromHandle(void * handle) {
    UOS_NP_GetPluginVersion NP_GetPluginVersion_ = reinterpret_cast<UOS_NP_GetPluginVersion>(dlsym(handle, "NP_GetPluginVersion"));
    const char* version_ = NULL;
    if (NP_GetPluginVersion_) {
        version_ = NP_GetPluginVersion_();
...
std::string readPluginNameFromHandle(void * handle) {
    UOS_NP_GetValueType NP_GetValue = reinterpret_cast<UOS_NP_GetValueType>(dlsym(handle, "NP_GetValue"));
    if (NP_GetValue) {
...
std::string readPluginDescFromHandle(void * handle) {
    UOS_NP_GetValueType NP_GetValue = reinterpret_cast<UOS_NP_GetValueType>(dlsym(handle, "NP_GetValue"));
    if (NP_GetValue) {
        const char* desc = NULL;
        NP_GetValue(NULL, nsPluginVariable_DescriptionString, &desc);
        if (desc) {
...
std::vector<std::string> regexsplit(const std::string& input, const std::string & sep)
{
    std::regex re(sep);
...
bool uos_startWith(const std::string &str, const std::string &head) {
    return str.compare(0, head.size(), head) == 0;
}

#define MAX_PIPE_BUFFER 10240

bool readWebPluginMetaData(const base::FilePath& filename, std::vector<std::string> & ret) {
  int p_id;
  int fd[2];
  char line[MAX_PIPE_BUFFER] = {0};
  ...
  } else if (p_id == 0) {//子进程获取插件信息，并通过管道传送给父进程
      close(fd[0]);

      void * handle = dlopen(filename.value().c_str(), RTLD_LAZY | RTLD_LOCAL);
      if (!handle) {
          write(fd[1], "err", 3);
          close(fd[1]);

      } else { //load Succcess
          auto _mime = "mime:" + readMimeInfoFromHandle(handle);
          auto _version = "version:" + readPluginVersionFromHandle(handle);
          auto _name = "name:" + readPluginNameFromHandle(handle);
          auto _desc = "desc:" + readPluginDescFromHandle(handle);
          std::string payload =  _mime + uos_plugin_sep + _version + uos_plugin_sep + _name + uos_plugin_sep + _desc;
          write(fd[1], payload.c_str(), payload.length());
          close(fd[1]);
      }
      exit(0);
  ...
  if (waitpid(p_id, NULL, 0) != p_id) {
      ...
      read(fd[0], line, MAX_PIPE_BUFFER);
  }
  ...
  ret = regexsplit(meta, uos_plugin_sep);
  return true;
}

void uos_ExtractVersionString(const std::string& desc,
                                      WebPluginInfo* info) {
  static const struct {
    const char* kPrefix;
    const char* kPostfix;
...
void uos_parseMIMEDescription(
  const std::string& description,
  std::vector<content::WebPluginMimeType>* mime_types) {
  ...
  for (;;) {
    ...
    mime_type.file_extensions = base::SplitString(extensions, ",",
...
bool webPluginInfo(const base::FilePath& filename, WebPluginInfo* info) {
  std::vector<std::string> lines;
  if (!readWebPluginMetaData(filename, lines)) {
    return false;
  }
  std::string _mime = "";
  std::string _version = "";
  ...
  for (auto k: lines) {
    if (uos_startWith(k, "mime:")) {
        _mime = k.substr(5);
    } else if (uos_startWith(k, "version:")) {
        _version = k.substr(8);
    } else if (uos_startWith(k, "name:")) {
        _name = k.substr(5);
    } else if (uos_startWith(k, "desc:")) {
        _desc = k.substr(5);
    }
  }
  ...
  uos_parseMIMEDescription(std::string(_mime), &info->mime_types);

  if (_version.length() > 0) {
    info->version = base::UTF8ToUTF16(_version.c_str());
  }

  if (_name.length() > 0) {
    info->name = base::UTF8ToUTF16(_name.c_str());
    if (info->version.empty()) {
      uos_ExtractVersionString(_name, info);
    }
  }

  if (_desc.length() > 0) {
      info->desc = base::UTF8ToUTF16(_desc.c_str());
      if (info->version.empty())
        uos_ExtractVersionString(_desc, info);
    }
...
// FIX CFI PROBLEM ON OFFICIAL BUILD
...
#endif               
{
...
#if defined(USE_UNIONTECH_NPAPI)
  return webPluginInfo(filename, info);
#else
  ...
  ParseMIMEDescription(std::string(mime_description), &info->mime_types);

  typedef const char* (*NP_GetPluginVersion)();
  NP_GetPluginVersion NP_GetPluginVersion_ = reinterpret_cast<NP_GetPluginVersion>(dlsym(dl, "NP_GetPluginVersion"));
  const char* version_ = NULL;
  if (NP_GetPluginVersion_) {
    version_ = NP_GetPluginVersion_();
    info->version = base::UTF8ToUTF16(version_);
  }
...
void PluginList::GetPluginDirectories(std::vector<base::FilePath>* plugin_dirs) {
  ...
  if (moz_plugin_path) {
    std::vector<std::string> paths = 
        base::SplitString(moz_plugin_path, ":", 
        base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
