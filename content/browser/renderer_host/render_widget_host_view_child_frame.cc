...
RenderWidgetHostViewChildFrame::RenderWidgetHostViewChildFrame(
    ...
    #if defined(USE_UNIONTECH_NPAPI)
      , plugin_container_manager_(std::make_unique<GtkPluginContainerManager>())
    #endif
{
...
#if defined(USE_UNIONTECH_NPAPI)
  // RenderWidgetHostViewAura destructor called before RenderWidgetHostImpl destructor, OnDestroyPluginContainer must call here.
  OnDestroyPluginContainer(0); // 0 means all.

  if (plugin_container_manager_)
    plugin_container_manager_.reset();
#endif
...
void RenderWidgetHostViewChildFrame::Init() {
  ...
  GetTextInputManager();
#if defined(USE_UNIONTECH_NPAPI)
  LOG(INFO) << "RenderWidgetHostViewChildFrame::Init -> ShowReparentPlugin";
  plugin_container_manager_->ShowReparentPlugin(GetHostWindow(), true);
#endif
}
...
void RenderWidgetHostViewChildFrame::MovePluginWindows(
  ...
  plugin_container_manager_->ShowReparentPlugin(GetHostWindow(), true);
  gfx::Rect view_bounds0 = gfx::ScaleToEnclosedRect(GetNativeView()->GetBoundsInRootWindow(), gtk::GtkUi::GetRawDeviceScaleFactor());
  gfx::Rect view_bounds1 = gfx::ScaleToEnclosedRect(last_screen_rect_, gtk::GtkUi::GetRawDeviceScaleFactor());　
  gfx::Rect view_bounds2 = gfx::ScaleToEnclosedRect(GetBoundsInRootWindow(), gtk::GtkUi::GetRawDeviceScaleFactor());　
  gfx::Rect view_bounds3 = gfx::ScaleToEnclosedRect(GetViewBounds(), gtk::GtkUi::GetRawDeviceScaleFactor());　    

  view_bounds1.Offset(view_bounds0.OffsetFromOrigin()); 
  std::vector<content::WebPluginGeometry> moves = plugin_window_moves;  
  for (size_t i = 0; i < moves.size(); ++i) {
      moves[i].window_rect.Offset(view_bounds1.OffsetFromOrigin());       
      if((view_bounds3.y()-view_bounds2.y())<moves[i].window_rect.y()){
        if(moves[i].window_rect.y()<view_bounds0.y()){          
            moves[i].clip_rect.set_y(view_bounds0.y()-moves[i].window_rect.y());             
            moves[i].clip_rect.set_height(moves[i].window_rect.height()-(view_bounds0.y()-moves[i].window_rect.y()));//当moves[i].window_rect.y()为负的时候 
        }else{                  
            moves[i].clip_rect.set_y(0); 
        }        
      }else{        
        if((view_bounds3.y()-view_bounds2.y())<view_bounds0.y()){          
            moves[i].clip_rect.set_y(view_bounds0.y()-moves[i].window_rect.y());             
            moves[i].clip_rect.set_height(moves[i].window_rect.height()-(view_bounds0.y()-moves[i].window_rect.y()));//当moves[i].window_rect.y()为负的时候 
        }else{                  
            int32_t value = view_bounds3.y()-view_bounds2.y()-moves[i].window_rect.y();
            moves[i].clip_rect.set_y(value);
            moves[i].clip_rect.set_height(moves[i].window_rect.height()-value);//当moves[i].window_rect.y()为负的时候       
        }  
      }
      if(view_bounds0.width()<(moves[i].window_rect.x()+moves[i].clip_rect.width())){
        moves[i].clip_rect.set_width(view_bounds0.width()-moves[i].window_rect.x());
      }      
  }
  for(auto &move_rect: moves)
    plugin_container_manager_->MovePluginContainer(move_rect);
}

gfx::AcceleratedWidget RenderWidgetHostViewChildFrame::GetHostWindow() const {
  if (!frame_connector_)
    return 0L;
  ...
  RenderWidgetHostView* parent_view =
      frame_connector_->GetRootRenderWidgetHostView();
  if (parent_view) {
    aura::WindowTreeHost* host = parent_view->GetNativeView()->GetHost();
    return host ? host->GetAcceleratedWidget() : 0L;
  } else {
    return 0L;
  }
}

views::NpapiPluginMaskObserver *RenderWidgetHostViewChildFrame::
    GetNpapiPluginMaskObserver(views::Widget *widget) {
  if (plugin_container_manager_) {
    if(widget)
      plugin_container_manager_->AddWidgetObserver(widget);
    return plugin_container_manager_.get();
  }
  return nullptr;
}

void RenderWidgetHostViewChildFrame::OnCreatePluginContainer(
    gfx::PluginWindowHandle id) {
  plugin_container_manager_->CreatePluginContainer(id, GetHostWindow());
}

void RenderWidgetHostViewChildFrame::OnDestroyPluginContainer(
    gfx::PluginWindowHandle id) {
  plugin_container_manager_->DestroyPluginContainer(id);
}

void RenderWidgetHostViewChildFrame::OnWindowActiveStatusChanged(bool active) {
  plugin_container_manager_->OnWindowActiveStatusChanged(active);
}

bool RenderWidgetHostViewChildFrame::RetrieveSurrounding(std::string* text,
                                                  size_t* cursor_index) {
  return false;
}
#endif
