...
void RenderWidgetHostViewAura::MovePluginWindows(
  ...
  view_bounds = gfx::ScaleToEnclosedRect(view_bounds, gtk::GtkUi::GetRawDeviceScaleFactor());
  ...
  LOG(INFO) << window_->GetBoundsInRootWindow().ToString();
...
#if defined(USE_UNIONTECH_NPAPI)
  //plugin_container_manager_->OnWindowTitleChanged(base::UTF16ToUTF8(window_->GetTitle()));
#endif
}
...
void RenderWidgetHostViewAura::RenderProcessGone() {
  ...
  Destroy();
#if defined(USE_UNIONTECH_NPAPI)
  //plugin_container_manager_->RenderProcessGone(0, 0);
#endif
}
...
#if defined(USE_UNIONTECH_NPAPI)
  LOG(INFO) << "RenderWidgetHostViewAura::AddedToRootWindow -> ShowReparentPlugin";
...
gfx::AcceleratedWidget RenderWidgetHostViewAura::GetHostWindow() const {
  aura::WindowTreeHost* host = window_->GetHost();
  return host ? host->GetAcceleratedWidget() : 0L;
}

views::NpapiPluginMaskObserver *RenderWidgetHostViewAura::
...
void RenderWidgetHostViewAura::OnCreatePluginContainer(
    gfx::PluginWindowHandle id) {
  plugin_container_manager_->CreatePluginContainer(id, GetHostWindow());
}

void RenderWidgetHostViewAura::OnDestroyPluginContainer(
    gfx::PluginWindowHandle id) {
  plugin_container_manager_->DestroyPluginContainer(id);
}

void RenderWidgetHostViewAura::OnWindowActiveStatusChanged(bool active) {
  plugin_container_manager_->OnWindowActiveStatusChanged(active);
}

bool RenderWidgetHostViewAura::RetrieveSurrounding(std::string* text,
                                                  size_t* cursor_index) {
#ifdef USE_UNIONTECH_NPAPI_TAG_CANNOTBUILD
...
#else
  return false;
#endif  // USE_UNIONTECH_NPAPI_TAG_CANNOTBUILD
}

#endif // USE_UNIONTECH_NPAPI
