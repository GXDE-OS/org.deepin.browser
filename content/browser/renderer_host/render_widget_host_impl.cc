...
#if defined(USE_UNIONTECH_NPAPI)
  OnWindowActiveStatusChanged(false);
#endif
  RejectMouseLockOrUnlockIfNecessary(
...
#if defined(USE_UNIONTECH_NPAPI)
  OnWindowActiveStatusChanged(true);
#endif
...
void RenderWidgetHostImpl::OnUpdateRect(
  ...
  LOG(INFO) << "[NPAPI] BrowserProcess RenderWidgetHostImpl::OnUpdateRect";
...
void RenderWidgetHostImpl::OnCreatePluginContainer(
  ...
  LOG(INFO) << "[NPAPI] BrowserProcess RenderWidgetHostImpl::OnCreatePluginContainer: " << id;

  if (view_)
    view_->OnCreatePluginContainer(id);
}

void RenderWidgetHostImpl::OnDestroyPluginContainer(
    gfx::PluginWindowHandle id) {
  if (view_)
    view_->OnDestroyPluginContainer(id);
}
void RenderWidgetHostImpl::OnWindowActiveStatusChanged(bool active) {
  if (view_)
    view_->OnWindowActiveStatusChanged(active);
}
#endif
