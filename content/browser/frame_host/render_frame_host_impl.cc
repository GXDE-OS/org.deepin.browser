...
#if 0
const GURL& RenderFrameHostImpl::GetLastCommittedURLForUOS() {
  if (last_committed_url_.scheme() == "chrome") {
    std::string new_scheme = "uos";
    url::Component comp;
    comp.len = static_cast<int>(new_scheme.length());
    GURL::Replacements replacement;
    replacement.SetScheme(new_scheme.c_str(), comp);
    last_commit_url_for_uos = last_committed_url_.ReplaceComponents(replacement);
    return last_commit_url_for_uos;
  }
  return last_committed_url_;
}
#endif
...
void RenderFrameHostImpl::ProcessUosCustomizeCompleted(
    bool proceed,
    bool treat_as_final_completion_callback,
    const base::TimeTicks& renderer_uos_customize_start_time,
    const base::TimeTicks& renderer_uos_customize_end_time) {
  LOG(ERROR) << __func__;
}
...
void RenderFrameHostImpl::RunBeforeUnloadConfirmForBankAndIRC(bool is_reload,
                                                              const base::string16& bank,
                                                              const base::string16& message,
                                                              RunBeforeUnloadConfirmForBankAndIRCCallback ipc_response_callback) {
  //
  TRACE_EVENT1("navigation", "RenderFrameHostImpl::OnRunBeforeUnloadConfirm",
               "frame_tree_node", frame_tree_node_->frame_tree_node_id());
  ...
  delegate_->RunBeforeUnloadConfirmForBankAndIRC(this, is_reload, is_tab_close_, bank, message,
                                    std::move(dialog_closed_callback));
}
...
void RenderFrameHostImpl::DispatchUosCustomizedEvent() {
  LOG(ERROR) << __func__;
  SendUosCustomize(this->GetWeakPtr());
}
...
void RenderFrameHostImpl::DispatchBeforeUnload(BeforeUnloadType type,
  ...
  //modify by xiaohuyang, Handle the task of #82352.
  is_tab_close_ = type == BeforeUnloadType::TAB_CLOSE ? true : false;
...
void RenderFrameHostImpl::SendUosCustomize(base::WeakPtr<RenderFrameHostImpl> rfh) {
  auto uos_customize_closure = base::BindOnce(
      [](base::WeakPtr<RenderFrameHostImpl> impl, bool proceed,
         base::TimeTicks renderer_uos_customize_start_time,
         base::TimeTicks renderer_uos_customize_end_time) {
        if (!impl)
          return;
        impl->ProcessUosCustomizeCompleted(
            proceed, false /* treat_as_final_completion_callback */,
            renderer_uos_customize_start_time, renderer_uos_customize_end_time);
      },
      rfh);
  rfh->GetAssociatedLocalFrame()->UosCustomize(std::move(uos_customize_closure));
}
