 ...
 public:
  explicit ClientCertificateDelegateImpl(
      ...
      : handler_(std::move(handler)) {
        // LOG(ERROR)<<"ClientCertificateDelegateImpl is constructed.";
      }

  ~ClientCertificateDelegateImpl() override {
    if (!continue_called_ && handler_) {
      // LOG(ERROR)<<"ClientCertificateDelegateImpl is destructed.CancelCertificateSelection is called!";
...
SSLClientAuthHandler::~SSLClientAuthHandler() {
  ...
  if (cancellation_callback_) {
    LOG(ERROR)<<"cancellation_callback_ is called";
...
void SSLClientAuthHandler::DidGetClientCerts(
  ...
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
  // LOG(ERROR)<<"DidGetClientCerts start.";
