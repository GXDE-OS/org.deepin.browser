...
void Watcher::DidRunTask(const base::PendingTask* task,
  ...
  if (UNLIKELY(currently_running_metadata->empty() ||
    ...
    *mismatched_task_identifiers += 1;
    ...
    // happens.
    //wangfeng, modify, bug/46748, crash when dragging tab into other browser window(may resolved), 2020/09/10 --start
#if defined(OS_LINUX)
    return currently_running_metadata_ui_.clear();
#endif
    //wangfeng, modify, bug/46748, crash when dragging tab into other browser window(may resolved), 2020/09/10 --end
#if UNUSE
