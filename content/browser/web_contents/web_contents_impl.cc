...
#if 0
const GURL& WebContentsImpl::GetLastCommittedURLForUOS() {
  if (GetLastCommittedURL().scheme() == "chrome") {
    std::string new_scheme = "uos";
    url::Component comp;
    comp.len = static_cast<int>(new_scheme.length());
    GURL::Replacements replacement;
    replacement.SetScheme(new_scheme.c_str(), comp);
    last_commit_url_for_uos = GetLastCommittedURL().ReplaceComponents(replacement);
    return last_commit_url_for_uos;
  }
  return GetLastCommittedURL();
}

const GURL& WebContentsImpl::GetVisibleURLForUOS() {
  if (GetVisibleURL().scheme() == "chrome") {
    std::string new_scheme = "uos";
    url::Component comp;
    comp.len = static_cast<int>(new_scheme.length());
    GURL::Replacements replacement;
    replacement.SetScheme(new_scheme.c_str(), comp);
    last_visible_url_for_uos = GetVisibleURL().ReplaceComponents(replacement);
    return last_visible_url_for_uos;
  }
  return GetVisibleURL();
}
#endif
...
std::set<RenderWidgetHostView*> WebContentsImpl::GetRenderWidgetChildFrameView() {
  ...
  for (auto* view : GetRenderWidgetHostViewsInWebContentsTree()) {
    ...
    if (rwhvb->IsRenderWidgetHostViewChildFrame()) {
      set.insert(static_cast<RenderWidgetHostViewChildFrame*>(view));
    }
  }
  return set;
}
...
void WebContentsImpl::DispatchUosCustomizedEvent() {
  GetMainFrame()->DispatchUosCustomizedEvent();
}
...
bool WebContentsImpl::ShouldIgnoreUnresponsiveRenderer() {
  ...
  //TODO:
  //    Just ingore the unresponsive message temporarily, should recover
  //the code when get a better solution for NP Dialog block the render
...
void WebContentsImpl::RunBeforeUnloadConfirmForBankAndIRC(RenderFrameHost* render_frame_host,
                                                          bool is_reload,
                                                          bool is_tab_close,
  ...
  if (dialog_manager_) {
    dialog_manager_->RunBeforeUnloadDialogForBankAndIRC(
        this, render_frame_host, is_reload, is_tab_close, bank, message,
        base::BindOnce(&CloseDialogCallbackWrapper::Run, wrapper, false));
  }
}
...
void WebContentsImpl::OnNativeThemeUpdated(ui::NativeTheme* observed_theme) {
  ...
  std::string script = "window.uosTheme = ";  
  ...
  GetFrameTree()->root()->current_frame_host()->ExecuteJavaScriptForTests(base::UTF8ToUTF16(script), base::NullCallback());
