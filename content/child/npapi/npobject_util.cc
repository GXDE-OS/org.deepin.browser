...
bool CreateNPVariant(const NPVariant_Param& param,
  ...
  switch (param.type) {
    ...
    case NPVARIANT_PARAM_STRING: {
      ...
      void* buffer = malloc(param.string_value.size() + 1);
      memset(buffer, 0, param.string_value.size() + 1);
    ...
    case NPVARIANT_PARAM_SENDER_OBJECT_ROUTING_ID: {
      ...
      #if 0
        // add refcount with NPObject for map
        object = NPObjectProxy::Create(channel, param.npobject_routing_id, render_view_id, page_url, owner);
        result->value.objectValue = WebBindings::retainObject(object);
      #else
        result->value.objectValue = NPObjectProxy::Create(channel, param.npobject_routing_id, render_view_id, page_url, owner);
      #endif
      }
      break;
    }
