...
struct NPObjectWrapper : public blink::NPObjectWrapperBase {
    NPObjectProxy* proxy;

    NPObjectWrapper(): proxy(nullptr) {}

    ~NPObjectWrapper() override {
      delete proxy;
    }

    const blink::WrapperTypeInfo* GetWrapperTypeInfo() const override { return blink::npObjectTypeInfo(); }
};

static NPObjectWrapper* npObjectToNPObjectWrapper(NPObject* npObject) {
    DCHECK(npObject != nullptr);

    NPObjectWrapper* wrapper = static_cast<NPObjectWrapper*>(blink::npObjectToNPObjectWrapperBase(npObject));
...
NPObjectProxy* NPObjectProxy::GetProxy(NPObject* object) {
  ...
  if (&npclass_proxy_ == object->_class) {
    proxy = npObjectToNPObjectWrapper(object)->proxy;
  }
...
NPObjectProxy::~NPObjectProxy() {
  ...
  #if 0
    NPObject* object = channel_->GetExistingNPObjectProxy(route_id_);
    if (object) {
...
NPObject* NPObjectProxy::Create(NPChannelBase* channel,
                                ...
                                const GURL& page_url,
                                NPP owner) {
  NPObject* object = WebBindings::createObject(owner, &npclass_proxy_);
  NPObjectWrapper* wrapper = npObjectToNPObjectWrapper(object);
  wrapper->proxy = new NPObjectProxy(channel, route_id, render_view_id, page_url);
  channel->AddMappingForNPObjectProxy(route_id, object);
  return object;
}
...
NPObject* NPObjectProxy::NPAllocate(NPP, NPClass*) {
  NPObjectWrapper* wrapper = blink::MakeGarbageCollected<NPObjectWrapper>();
  return blink::npObjectWrapperBaseToNPObject(wrapper);
}
