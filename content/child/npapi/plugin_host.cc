  ...
  do {
    ...
    if (state != newstate) {
      switch (newstate) {
        ...
        case GETVALUE:
          // Got a header.
          name = base::ToLowerASCII(std::string(start, ptr - start));
...
static NPError PostURLNotify(NPP id,
  ...
  if (file) {
    ...
    if (base::StartsWith(file_path_ascii, kFileUrlPrefix, base::CompareCase::INSENSITIVE_ASCII)) {
...
const char* NPN_UserAgent(NPP id) {
  ...
  if (!leaky_user_agent.IsCreated())
    leaky_user_agent.Get() = content::GetContentClient()->plugin()->GetUserAgentInPlugin();
...
void NPN_InvalidateRegion(NPP id, NPRegion invalidRegion) {
  ...
  LOG(INFO) << "[NPAPI] NPN_InvalidateRegion";
  scoped_refptr<PluginInstance> plugin(FindInstance(id));
...
NPError NPN_GetValue(NPP id, NPNVariable variable, void* value) {
  ...
  switch (static_cast<int>(variable)) {
    ...
    case NPNVPluginElementNPObject: {
      LOG(INFO) << "[NPAPI] NPN_GetValue -> NPNVPluginElementNPObject";
      scoped_refptr<PluginInstance> plugin(FindInstance(id));
      if (!plugin.get()) {
        NOTREACHED();
        return NPERR_INVALID_INSTANCE_ERROR;
    ...
    case NPNVnetscapeWindow: {      
      LOG(INFO) << "[NPAPI] NPN_GetValue -> NPNVnetscapeWindow";
      scoped_refptr<PluginInstance> plugin = FindInstance(id);
      if (!plugin.get()) {
        NOTREACHED();
        return NPERR_INVALID_INSTANCE_ERROR;
    ...
    case NPNVjavascriptEnabledBool: {
      LOG(INFO) << "[NPAPI] NPN_GetValue -> NPNVjavascriptEnabledBool";
      // yes, JS is enabled.
      *((void**)value) = (void*)1;
      rv = NPERR_NO_ERROR;
      break;
    ...
    case NPNVToolkit:
      LOG(INFO) << "[NPAPI] NPN_GetValue -> NPNVToolkit";
      // Tell them we are GTK2.  (The alternative is GTK 1.2.)
      *reinterpret_cast<int*>(value) = NPNVGtk2;
      rv = NPERR_NO_ERROR;
      break;  
    case NPNVSupportsXEmbedBool:
      LOG(INFO) << "[NPAPI]NPN_GetValue -> NPNVSupportsXEmbedBool";
      *reinterpret_cast<NPBool*>(value) = true;
      rv = NPERR_NO_ERROR;
      break;
    case NPNVSupportsWindowless: {
      LOG(INFO) << "[NPAPI]NPN_GetValue -> NPNVSupportsWindowless";
    ...
    case NPNVprivateModeBool: {
      LOG(INFO) << "[NPAPI]NPN_GetValue -> NPNVprivateModeBool";
  ...
  #endif  // OS_MACOSX
    default:
      LOG(INFO) << "[NPAPI] NPN_GetValue(" << variable << ") is not implemented yet.";
      break;
  }
  return rv;
}
