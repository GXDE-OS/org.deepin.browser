...
class MultiPartResponseClient : public blink::WebAssociatedURLLoaderClient {  
 public:
  ...
  //                     const char* data, int data_length) override {
...
PluginURLFetcher::~PluginURLFetcher() {
  if (request_id_ >= 0) {
    RenderThreadImpl::current()->resource_dispatcher()->RemovePendingRequest(
        request_id_, RenderThreadImpl::current()->main_thread_runner());
  }

  // PluginStreamUrl::RemoveFetcher(request_id_);
}

void PluginURLFetcher::AttachRequestID(int request_id)
{
    request_id_ = request_id;
}

void PluginURLFetcher::Cancel() {
  RenderThreadImpl::current()->resource_dispatcher()->Cancel(
      request_id_, RenderThreadImpl::current()->main_thread_runner());
...
void PluginURLFetcher::URLRedirectResponse(bool allow) {
  ...
  if (allow) {
    RenderThreadImpl::current()->resource_dispatcher()->SetDefersLoading(
        request_id_, false);
  } else {
    RenderThreadImpl::current()->resource_dispatcher()->Cancel(
        request_id_, RenderThreadImpl::current()->main_thread_runner());
...
bool PluginURLFetcher::OnReceivedRedirect(
  ...
  std::cout<<"----------[TODO]PluginURLFetcher::OnReceivedRedirect---first_party_for_cookies_"<<std::endl;
...
void PluginURLFetcher::OnReceivedResponse(network::mojom::URLResponseHeadPtr head) {
  if (!plugin_stream_)
    return;
  ...
  if (plugin_stream_->seekable()) {
    ...
    if (response_code == 206) {
      ...
      std::cout<<"--------------------------------------xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx______________________"<<std::endl;
      // WebURLLoaderImpl::PopulateURLResponse(url_, info, &response, false, request_id_);
  ...
  int length = static_cast<int>(head->content_length);
  int expected_length = std::max(length, 0);

  base::Time temp;
  uint32_t last_modified = 0;
  std::string headers;
  if (head->headers.get()) {  // NULL for data: urls.
    if (head->headers->GetLastModifiedValue(&temp))
    ...
    headers += head->headers->GetStatusText();
    headers += "\n";
  ...
  plugin_stream_->DidReceiveResponse(head->mime_type,
                                     headers,
                                     expected_length,
                                     last_modified,
                                     request_is_seekable);
...
void PluginURLFetcher::OnStartLoadingResponseBody(
    mojo::ScopedDataPipeConsumerHandle body) {

}

void PluginURLFetcher::OnReceivedData(std::unique_ptr<ReceivedData> data) {
  const char* payload = data->payload();
  int data_length = data->length();
  int encoded_data_length = data->length();
  if (!plugin_stream_)
    return;
...
void PluginURLFetcher::OnTransferSizeUpdated(int transfer_size_diff) {
