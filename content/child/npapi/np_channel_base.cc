...
static std::string GenerateVerifiedChannelID(const std::string& prefix){
  ...
  id.append(IPC::Channel::GenerateUniqueRandomChannelID());
  id.append(base::StringPrintf("\\%d", secret));
  return id;
}

typedef std::map<std::string, scoped_refptr<NPChannelBase> > ChannelMap;
...
NPChannelBase* NPChannelBase::GetChannel(
  ...
  if (!channel->channel_valid()) {
    if (mode & IPC::Channel::MODE_SERVER_FLAG) {
      mojo::MessagePipeHandle handle0, handle1;
      MojoCreateMessagePipe(nullptr, handle0.mutable_value(), handle1.mutable_value());
      channel->peer_handle_.mojo_handle = handle0;
      channel->peer_handle_.name = GenerateVerifiedChannelID(channel_key);

      channel->channel_handle_.mojo_handle = handle1;
      channel->channel_handle_.name = GenerateVerifiedChannelID(channel_key);
      LOG(INFO) << "NPAPI CHANNEL NAME :  " << channel->channel_handle_.name;
    } else if (mode & IPC::Channel::MODE_CLIENT_FLAG) {
      channel->channel_handle_.name = channel_key;
      channel->channel_handle_.mojo_handle = channel_handle.mojo_handle;
    }
    ...
    if (use_handle_name_only)
      *use_handle_name_only = false;
  } else {
    if (use_handle_name_only)
      *use_handle_name_only = true;
  }
  return channel.get();
}
...
bool NPChannelBase::Init(base::SingleThreadTaskRunner* ipc_task_runner,
  ...
  channel_ =
      ...
      base::ThreadTaskRunnerHandle::Get(), create_pipe_now, shutdown_event);
...
bool NPChannelBase::Send(IPC::Message* message) {
  if (!channel_) {
    LOG(INFO) << "Channel is NULL; dropping message";
    delete message;
    return false;
  }
...
IPC::ChannelHandle NPChannelBase::peer_mojo_handle() const {
  return peer_handle_;
