...
class WebPluginDelegateImpl : public WebPluginDelegate,
                              public PluginResourceDispatcherDelegate {
public:
  enum PluginQuirks {
  ...
  void WindowlessPaint(cairo_t* context, const gfx::Rect& damage_rect); 
  ...
  void Paint(SkCanvas* canvas, const gfx::Rect& rect);
  ...
  void FetchedURL(int resource_id,
                  int response_code,
  ...
  // PluginResourceDispatcherDelegate implementation
  void OnResourceFetched(int resource_id,
    int response_code, const std::string& mime,
    const std::string& head, const std::string& data) override;
  // End of PluginResourceDispatcherDelegate implementation.
 ...
 private:
  ...
  std::map<int, PluginStreamUrl*> download_urls;
  ...
  std::unique_ptr<WebPluginIMEWin> plugin_ime_;
...
// #endif
  ...
  void emptyPixmap(GdkPixmap* pixmap, double x, double y,
...
// #endif
  ...
  std::unique_ptr<base::RepeatingTimer<WebPluginDelegateImpl> > redraw_timer_;
