...
bool WebPluginDelegateImpl::Initialize(
  ...
  if (quirks_ & PLUGIN_QUIRK_DONT_ALLOW_MULTIPLE_INSTANCES) {
    ...
    if (plugin_lib->instance_count() > 1) {
      LOG(ERROR) << "[NPAPI] WebPluginDelegateImpl::Initialize FALSE:plugin_lib instance_count > 1";
      return false;
    }
  }
  ...
  LOG(INFO) << "[NPAPI] WebPluginDelegateImpl::Initialize -> instance Start";
  creation_succeeded_ = instance_->Start(
      url, argn.get(), argv.get(), argc, load_manually);
  if (!creation_succeeded_) {
    LOG(ERROR) << "[NPAPI] WebPluginDelegateImpl::Initialize FALSE: Couldn't start plugin instance";
    return false;
  }
  ...
  if (!windowless_) {
    if (!WindowedCreatePlugin()) {
      LOG(ERROR) << "[NPAPI] WebPluginDelegateImpl::Initialize False: Couldn't create windowed plugin";
      return false;
    }
  }
...
void WebPluginDelegateImpl::UpdateGeometry(
  ...
  if (windowless_) {
    DLOG(INFO) << "[NPAPI] WebPluginDelegateImpl::UpdateGeometry -> WindowlessUpdateGeometry";
    WindowlessUpdateGeometry(window_rect, clip_rect);
  } else {
    DLOG(INFO) << "[NPAPI] WebPluginDelegateImpl::UpdateGeometry -> WindowedUpdateGeometry";
...
#if defined(ARCH_CPU_MIPS_FAMILY) || defined(ARCH_CPU_ARM_FAMILY)
  if (WindowedReposition(window_rect, clip_rect) ||
#else
  WindowedReposition(window_rect, clip_rect);
  if (
#endif
#else
  if (WindowedReposition(window_rect, clip_rect) ||
...
void WebPluginDelegateImpl::FetchURL(unsigned long resource_id,
  ...
  download_urls[resource_id] = plugin_stream;

  bool copy_stream_data = !!(quirks_ & PLUGIN_QUIRK_COPY_STREAM_DATA);
  plugin_stream->StartFetch(url,
  ...
  PluginThread::current()->AddPluginResourceDispatcherDelegate(routing_id, this);

}

void WebPluginDelegateImpl::FetchedURL(int resource_id,
                                       int response_code,
                                       const std::string& mime,
                                       const std::string& head,
                                       const std::string& data) {

  std::map<int, PluginStreamUrl*>::iterator it =
      download_urls.find(resource_id);
  if (it != download_urls.end()) {
    it->second->FetchedURL(resource_id,
                           response_code, mime, head,
                           data);
    download_urls.erase(it);
  }
}


void WebPluginDelegateImpl::OnResourceFetched(int resource_id,
  int response_code, const std::string& mime,
  const std::string& head, const std::string& data) {
  return FetchedURL(resource_id, response_code, mime, head, data);
}
