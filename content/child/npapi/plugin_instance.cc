...
void PluginInstance::AddStream(PluginStream* stream) {
  open_streams_.push_back(base::WrapRefCounted(stream));
}

bool PluginInstance::HasStream(PluginStream* stream) {
...
bool PluginInstance::Start(const GURL& url,
  ...
  LOG(INFO) << "[NPAPI]PluginInstance::Start";
  ...
  if (err == NPERR_NO_ERROR) {
    ...
    LOG(ERROR) << "[NPAPI]PluginInstance Start failed! errorcode: " << err;
  }
  return err == NPERR_NO_ERROR;
}

NPObject *PluginInstance::GetPluginScriptableObject() {
  LOG(INFO) << "[NPAPI] PluginInstance::GetPluginScriptableObject";
  NPObject *value = NULL;
...
bool PluginInstance::GetFormValue(base::string16* value) {
  LOG(INFO) << "[NPAPI] PluginInstance::GetFormValue";
...
NPError PluginInstance::NPP_New(unsigned short mode,
  ...
  LOG(INFO) << "[NPAPI]PluginInstance::NPP_New";
...
void PluginInstance::NPP_Destroy() {
  ...
  LOG(INFO) << "[NPAPI]PluginInstance::NPP_Destroy";
...
void PluginInstance::DidReceiveManualResponse(const GURL& url,
  ...
  // leiyuanxiang , modify , bug (56725) , 2020/12/18 --start
  if(!load_manually_) return;
  // leiyuanxiang , modify , bug (56725) , 2020/12/18 --end
...
void PluginInstance::RequestRead(NPStream* stream, NPByteRange* range_list) {
  ...
  LOG(INFO) << " ---- [NPAPI] PluginInstance::RequestRead ----";
  while (range_list) {
    range_info += base::NumberToString(range_list->offset);
    range_info.push_back('-');
    range_info +=
        base::NumberToString(range_list->offset + range_list->length - 1);
