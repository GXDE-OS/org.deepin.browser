...
class NPChannelBase : public IPC::Listener,
 ...
 public:
  ...
  IPC::ChannelHandle peer_mojo_handle() const;
 ...
 protected:
  ...
  // if use_handle_name_only is true, this means mojo channel has already been
  // connected, and the caller should use an invalid
  // mojo handle instead of ChannelHandle's mojo handle.
  static NPChannelBase* GetChannel(
      ...
      bool *use_handle_name_only = nullptr);
  ...
  // only available in plugin process
  IPC::ChannelHandle peer_handle_;
 ...
 private:
  ...
  typedef std::map<int, NPObjectBase*> ListenerMap;
  ListenerMap npobject_listeners_;

  typedef std::map<int, NPObject*> ProxyMap;
  ProxyMap proxy_map_;

  typedef std::map<NPObject*, int> StubMap;
  StubMap stub_map_;

  typedef std::map<struct _NPP*, int> OwnerToRouteMap;
  OwnerToRouteMap owner_to_route_;

  typedef std::map<int, struct _NPP*> RouteToOwnerMap;
