...
class HeaderCopier : public WebHTTPHeaderVisitor {
 public:
  ...
  virtual void VisitHeader(const WebString& name, const WebString& value) {
    ...
    for (size_t i = 0; i < sizeof(kReplaceHeaders)/sizeof(kReplaceHeaders)[0]; ++i) {
  ...
  while ((boundary_pos = FindBoundary()) != std::string::npos) {
    if (client_) {
      ...
      if (data_length > 0) {
        ...
        client_->DidReceiveData(data_.data(),
...
bool MultipartResponseDelegate::ParseHeaders() {
  ...
  WebURLResponse response(original_response_.ResponseUrl());
