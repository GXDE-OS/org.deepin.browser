...
bool WebPluginDelegateStub::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(WebPluginDelegateStub, msg)
    ...
    IPC_MESSAGE_HANDLER(PluginMsg_DidFinishLoadWithReason, OnDidFinishLoadWithReason)
    ...
    IPC_MESSAGE_HANDLER(PluginMsg_GetPluginScriptableObject, OnGetPluginScriptableObject)
    ...
    IPC_MESSAGE_HANDLER(PluginMsg_SendJavaScriptStream, OnSendJavaScriptStream)
    ...
    IPC_MESSAGE_HANDLER(PluginMsg_FetchedURL, OnFetchedURL)
...
void WebPluginDelegateStub::OnInit(const PluginMsg_Init_Params& params,
                                   bool* transparent,
                                   bool* result) {
  LOG(INFO) << "[NPAPI] Web Plugin Delegate Stub Start OnInit, page_url:" << params.page_url << "plugin pid:" << base::GetCurrentProcId();
  ...
  *result = false;
  ...
  if (delegate_) {
    ...
    channel_->AddMappingForNPObjectOwner(instance_id_, delegate_->GetPluginNPP());
...
void WebPluginDelegateStub::OnSetFocus(bool focused) {
  LOG(INFO)<<"--------------------[NPAPI]-----------------------WebPluginDelegateStub::OnSetFocus";
...
void WebPluginDelegateStub::OnHandleInputEvent(
    ...
    *handled = delegate_->HandleInputEvent(*event, &cursor_info);
    cursor->SetCursor(cursor_info);
    // cursor->InitFromCursorInfo(cursor_info);
    LOG(INFO)<<"------------[TODO]-------WebPluginDelegateStub::OnHandleInputEvent--------";
}
...
void WebPluginDelegateStub::OnUpdateGeometry(
  ...
  DLOG(INFO) << "[NPAPI] WebPluginDelegateStub::OnUpdateGeometry instance_id " << instance_id_;
...
void WebPluginDelegateStub::OnGetFormValue(base::string16* value, bool* success) {
  *success = false;
  if (!delegate_)
    return;
...
void WebPluginDelegateStub::OnFetchURL(
  ...
  delegate_->FetchURL(params.resource_id,
                      ...
                      instance_id_, // WebpluginDelegateProxy/WebpluginDelegateStub/WebpluginProxy instance id
#else
...
void WebPluginDelegateStub::OnFetchedURL(
    const PluginMsg_FetchedURL_Params& params) {
  delegate_->FetchedURL(params.resource_id,
                        params.response_code, params.mime, params.head,
                        params.data);
}
