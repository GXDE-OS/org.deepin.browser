 ...
 private:
  class SharedTransportDIB : public base::RefCounted<SharedTransportDIB> {
   public:
    ...
    CairoSurface*   cairo() { return cairo_.get(); }
    void reset_cairo(unsigned char *data, int width, int height);
   private:
    friend class base::RefCounted<SharedTransportDIB>;
    ~SharedTransportDIB();


    std::unique_ptr<CairoSurface> cairo_;
  ...
  typedef std::map<int, WebPluginResourceClient*> ResourceClientMap;
