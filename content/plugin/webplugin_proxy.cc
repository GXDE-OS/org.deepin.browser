...
#include "third_party/ced/src/util/string_util.h"
...
class CairoSurface {
public:
  CairoSurface(unsigned char *data, int width, int height):
      data_(data),
      width_(width),
      height_(height) {
    if(data_ == nullptr)
      LOG(INFO) << "[NPAPI] CairoSurface::data_:" << data_;
    stride_ = cairo_format_stride_for_width (format_, width_);
  }
  ...
  bool GetCairoSurface(cairo_t** cairo, cairo_surface_t** surface) {
    if(cairo_surface_) {
      cairo_surface_destroy(cairo_surface_);
      cairo_surface_ = nullptr;
    }
    if(data_) {
      cairo_surface_ = *surface = cairo_image_surface_create_for_data (
          data_, format_,
          width_, height_, stride_);
      *cairo = cairo_create(*surface);
      DLOG(INFO) << "[NPAPI] WebPluginProxy::Paint CairoSurface GetCairoSurface";
      return true;
    }
    cairo = nullptr;
    surface = nullptr;
    LOG(INFO) << "[NPAPI] WebPluginProxy::Paint CairoSurface GetCairoSurface false";
    return false;
  }
...
void WebPluginProxy::SharedTransportDIB::reset_cairo(
    unsigned char* data, int width, int height) {
  cairo_.reset(new CairoSurface(
      data, width, height));
}
...
WebPluginProxy::WebPluginProxy(
    ...
    : channel_(channel),
      ...
      weak_factory_(this) {
#if defined(USE_X11)
  windowless_shm_pixmaps_[0] = x11::None;
  windowless_shm_pixmaps_[1] = x11::None;
...
WebPluginProxy::~WebPluginProxy() {
#if defined(USE_X11)
  if (windowless_shm_pixmaps_[0] != x11::None)
    XFreePixmap(gfx::GetXDisplay(), windowless_shm_pixmaps_[0]);
  if (windowless_shm_pixmaps_[1] != x11::None)
...
void WebPluginProxy::InvalidateRect(const gfx::Rect& rect) {
  ...
  if (!waiting_for_paint_) {
    ...
    DLOG(INFO) << "[NPAPI] WebPluginProxy::InvalidateRect";
    base::ThreadTaskRunnerHandle::Get()->PostTask(
                                          FROM_HERE,
                                          base::BindOnce(&WebPluginProxy::OnPaint,
...
NPObject* WebPluginProxy::GetWindowScriptNPObject() {
  ...
  LOG(INFO) << "[NPAPI] WebPluginProxy::GetWindowScriptNPObject SEND PluginHostMsg_GetWindowScriptNPObject";
  int npobject_route_id = channel_->GenerateRouteID();
  bool success = false;
  Send(new PluginHostMsg_GetWindowScriptNPObject(route_id_, npobject_route_id, &success));
  if (!success) {
    LOG(ERROR) << "[NPAPI] WebPluginProxy::GetWindowScriptNPObject FAILED!!";
    return NULL;
  }
...
void WebPluginProxy::Paint(const gfx::Rect& rect) {
  DLOG(INFO) << "[NPAPI] WebPluginProxy::Paint";
#if defined(OS_MACOSX)
  if (!windowless_context())
    return;
#else
  ...
  offset_rect.Offset(delegate_->GetRect().OffsetFromOrigin());
   auto* windowless_buffer_cairo = windowless_dibs_[windowless_buffer_index_]->cairo();
   cairo_t* cairo = nullptr;
   cairo_surface_t* cairo_surface = nullptr;
   if(windowless_buffer_cairo && windowless_buffer_cairo->GetCairoSurface(&cairo, &cairo_surface)){
     DLOG(INFO) << "[NPAPI] WebPluginProxy::Paint WindowlessPaints";
     delegate_->WindowlessPaint(cairo, offset_rect);
   }else{
     DLOG(INFO) << "[NPAPI] WebPluginProxy::Paint not do WindowlessPaints";
   }
   return;
#if defined(OS_MACOSX)
  ...
  // will work.
  saved_canvas->drawColor(SkColorSetARGB(0, 0, 0, 0), SkBlendMode::kSrcOver);
...
void WebPluginProxy::UpdateGeometry(
  ...
  if (TransportDIB::is_valid_handle(windowless_buffer0)) {
    ...
    LOG(INFO) << "[NPAPI] WebPluginProxy::UpdateGeometry ->  SetWindowlessBuffers";
  ...
  if (delegate_->IsWindowless() && !clip_rect.IsEmpty() &&
    ...
    LOG(INFO) << "[NPAPI] WebPluginProxy::UpdateGeometry ->  InvalidateRect";
    InvalidateRect(damaged_rect_);
  }
}
...
void WebPluginProxy::CreateDIBAndCanvasFromHandle(
  ...
  TransportDIB* dib = TransportDIB::Map(dib_handle).release();
  ...
  if (dib) {
    *canvas = skia::AdoptRef(
        (dib->GetPlatformCanvasGTK(window_rect.width(), window_rect.height())).release());
  } else {
    canvas->clear();
  }
  *dib_out = new SharedTransportDIB(dib);
  (*dib_out)->reset_cairo( (unsigned char*)dib->memoryGTK(), window_rect.width(),
                           window_rect.height() );  
}
