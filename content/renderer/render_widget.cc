...
#if defined(USE_UNIONTECH_NPAPI)
      need_update_rect_for_plugin_(false),
#endif
      is_hidden_(hidden),
...
void RenderWidget::DidReceiveCompositorFrameAck() {
  ...
  if (!need_update_rect_for_plugin_) {
    return;
  }

  ViewHostMsg_UpdateRect_Params params;
  if (plugin_window_moves_.size() > 0) {
...
void RenderWidget::SchedulePluginMove(const WebPluginGeometry& move) {
  ...
  if (plugin_window_moves_.size())
    need_update_rect_for_plugin_ = true;
}
...
cc::LayerTreeSettings RenderWidget::GenerateLayerTreeSettings(
  ...
  settings.send_compositor_frame_ack = true;
