...
RenderFrameImpl::RenderFrameImpl(CreateParams params)
    : frame_(nullptr),
      ...
      render_view_(params.render_view->GetWeakPtr()),
...
blink::WebPlugin* RenderFrameImpl::CreatePlugin(
  ...
  if (info.type == WebPluginInfo::PLUGIN_TYPE_NPAPI) {
    ...
    LOG(INFO) << "[NPAPI] RenderFrameImpl::CreatePlugin start create WebPluginImpl";
...
blink::WebPlugin* RenderFrameImpl::CreatePlugin(
  ...
  if (GetContentClient()->renderer()->OverrideCreatePlugin(frame, this, params, &plugin)) {
    return plugin;
  }
...
blink::WebLocalFrame* RenderFrameImpl::CreateChildFrame(
  ...
  RenderFrameImpl* child_render_frame = RenderFrameImpl::Create(
      render_view_.get(), params_reply.child_routing_id,
...
void RenderFrameImpl::RunScriptsAtDocumentReady(bool document_is_empty) {
  ...
  // modify by xiaohuyang, fix bug#63969,  2021/05/27  --start
  // Besause some '404' error pages are useful, we simply judge that only when the content type is 'text/plain', use a custom page.
  // It may not be very reasonable, but it just meets the product requirements.
#if UNUSED
  ...
  if ((!document_is_empty || !IsMainFrame()))
    return;
#else
  bool use_custom_error_page = false;
  if (frame_->GetDocumentLoader()->GetResponse().HttpStatusCode() == 404 &&
    frame_->GetDocumentLoader()->GetResponse().MimeType() == "text/plain") {
      use_custom_error_page = true;
  }
  
  // wangjuna: 添加404页面错误特殊处理
  // if ((!document_is_empty || !IsMainFrame()) && frame_->GetDocumentLoader()->GetResponse().HttpStatusCode() != 404) {
  if ((!document_is_empty || !IsMainFrame()) && !use_custom_error_page) {
    return;
  }
#endif
  // modify by xiaohuyang, fix bug#63969,  2021/05/27  --end
...
RenderWidget* RenderFrameImpl::GetWidget() {
  return render_widget_;
}

const RenderWidget* RenderFrameImpl::GetWidget() const {
  return render_widget_;
}
...
void RenderFrameImpl::RegisterPluginDelegate(WebPluginDelegateProxy* delegate) {  
  ...
  delegate->SetContentAreaFocus(GetLocalRootRenderWidget()->has_focus());
