...
bool RenderThreadImpl::Send(IPC::Message* msg) {
  ...
  #if defined(USE_UNIONTECH_NPAPI)
    RenderViewImpl* render_view = RenderViewImpl::FromRoutingID(msg->routing_id());
    if (render_view) {
      render_view_id = msg->routing_id();
      LOG(INFO) << " Broadcast PluginMsg_SignalModalDialogEvent";
  ...
  if (pumping_events) {
    if (render_view_id != MSG_ROUTING_NONE) {
      LOG(INFO) << " Broadcast PluginMsg_ResetModalDialogEvent";
      PluginChannelHost::Broadcast(
...
#if 0
  // uos:
  WebString uos_scheme(WebString::FromASCII(kUosUIScheme));
  WebSecurityPolicy::RegisterURLSchemeAsDisplayIsolated(uos_scheme);
  WebSecurityPolicy::RegisterURLSchemeAsNotAllowingJavascriptURLs(uos_scheme);
#endif
  // chrome-untrusted:
