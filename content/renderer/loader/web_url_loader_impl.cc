...
bool WebURLLoaderImpl::Context::OnReceivedRedirect(
  ...
  return client_->WillFollowRedirect(
      loader_, url_, redirect_info.new_site_for_cookies,
...
void WebURLLoaderImpl::Context::OnStartLoadingResponseBody(
  ...
  if (client_)
  #if defined(USE_UNIONTECH_NPAPI)
    client_->DidStartLoadingResponseBody(loader_, std::move(body));
  #else
...
#if defined(USE_UNIONTECH_NPAPI)
  client_->DidReceiveTransferSizeUpdate(loader_, transfer_size_diff);
#else
...
void WebURLLoaderImpl::Context::OnReceivedCachedMetadata(
  ...
  TRACE_EVENT_WITH_FLOW1(
      ...
      data.size());
#if defined(USE_UNIONTECH_NPAPI)
  client_->DidReceiveCachedMetadata(loader_, std::move(data));
#else
...
void WebURLLoaderImpl::Context::OnCompletedRequest(
  ...
  if (client_) {
    ...
    #if defined(USE_UNIONTECH_NPAPI)
      client_->DidFail(loader_, PopulateURLError(status, url_), total_transfer_size,
    ...
    #if defined(USE_UNIONTECH_NPAPI)
      client_->DidFinishLoading(loader_, status.completion_time, total_transfer_size,
                                encoded_body_size, status.decoded_body_length,
...
void WebURLLoaderImpl::Context::CancelBodyStreaming() {
  ...
  #if defined(USE_UNIONTECH_NPAPI)
    client_->DidFail(loader_, WebURLError(net::ERR_ABORTED, url_),
