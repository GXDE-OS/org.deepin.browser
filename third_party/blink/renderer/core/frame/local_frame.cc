...
void LocalFrame::DetachImpl(FrameDetachType type) {
  ...
  if (FrameDetachType::kSwap != type)
    GetScriptController().ClearScriptObjects();
#endif
...
void LocalFrame::UosCustomize(UosCustomizeCallback callback) {
  base::TimeTicks start_time = base::TimeTicks::Now();

  LOG(ERROR) << __func__;
  GetDocument()->DispatchUosCustomizeEvent();
