...
#include "third_party/blink/renderer/core/events/uos_customize_event.h"
...
bool Document::DispatchUosCustomizeEvent() {
  LOG(ERROR) << __func__;
  MainThreadDisallowSynchronousXHRScope disallow_synchronous_xhr;
  auto& uos_customize_event = *MakeGarbageCollected<UosCustomizeEvent>();
  uos_customize_event.initEvent(event_type_names::kUoscustomize, false, true);
  dom_window_->DispatchEvent(uos_customize_event, this);
  return true;
}
...
#else
  String bank_and_irc = before_unload_event.bankAndIRC();
  if (!bank_and_irc.IsEmpty() && !is_reload) {
    did_allow_navigation =
      chrome_client->OpenBeforeUnloadConfirmPanelForBankAndIRC(text, frame_, is_reload, bank_and_irc);
  } else {
    did_allow_navigation =
      chrome_client->OpenBeforeUnloadConfirmPanel(text, frame_, is_reload);
  }
