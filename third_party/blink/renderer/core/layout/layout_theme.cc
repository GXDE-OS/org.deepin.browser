...
Color LayoutTheme::PlatformTextSearchHighlightColor(
    ...
    WebColorScheme color_scheme) const {
  //modify by xiepinze, set search text background color, 20201201, start
  //if (active_match) {
  ...
  if (active_match) {
    if (in_forced_colors_mode)
      return GetTheme().SystemColor(CSSValueID::kHighlight, color_scheme);
    return Color(1, 236, 255);
  }
  return Color(123, 188, 251);
  //modify by xiepinze, set search text background color, 20201201, end
}
