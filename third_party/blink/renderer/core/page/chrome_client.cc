...
bool ChromeClient::OpenBeforeUnloadConfirmPanelForBankAndIRC(const String& message,
                                                             LocalFrame* frame,
                                                             bool is_reload,
                                                             const String& bank) {
  DCHECK(frame);
  return OpenJavaScriptDialog(frame, message, [this, frame, is_reload, bank, message]() {
    return OpenBeforeUnloadConfirmPanelDelegateForBankAndIRC(frame, is_reload, bank, message);
  });
}
