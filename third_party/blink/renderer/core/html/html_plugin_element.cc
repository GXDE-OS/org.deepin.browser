...
NPObject* HTMLPlugInElement::getNPObject() {
  ...
  if (!m_NPObject && GetDocument().GetFrame())
    m_NPObject = GetDocument()
                     .GetFrame()
                     ->GetScriptController()
                     .CreateScriptObjectForPluginElement(this);
  return m_NPObject;
}
...
void HTMLPlugInElement::setPluginFocus(bool focused) {
  ...
  if (getNPObject() && PluginEmbeddedContentView())
    PluginEmbeddedContentView()->SetFocused(focused, mojom::blink::FocusType::kNone);
}
...
v8::Local<v8::Object> HTMLPlugInElement::PluginWrapper(v8::Isolate* isolate_nouse) {
...
bool HTMLPlugInElement::LoadPlugin(const KURL& url,
  ...
  LOG(INFO) << "[NPAPI] Plugin URL:" << url.GetString();
