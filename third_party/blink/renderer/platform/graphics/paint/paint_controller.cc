...
void PaintController::FinishCycle() {
  ...
  if (VLOG_IS_ON(1)) {
    ...
    if (usage_ != kTransient) {
      //delete by xiepinze, don't print "PaintController::FinishCycle() completed", 2020/10/12, start
      //LOG(ERROR) << "PaintController::FinishCycle() completed";
      //delete by xiepinze, don't print "PaintController::FinishCycle() completed", 2020/10/12, end
#if DCHECK_IS_ON()
      if (VLOG_IS_ON(3))
