...
#include "../crypto/skf/skf_manager.h"
#include "internal.h"
...
// GMTLS
    ...
    {
        "-ukey", kOptionalArgument,
        "use usbkey",
    },
    {
        "-skf-module-dir", kOptionalArgument,
        "directory contains skf module",
    },
    {
        "-dkey", kOptionalArgument,
        "PEM-encoded file containing the private key.",
    },
    {
        "-dcert", kOptionalArgument,
        "PEM-encoded file containing the leaf certificate and optional "
        "certificate chain. This is taken from the -key argument if this "
        "argument is not provided.",
    },
...
bool Client(const std::vector<std::string> &args) {
  ...
  SSL_CTX_set_gm_implement(ctx.get(),tassl_like);
  skf_module_enumerator * enumerator=skf_module_enumerator::get_enumerator();
  if(args_map.count("-skf-module-dir")!=0)
  {
    if(false==enumerator->EnumDirectory(args_map["-skf-module-dir"]))
    {
      fprintf(stderr, "Enumerate Skf Module Failed\n");
      return false;
    }
  }

if(args_map.count("-ukey")==0)
{
...
// GMTLS
#ifndef OPENSSL_NO_GMTLS
    if (args_map.count("-dkey") != 0) {
    const std::string &key = args_map["-dkey"];
    if (!SSL_CTX_use_enc_PrivateKey_file(ctx.get(), key.c_str(),
                                     SSL_FILETYPE_PEM)) {
      fprintf(stderr, "Failed to load private key: %s\n", key.c_str());
      return false;
    }
    const std::string &cert =
        args_map.count("-dcert") != 0 ? args_map["-dcert"] : key;
    if (!SSL_CTX_use_enc_certificate_file(ctx.get(), cert.c_str(),SSL_FILETYPE_PEM)) {
      fprintf(stderr, "Failed to load cert chain: %s\n", cert.c_str());
      return false;
    }
  }
  ...
  {
    SSL_CTX_use_skf_module(ctx.get(),"libmkskf.so");
    SSL_CTX_use_skf_containner(ctx.get(),"","","","88888888");
  }
